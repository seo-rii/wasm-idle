...
#endif
`,U=[{path:`include/setjmp.h`,contents:H},{path:`include/bits/stdc++.h`,contents:Ee},{path:`include/bits/extc++.h`,contents:De},{path:`include/c++/v1/ext/rope`,contents:Te},{path:`include/c++/v1/ext/pb_ds/tree_policy.hpp`,contents:xe},{path:`include/c++/v1/ext/pb_ds/assoc_container.hpp`,contents:Se},{path:`include/c++/v1/ext/pb_ds/hash_policy.hpp`,contents:Ce},{path:`include/c++/v1/ext/pb_ds/priority_queue.hpp`,contents:we}],Oe=[`include/c++/v1/ext/pb_ds`,`include/bits`],W=(e,t)=>{let n=e.replace(/\/+$/,``),r=t.replace(/^\/+/,``);return n?`${n}/${r}`:r};function ke(e,t=``){for(let n of Oe)e.mkdirTree(W(t,n));for(let n of U)e.writeFile(W(t,n.path),n.contents)}var G=class{inJson=!1;rawText=[];unbalancedBraces=0;inString=!1;inEscape=0;textDecoder=new TextDecoder;insert(e){if(!this.inJson&&e===123&&(this.inJson=!0,this.rawText=[]),!this.inJson)return null;if(this.rawText.push(e),this.inString)this.inEscape?(e===75&&(this.inEscape+=4),--this.inEscape):e===92?this.inEscape=1:e===34&&(this.inString=!1);else if(e===123)this.unbalancedBraces+=1;else if(e===125){if(--this.unbalancedBraces,this.unbalancedBraces===0)return this.inJson=!1,this.textDecoder.decode(new Uint8Array(this.rawText))}else e===34&&(this.inString=!0);return null}};const K=e=>e.byteLength>=2&&e[0]===31&&e[1]===139;async function Ae(e,t,n,r){let i=e.getReader(),a=r,o=!1;if(a?.aborted){o=!0;let e=q(a);try{Promise.resolve(i.cancel(e)).catch(()=>{})}catch{}try{i.releaseLock()}catch{}throw e}let s,c=a?new Promise((e,t)=>{s=()=>{if(o)return;o=!0;let e=q(a);try{Promise.resolve(i.cancel(e)).catch(()=>{})}catch{}t(e)},a.addEventListener(`abort`,s,{once:!0})}):void 0,l=new Uint8Array(Math.min(65536,n)),u=0,d=!1,f,p;try{for(J(a);;){let e=i.read(),{done:r,value:o}=c?await Promise.race([e,c]):await e;if(J(a),r)break;if(!o)continue;let s=u+o.byteLength;if(s>n)throw Error(`Runtime asset ${t} decompressed size exceeds the ${n} byte limit`);if(s>l.byteLength){let e=Math.min(n,Math.max(s,Math.max(l.byteLength*2,1))),t=new Uint8Array(e);t.set(l.subarray(0,u)),l=t}l.set(o,u),u=s}J(a),f=l.subarray(0,u),d=!0}catch(e){if(a?.aborted)throw q(a);if(!o){o=!0;try{Promise.resolve(i.cancel(e)).catch(()=>{})}catch{}}throw e}finally{s&&a?.removeEventListener(`abort`,s);try{i.releaseLock()}catch(e){d&&(p={error:e})}}if(p)throw p.error;return f}function q(e){return e.reason??new DOMException(`Runtime asset load aborted`,`AbortError`)}function J(e){if(e?.aborted)throw q(e)}async function je(e,t=`runtime asset`,n=134217728,r){if(!Number.isSafeInteger(n)||n<0)throw Error(`Runtime asset decompression limit must be a non-negative safe integer`);if(J(r),!K(e)){if(e.byteLength>n)throw Error(`Runtime asset ${t} decompressed size exceeds the ${n} byte limit`);return e}if(typeof DecompressionStream!=`function`)throw Error(`Failed to decompress runtime asset ${t}: DecompressionStream('gzip') is unavailable`);try{let i=Uint8Array.from(e),a=new ReadableStream({start(e){e.enqueue(i),e.close()}}),o=new DecompressionStream(`gzip`);return await Ae(a.pipeThrough({readable:o.readable,writable:o.writable}),t,n,r)}catch(e){throw r?.aborted?q(r):Error(`Failed to decompress runtime asset ${t}: ${e instanceof Error?e.message:String(e)}`)}}const Y=`/workspace`;`${Y}`;const X=`${Y}/main.cpp`;`${X}`;const Me=e=>e.endsWith(`/`)?e.slice(0,-1):e,Z=()=>[`-std=gnu++2a`,`-xc++`,`--target=wasm32-wasi`,`-isystem/usr/include/c++/v1`,`-isystem/usr/include/wasm32-wasi/c++/v1`,`-isystem/usr/include`,`-isystem/usr/include/wasm32-wasi`],Ne=new TextEncoder,Pe=`${Y}/`;function Fe(e){let t=F(e.startsWith(Pe)?e.slice(Pe.length):e),n=Ne.encode(t).byteLength;if(n>M.maxPathBytes)throw new j(`path-size-limit`,`Clangd workspace path ${t} is ${n} bytes; limit is ${M.maxPathBytes}`,{path:t,limit:M.maxPathBytes,actual:n});return`${Pe}${t}`}var Ie=class{maxFiles;paths=new Set([X]);constructor(e=M.maxFiles){if(this.maxFiles=e,!Number.isSafeInteger(e)||e<1)throw new j(`invalid-limit`,`Clangd workspace file limit must be a positive safe integer`)}register(e){let t=Fe(e);if(this.paths.has(t))return{path:t,added:!1};let n=this.paths.size+1;if(n>this.maxFiles)throw new j(`file-count-limit`,`Clangd workspace contains ${n} files; limit is ${this.maxFiles}`,{limit:this.maxFiles,actual:n});return this.paths.add(t),{path:t,added:!0}}unregister(e){e!==X&&this.paths.delete(e)}};const Le=new TextEncoder,Re=new TextDecoder,ze=new G;let Be=()=>{};const Ve=[],He=[];let Ue=!1,We=``;const Ge=e=>{let t=e;return!t||typeof t!=`object`?typeof e:typeof t.method==`string`?t.method:t.id===void 0?`unknown`:`response:${String(t.id)}`},Ke=e=>{let t=e;return!!t&&typeof t==`object`&&t.jsonrpc===`2.0`},Q=(...e)=>{Ue&&console.debug(`[wasm-idle:clangd-worker]`,...e)},qe=()=>{if(He.length===0){if(Ve.length===0)return null;let e=Ve.shift();if(!e)return null;He.push(...Le.encode(e))}return He.shift()??null},Je=async()=>{if(Ve.length===0)return new Promise(e=>{Be=e})};let Ye=null,$=null;const Xe=new Ie,Ze=e=>{let t=ze.insert(e);if(!t||!Ye)return;let n=JSON.parse(t);Q(`stdout`,Ge(n)),Ye.write(n)},Qe=e=>{if(Ue){if(e===10||e===13){We&&Q(`stderr`,We),We=``;return}We+=String.fromCharCode(e)}},$e=()=>{Ye?.end(),self.reportError?.(`clangd aborted`)},et=e=>{let t=Fe(e);if(!$)return;let n=Xe.register(t),r=n.path.lastIndexOf(`/`),i=r>0?n.path.slice(0,r):Y;try{$.FS.mkdirTree(i),$.FS.writeFile(n.path,``)}catch(e){throw n.added&&Xe.unregister(n.path),e}};self.addEventListener(`message`,async e=>{if(e.data?.type===`sync-file`){try{if(typeof e.data.name!=`string`)throw TypeError(`clangd sync-file name must be a string`);et(e.data.name)}catch(e){self.postMessage({type:`error`,message:`Failed to sync clangd workspace file: ${e instanceof Error?e.message:String(e)}`})}return}if(e.data?.type===`init`){Ue=!!e.data.debug;try{if(typeof e.data.baseUrl!=`string`||!e.data.baseUrl.trim())throw Error(`clangd init requires an explicit baseUrl`);if(Q(`init`,Me(e.data.baseUrl.trim())+`/`),!e.data.assets)throw Error(`clangd init requires preloaded runtime assets`);let t=new Uint8Array(e.data.assets.clangdJs);self.postMessage({type:`progress`,value:1,max:3});let n=Re.decode(t),r=URL.createObjectURL(new Blob([n],{type:`text/javascript;charset=utf-8`})),i=new Uint8Array(e.data.assets.clangdWasmGz);self.postMessage({type:`progress`,value:2,max:3});let a=await je(i,`clangd.wasm.gz`);e.data.assets.clangdWasmIntegrity&&await D({asset:`clangd.wasm.gz`,bytes:a,expected:e.data.assets.clangdWasmIntegrity,stage:`uncompressed`,mimeType:`application/wasm`,runtimeId:`clangd`}),self.postMessage({type:`progress`,value:3,max:3});let o=import(r),s=new Uint8Array(a.byteLength);s.set(a);let c=new Blob([s.buffer],{type:`application/wasm`}),l=URL.createObjectURL(c),{default:u}=await o;$=await u({thisProgram:`/usr/bin/clangd`,mainScriptUrlOrBlob:r,locateFile:(e,t)=>e.endsWith(`.wasm`)?l:`${t}${e}`,stdinReady:Je,stdin:qe,stdout:Ze,stderr:Qe,onExit:$e,onAbort:$e}),$.FS.mkdirTree(Y),ke($.FS,`/usr`),et(X),$.FS.writeFile(`${Y}/.clangd`,JSON.stringify({CompileFlags:{Add:Z()}})),Q(`callMain start`),Q(`callMain returned`,$.callMain([])),Ye=new S(self),new x(self).listen(e=>{if(!Ke(e)){Q(`ignored control message`,Ge(e));return}Q(`stdin message`,Ge(e));let t=JSON.stringify(e).replace(/[\u007F-\uFFFF]/g,e=>`\\u`+e.codePointAt(0)?.toString(16).padStart(4,`0`)),n=Le.encode(t).byteLength;Ve.push(`Content-Length: ${n}\r\n\r\n${t}`),Q(`stdin queued bytes`,n),Be()}),self.postMessage({type:`ready`,value:a.byteLength})}catch(e){self.postMessage({type:`error`,message:e instanceof Error?e.message:String(e)})}}});