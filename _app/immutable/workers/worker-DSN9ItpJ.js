...
`;let X=null,Z=null,Q=null,$=`/pyodide/`;function be(e){return e instanceof Error?e.message:String(e)}function xe(e){return e.endsWith(`/`)?e:`${e}/`}function Se(e){try{self.postMessage(JSON.parse(e))}catch(t){console.error(`Failed to emit Python LSP payload`,t,e)}}function Ce(e){e.FS.mkdirTree(`/wasm_idle_lsp/wasm_idle_python_lsp`),e.FS.writeFile(`/wasm_idle_lsp/wasm_idle_python_lsp/__init__.py`,ve),e.FS.writeFile(`/wasm_idle_lsp/wasm_idle_python_lsp/server.py`,ye)}async function we(e){if(!Z)return Q||(Q=(async()=>{self.postMessage({type:`progress`,stage:`load-pyodide`}),X=await Y({indexURL:xe(e)}),X.registerJsModule(`wasm_idle_lsp_bridge`,{emit:Se}),self.postMessage({type:`progress`,stage:`load-jedi`});try{await X.loadPackage(`jedi`)}catch(e){console.warn(`Python LSP semantic features disabled because jedi failed to load`,e)}Ce(X),await X.runPythonAsync(`
import sys
...
`);let t=X.globals.get(`_wasm_idle_python_lsp_bridge`);Z=e=>{t(e)},self.postMessage({type:`ready`})})().catch(e=>{let t=be(e);throw console.error(`Python LSP bootstrap failed`,e),self.postMessage({type:`error`,error:t}),Q=null,Z=null,e}),Q)}self.addEventListener(`message`,e=>{let t=e.data;if(!t||typeof t!=`object`)return;if(t.type===`init`){$=typeof t.pyodideBaseUrl==`string`?t.pyodideBaseUrl:`/pyodide/`,we($).catch(e=>{console.error(`Failed to initialize Python LSP worker`,e)});return}if(`type`in t)return;let n=JSON.stringify(t);if(Z){Z(n);return}we($).then(()=>{Z?.(n)}).catch(e=>{console.error(`Failed to forward LSP message to Python bridge`,e)})});