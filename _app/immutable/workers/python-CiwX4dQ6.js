...
`,r=``,i=[];for(let r of e)r?.end===void 0?r?.sep===void 0?i.push(r):t=s(r.sep):n=s(r.end);for(let e=0;e<i.length;e++)(typeof i[e]==`string`||!i[e]?.end&&!i[e]?.sep)&&(r+=s(i[e]),e<i.length-1&&(r+=t));r+=n,postMessage({output:r})};let x=`__wasm_idle_python_debug_pause_${e}`,S=`__wasm_idle_python_debug_wait_${e}`,C=`__wasm_idle_python_debug_watch_read_${e}`,w=`__wasm_idle_python_debug_watch_write_${e}`,T=`__wasm_idle_python_debug_breakpoints_${e}`,E=`__wasm_idle_python_execution_ready_${e}`,D=!1;self[E]=()=>{D||(D=!0,delete self[E],postMessage({progress:{kind:`ready`,state:`running`,reason:`started`,label:`Python program started`}}))},self[x]=(e,t,n,r)=>{let i,a;try{i=JSON.parse(n)}catch{i=[]}try{a=JSON.parse(r)}catch{a=[]}postMessage({debugEvent:{type:`pause`,line:Number(e),reason:t,locals:i,callStack:a}})},self[S]=()=>{let e=Atomics.load(N,0);for(;;){if(I?.[0]===2||(Atomics.wait(N,0,e,100),I?.[0]===2))return-1;let t=Atomics.exchange(N,1,0);if(t)return t}},self[C]=()=>l(P)||``,self[w]=e=>{c([e],F)},self[T]=()=>{let e=Atomics.load(N,2),t=Math.max(0,Math.min(Atomics.load(N,3),N.length-4)),n=[];for(let e=0;e<t;e+=1){let t=Atomics.load(N,4+e);Number.isInteger(t)&&t>0&&n.push(t)}return JSON.stringify({version:e,lines:n})};let O=W(v||``)||`__wasm_idle_user__.py`,k=W(y||``)||O,A=JSON.stringify(O),j=JSON.stringify(k),z=JSON.stringify([...Array.isArray(g)?g:[]].map(e=>Number(e)).filter(e=>Number.isInteger(e)&&e>0));try{await L.runPythonAsync(`import ast
import builtins
import inspect
import json
import sys
...
${h?`
__wasm_idle_debug_breakpoints = set(${z})
