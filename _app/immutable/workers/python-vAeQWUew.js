...
`,r=``,i=[];for(let r of e)r?.end===void 0?r?.sep===void 0?i.push(r):t=s(r.sep):n=s(r.end);for(let e=0;e<i.length;e++)(typeof i[e]==`string`||!i[e]?.end&&!i[e]?.sep)&&(r+=s(i[e]),e<i.length-1&&(r+=t));r+=n,postMessage({output:r})};let x=`__wasm_idle_python_debug_pause_${e}`,S=`__wasm_idle_python_debug_wait_${e}`,C=`__wasm_idle_python_debug_watch_read_${e}`,w=`__wasm_idle_python_debug_watch_write_${e}`;self[x]=(e,t,n,r)=>{let i,a;try{i=JSON.parse(n)}catch{i=[]}try{a=JSON.parse(r)}catch{a=[]}postMessage({debugEvent:{type:`pause`,line:Number(e),reason:t,locals:i,callStack:a}})},self[S]=()=>{let e=Atomics.load(j,0);for(;;){if(P?.[0]===2||(Atomics.wait(j,0,e,100),P?.[0]===2))return-1;let t=Atomics.exchange(j,1,0);if(t)return t}},self[C]=()=>l(M)||``,self[w]=e=>{c([e],N)};let T=H(v||``)||`__wasm_idle_user__.py`,E=H(y||``)||T,D=JSON.stringify(T),O=JSON.stringify(E),k=JSON.stringify([...Array.isArray(g)?g:[]].map(e=>Number(e)).filter(e=>Number.isInteger(e)&&e>0));try{await F.runPythonAsync(`import ast
import builtins
import inspect
import json
import sys
...
${h?`
__wasm_idle_debug_breakpoints = set(${k})
...
def __wasm_idle_debug_depth(frame):
    ...
    while current is not None:
        if current.f_code.co_filename == ${O}:
            depth += 1
...
try:
    ...
    __wasm_idle_result = eval(
        compile(${JSON.stringify(n)}, ${D}, "exec", flags = ast.PyCF_ALLOW_TOP_LEVEL_AWAIT),
        __wasm_idle_globals,
        __wasm_idle_globals,
    )
    ...
    __wasm_idle_debug_step_out_depth = None
`:``}
`),self.postMessage({results:!0})}catch(e){self.postMessage({error:e.message||`Unknown error`})}finally{delete self[x],delete self[S],delete self[C],delete self[w]}}};