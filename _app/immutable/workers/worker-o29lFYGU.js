var e=`from .server import create_bridge
...
`;let n=null,r=null,i=null,a=null;function o(e){return e instanceof Error?e.message:String(e)}function s(e){return e.endsWith(`/`)?e:`${e}/`}function c(e){try{self.postMessage(JSON.parse(e))}catch(t){console.error(`Failed to emit Python LSP payload`,t,e)}}function l(n){n.FS.mkdirTree(`/wasm_idle_lsp/wasm_idle_python_lsp`),n.FS.writeFile(`/wasm_idle_lsp/wasm_idle_python_lsp/__init__.py`,e),n.FS.writeFile(`/wasm_idle_lsp/wasm_idle_python_lsp/server.py`,t)}async function u(e){if(!r)return i||(i=(async()=>{self.postMessage({type:`progress`,stage:`load-pyodide`});let{loadPyodide:t}=await import(`${s(e)}pyodide.mjs`);n=await t({indexURL:s(e)}),n.registerJsModule(`wasm_idle_lsp_bridge`,{emit:c}),self.postMessage({type:`progress`,stage:`load-jedi`});try{await n.loadPackage(`jedi`)}catch(e){console.warn(`Python LSP semantic features disabled because jedi failed to load`,e)}l(n),await n.runPythonAsync(`
import sys
...
`);let i=n.globals.get(`_wasm_idle_python_lsp_bridge`);r=e=>{i(e)},self.postMessage({type:`ready`})})().catch(e=>{let t=o(e);throw console.error(`Python LSP bootstrap failed`,e),self.postMessage({type:`error`,error:t}),i=null,r=null,e}),i)}self.addEventListener(`message`,e=>{let t=e.data;if(!t||typeof t!=`object`)return;if(t.type===`init`){if(typeof t.pyodideBaseUrl!=`string`||!t.pyodideBaseUrl.trim()){self.postMessage({type:`error`,error:`Python LSP init requires an explicit pyodideBaseUrl`});return}a=t.pyodideBaseUrl.trim(),u(a).catch(e=>{console.error(`Failed to initialize Python LSP worker`,e)});return}if(`type`in t)return;if(!a){self.postMessage({type:`error`,error:`Python LSP worker requires init before JSON-RPC messages`});return}let n=JSON.stringify(t);if(r){r(n);return}u(a).then(()=>{r?.(n)}).catch(e=>{console.error(`Failed to forward LSP message to Python bridge`,e)})});