...
${p?`
__wasm_idle_debug_breakpoints = set(${x})
...
def __wasm_idle_debug_depth(frame):
    ...
    while current is not None:
        if current.f_code.co_filename == "${b}":
            depth += 1
...
try:
    ...
    __wasm_idle_result = eval(
        compile(${JSON.stringify(t)}, "${b}", "exec", flags = ast.PyCF_ALLOW_TOP_LEVEL_AWAIT),
        __wasm_idle_globals,
        __wasm_idle_globals,
    )
    ...
    __wasm_idle_debug_step_out_depth = None
`:``}
`),self.postMessage({results:!0})}catch(e){self.postMessage({error:e.message||`Unknown error`})}finally{delete self[d],delete self[f],delete self[v],delete self[y]}}};