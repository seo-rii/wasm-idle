...
#endif
`,w=String.raw`#ifndef WASM_CLANG_EXT_PB_DS_ASSOC_CONTAINER_HPP
...
#endif
`,T=String.raw`#ifndef WASM_CLANG_EXT_PB_DS_HASH_POLICY_HPP
...
#endif
`,E=String.raw`#ifndef WASM_CLANG_EXT_PB_DS_PRIORITY_QUEUE_HPP
...
#endif
`,D=String.raw`#ifndef WASM_CLANG_EXT_ROPE
...
#endif
`,O=[{path:`include/bits/extc++.h`,contents:String.raw`#ifndef WASM_CLANG_BITS_EXTCXX_H
...
#endif
`},{path:`include/c++/v1/ext/rope`,contents:D},{path:`include/c++/v1/ext/pb_ds/tree_policy.hpp`,contents:C},{path:`include/c++/v1/ext/pb_ds/assoc_container.hpp`,contents:w},{path:`include/c++/v1/ext/pb_ds/hash_policy.hpp`,contents:T},{path:`include/c++/v1/ext/pb_ds/priority_queue.hpp`,contents:E}],k=[`include/c++/v1/ext/pb_ds`,`include/bits`],A=(e,t)=>{let n=e.replace(/\/+$/,``),r=t.replace(/^\/+/,``);return n?`${n}/${r}`:r};function j(e,t=``){for(let n of k)e.mkdirTree(A(t,n));for(let n of O)e.writeFile(A(t,n.path),n.contents)}const M=`/workspace`;`${M}`;const N=`${M}/main.cpp`;`${N}`;const P=e=>e.endsWith(`/`)?e.slice(0,-1):e,F=()=>[`-std=gnu++2a`,`-xc++`,`--target=wasm32-wasi`,`-isystem/usr/include/c++/v1`,`-isystem/usr/include/wasm32-wasi/c++/v1`,`-isystem/usr/include`,`-isystem/usr/include/wasm32-wasi`];var I=class{inJson=!1;rawText=[];unbalancedBraces=0;inString=!1;inEscape=0;textDecoder=new TextDecoder;insert(e){if(!this.inJson&&e===123&&(this.inJson=!0,this.rawText=[]),!this.inJson)return null;if(this.rawText.push(e),this.inString)this.inEscape?(e===75&&(this.inEscape+=4),--this.inEscape):e===92?this.inEscape=1:e===34&&(this.inString=!1);else if(e===123)this.unbalancedBraces+=1;else if(e===125){if(--this.unbalancedBraces,this.unbalancedBraces===0)return this.inJson=!1,this.textDecoder.decode(new Uint8Array(this.rawText))}else e===34&&(this.inString=!0);return null}};const L=new TextEncoder,R=new TextDecoder,ie=new I;let z=()=>{};const B=[],V=[];let H=!1,U=``;const W=e=>{let t=e;return!t||typeof t!=`object`?typeof e:typeof t.method==`string`?t.method:t.id===void 0?`unknown`:`response:${String(t.id)}`},ae=e=>{let t=e;return!!t&&typeof t==`object`&&t.jsonrpc===`2.0`},G=(...e)=>{H&&console.debug(`[wasm-idle:clangd-worker]`,...e)},oe=()=>{if(V.length===0){if(B.length===0)return null;let e=B.shift();if(!e)return null;V.push(...L.encode(e))}return V.shift()??null},se=async()=>{if(B.length===0)return new Promise(e=>{z=e})};let K=null,q=null;const J=e=>{let t=ie.insert(e);if(!t||!K)return;let n=JSON.parse(t);G(`stdout`,W(n)),K.write(n)},ce=e=>{if(H){if(e===10||e===13){U&&G(`stderr`,U),U=``;return}U+=String.fromCharCode(e)}},le=()=>{K?.end(),self.reportError?.(`clangd aborted`)},ue=e=>{if(!q)return;let t=e.startsWith(`/workspace`)?e:`${M}/${e.replace(/^\/+/,``)}`,n=t.lastIndexOf(`/`),r=n>0?t.slice(0,n):M;q.FS.mkdirTree(r),q.FS.writeFile(t,``)};async function de(e,t){let n=await fetch(new URL(t,e).href);if(!n.ok)throw Error(`Failed to load ${t}: ${n.status}`);return new Uint8Array(await n.arrayBuffer())}async function fe(e){if(e[0]!==31||e[1]!==139)return e;if(typeof DecompressionStream>`u`)throw Error(`Failed to decompress clangd.wasm.gz: DecompressionStream is unavailable`);let t=new Uint8Array(e.byteLength);t.set(e);let n=new Blob([t.buffer]).stream().pipeThrough(new DecompressionStream(`gzip`));return new Uint8Array(await new Response(n).arrayBuffer())}self.addEventListener(`message`,async e=>{if(e.data?.type===`sync-file`&&typeof e.data?.name==`string`){ue(e.data.name);return}if(e.data?.type!==`init`)return;let t=P(e.data.baseUrl||`/clangd`)+`/`;H=!!e.data.debug,G(`init`,t);try{let n=e.data.assets?.clangdJs?new Uint8Array(e.data.assets.clangdJs):await de(t,`clangd.js`);self.postMessage({type:`progress`,value:1,max:3});let r=R.decode(n),i=URL.createObjectURL(new Blob([r],{type:`text/javascript;charset=utf-8`})),a=import(i),o=e.data.assets?.clangdWasmGz?new Uint8Array(e.data.assets.clangdWasmGz):await de(t,`clangd.wasm.gz`);self.postMessage({type:`progress`,value:2,max:3});let s=await fe(o);self.postMessage({type:`progress`,value:3,max:3});let c=new Uint8Array(s.byteLength);c.set(s);let l=new Blob([c.buffer],{type:`application/wasm`}),u=URL.createObjectURL(l),{default:d}=await a;q=await d({thisProgram:`/usr/bin/clangd`,mainScriptUrlOrBlob:i,locateFile:(e,t)=>e.endsWith(`.wasm`)?u:`${t}${e}`,stdinReady:se,stdin:oe,stdout:J,stderr:ce,onExit:le,onAbort:le}),q.FS.mkdirTree(M),j(q.FS,`/usr`),ue(N),q.FS.writeFile(`${M}/.clangd`,JSON.stringify({CompileFlags:{Add:F()}})),G(`callMain start`),G(`callMain returned`,q.callMain([])),K=new S(self),new x(self).listen(e=>{if(!ae(e)){G(`ignored control message`,W(e));return}G(`stdin message`,W(e));let t=JSON.stringify(e).replace(/[\u007F-\uFFFF]/g,e=>`\\u`+e.codePointAt(0)?.toString(16).padStart(4,`0`)),n=L.encode(t).byteLength;B.push(`Content-Length: ${n}\r\n\r\n${t}`),G(`stdin queued bytes`,n),z()}),self.postMessage({type:`ready`,value:s.byteLength})}catch(e){self.postMessage({type:`error`,message:e instanceof Error?e.message:String(e)})}});