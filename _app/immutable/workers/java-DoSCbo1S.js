...
`)||`TeaVM WebAssembly generation failed`);W=o,G=m.stdinCacheKey,K=e,q=new Uint8Array(I.getWebAssemblyOutputFile(`app.wasm`)),J=g,Y=v.map(e=>({...e}))}if(s){self.postMessage({results:!0});return}V=new Int32Array(a),H=e?new Uint8Array:new TextEncoder().encode(l),U=0;let b=globalThis,x=b.window;b.window=b,b.wasmIdleJavaStdin={readByte(){for(;;){if(U<H.length)return H[U++]??-1;if(e)return-1;let t=f(V,()=>self.postMessage({buffer:!0}));if(t===null)return-1;H=new TextEncoder().encode(t),U=0}}};try{let e=await L(q,{installImports(e){e.teavmConsole.putcharStdout=ie,e.teavmConsole.putcharStderr=ae},stackDeobfuscator:{enabled:!1}});self.postMessage({progress:{kind:`ready`,state:`running`,reason:`started`,label:`Java program started`}}),e.exports.main(c),Q(),$(),self.postMessage({results:!0,mainClass:K})}finally{delete b.wasmIdleJavaStdin,x===void 0?Reflect.deleteProperty(b,`window`):b.window=x,H=new Uint8Array,U=0,V=null}}catch(e){Q(),$(),self.postMessage({error:e instanceof Error?e.message:String(e)})}});