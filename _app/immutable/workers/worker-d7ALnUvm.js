...
#endif
`,k=[{path:`include/bits/extc++.h`,contents:String.raw`#ifndef WASM_CLANG_BITS_EXTCXX_H
...
#endif
`},{path:`include/c++/v1/ext/rope`,contents:O},{path:`include/c++/v1/ext/pb_ds/tree_policy.hpp`,contents:w},{path:`include/c++/v1/ext/pb_ds/assoc_container.hpp`,contents:T},{path:`include/c++/v1/ext/pb_ds/hash_policy.hpp`,contents:E},{path:`include/c++/v1/ext/pb_ds/priority_queue.hpp`,contents:D}],A=[`include/c++/v1/ext/pb_ds`,`include/bits`],j=(e,t)=>{let n=e.replace(/\/+$/,``),r=t.replace(/^\/+/,``);return n?`${n}/${r}`:r};function M(e,t=``){for(let n of A)e.mkdirTree(j(t,n));for(let n of k)e.writeFile(j(t,n.path),n.contents)}const N=`/workspace`;`${N}`;const P=`${N}/main.cpp`;`${P}`;const F=e=>e.endsWith(`/`)?e.slice(0,-1):e,I=()=>[`-std=gnu++2a`,`-xc++`,`--target=wasm32-wasi`,`-isystem/usr/include/c++/v1`,`-isystem/usr/include/wasm32-wasi/c++/v1`,`-isystem/usr/include`,`-isystem/usr/include/wasm32-wasi`];var L=class{inJson=!1;rawText=[];unbalancedBraces=0;inString=!1;inEscape=0;textDecoder=new TextDecoder;insert(e){if(!this.inJson&&e===123&&(this.inJson=!0,this.rawText=[]),!this.inJson)return null;if(this.rawText.push(e),this.inString)this.inEscape?(e===75&&(this.inEscape+=4),--this.inEscape):e===92?this.inEscape=1:e===34&&(this.inString=!1);else if(e===123)this.unbalancedBraces+=1;else if(e===125){if(--this.unbalancedBraces,this.unbalancedBraces===0)return this.inJson=!1,this.textDecoder.decode(new Uint8Array(this.rawText))}else e===34&&(this.inString=!0);return null}};const R=new TextEncoder,z=new TextDecoder,re=new L;let B=()=>{};const V=[],H=[],ie=()=>{if(H.length===0){if(V.length===0)return null;let e=V.shift();if(!e)return null;H.push(...R.encode(e),null)}return H.shift()??null},U=async()=>{if(V.length===0)return new Promise(e=>{B=e})};let W=null,G=null;const ae=e=>{let t=re.insert(e);t&&W&&W.write(JSON.parse(t))},oe=()=>{},se=()=>{W?.end(),self.reportError?.(`clangd aborted`)},K=e=>{if(!G)return;let t=e.startsWith(`/workspace`)?e:`${N}/${e.replace(/^\/+/,``)}`,n=t.lastIndexOf(`/`),r=n>0?t.slice(0,n):N;G.FS.mkdirTree(r),G.FS.writeFile(t,``)};async function q(e,t){let n=await fetch(new URL(t,e).href);if(!n.ok)throw Error(`Failed to load ${t}: ${n.status}`);return new Uint8Array(await n.arrayBuffer())}async function J(e){if(e[0]!==31||e[1]!==139)return e;if(typeof DecompressionStream>`u`)throw Error(`Failed to decompress clangd.wasm.gz: DecompressionStream is unavailable`);let t=new Uint8Array(e.byteLength);t.set(e);let n=new Blob([t.buffer]).stream().pipeThrough(new DecompressionStream(`gzip`));return new Uint8Array(await new Response(n).arrayBuffer())}self.addEventListener(`message`,async e=>{if(e.data?.type===`sync-file`&&typeof e.data?.name==`string`){K(e.data.name);return}if(e.data?.type!==`init`)return;let t=F(e.data.baseUrl||`/clangd`)+`/`;try{let n=e.data.assets?.clangdJs?new Uint8Array(e.data.assets.clangdJs):await q(t,`clangd.js`);self.postMessage({type:`progress`,value:1,max:3});let r=z.decode(n),i=URL.createObjectURL(new Blob([r],{type:`text/javascript;charset=utf-8`})),a=import(i),o=e.data.assets?.clangdWasmGz?new Uint8Array(e.data.assets.clangdWasmGz):await q(t,`clangd.wasm.gz`);self.postMessage({type:`progress`,value:2,max:3});let s=await J(o);self.postMessage({type:`progress`,value:3,max:3});let c=new Uint8Array(s.byteLength);c.set(s);let l=new Blob([c.buffer],{type:`application/wasm`}),u=URL.createObjectURL(l),{default:d}=await a;G=await d({thisProgram:`/usr/bin/clangd`,mainScriptUrlOrBlob:i,locateFile:(e,t)=>e.endsWith(`.wasm`)?u:`${t}${e}`,stdinReady:U,stdin:ie,stdout:ae,stderr:oe,onExit:se,onAbort:se}),G.FS.mkdirTree(N),M(G.FS,`/usr`),K(P),G.FS.writeFile(`${N}/.clangd`,JSON.stringify({CompileFlags:{Add:I()}})),G.callMain([]),W=new C(self),new S(self).listen(e=>{let t=JSON.stringify(e).replace(/[\u007F-\uFFFF]/g,e=>`\\u`+e.codePointAt(0)?.toString(16).padStart(4,`0`));V.push(`Content-Length: ${t.length}\r\n`,`\r
`,t),B()}),self.postMessage({type:`ready`,value:s.byteLength})}catch(e){self.postMessage({type:`error`,message:e instanceof Error?e.message:String(e)})}});