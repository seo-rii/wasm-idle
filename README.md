...
- `@seo-rii/wasm-llvm`: pinned external owner of shared memory/MemFS/tar/WASM helpers and the Clang,
  Objective-C, Swift, Nim LLVM 8, TinyGo emception, and legacy D/Rust Emscripten LLD profiles. Its
  `producer/rust-browser` source build owns the current Rust 1.99 compiler, matching LLVM 22 code
  generator, and in-process LLD; current Rust assets do not use the shared LLVM 16 split backend.
