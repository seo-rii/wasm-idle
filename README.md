...
| -------------- | ------------------------------------- | ------- | -------------------- | ----- |
| C              | @seo-rii/wasm-llvm / Clang WASI       | Yes     | clangd               | -     |
| C++            | @seo-rii/wasm-llvm / Clang WASI       | Yes     | clangd               | Trace |
| Objective-C    | GNUstep libobjc2 + @seo-rii/wasm-llvm | Yes     | clangd               | -     |
...
| Fortran        | f2c + @seo-rii/wasm-llvm              | Yes     | Fortran LSP          | -     |
| COBOL          | GnuCOBOL 3.2 + @seo-rii/wasm-llvm     | Yes     | syntax               | -     |
| R              | WebR                                  | Yes     | syntax               | -     |
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| C<br>`C`                           | @seo-rii/wasm-llvm@0.4.0 / Clang 22.1.8 WASI sysroot                                              | `clang` for `wasm32-wasi`; default `-std=gnu11`; WASI preview1 execution supports `stdin` and `programArgs`                                                                                                                                                                                                                                                                                                                                                                                          | `runtimeAssets.clang.baseUrl`/`loader` or `rootUrl`; `compileArgs`, `programArgs`, `cVersion`, `activePath`, `workspaceFiles`                                                                                                                                                              |
| C++<br>`CPP`                       | @seo-rii/wasm-llvm@0.4.0 / Clang 22.1.8 WASI sysroot                                              | `clang++` for `wasm32-wasi`; default `-std=gnu++2a`; trace debug uses wasm-idle controls; supports `stdin` and `programArgs`                                                                                                                                                                                                                                                                                                                                                                         | `runtimeAssets.clang.baseUrl`/`loader` or `rootUrl`; `compileArgs`, `programArgs`, `cppVersion`, `activePath`, `workspaceFiles`                                                                                                                                                            |
| Objective-C<br>`OBJC`              | GNUstep libobjc2 v2.3 (`static/wasm-objectivec/libobjc.a`) + @seo-rii/wasm-llvm@0.4.0             | `clang -x objective-c -fobjc-runtime=gnustep-2.0 -fblocks` for `wasm32-wasi`; links `libobjc.a`, `libgnustep-base.a`, and `libffi.a` when Foundation is imported; Foundation headers are inlined from `foundation-headers.json`; includes a constructor wrapper for Objective-C class registration; auto-compiles `.m` and `.c` workspace sources; supports `stdin` and `programArgs`; large Objective-C assets may be served as gzip-only `.gz` files through the service worker or worker fallback | `runtimeAssets.objectivec.baseUrl`/`libobjcUrl`/`headersUrl`/`libgnustepBaseUrl`/`libgnustepBaseObjectUrl`/`foundationHeadersUrl`/`libffiUrl` or `PUBLIC_WASM_OBJECTIVEC_*`; `runtimeAssets.clang.baseUrl`/`loader` for the clang toolchain; `activePath`, `workspaceFiles`, `compileArgs` |
...
| Rust<br>`RUST`                     | wasm-rust@0.1.0 / rust-1.79.0-dev-browser-split-v3                                                | default target `wasm32-wasip1`; selectable `wasm32-wasip1`, `wasm32-wasip2`, `wasm32-wasip3`; supports `stdin` and `programArgs`                                                                                                                                                                                                                                                                                                                                                                     | `runtimeAssets.rust.compilerUrl` or `PUBLIC_WASM_RUST_COMPILER_URL`; `rootUrl`, `rustTargetTriple`, `programArgs`                                                                                                                                                                          |
...
| C#<br>`CSHARP`                     | wasm-dotnet@0.1.0 / browser-wasm .NET module                                                      | `language=csharp` compile target `browser-wasm`; supports `stdin` and `programArgs`                                                                                                                                                                                                                                                                                                                                                                                                                  | `runtimeAssets.dotnet.moduleUrl` or `PUBLIC_WASM_DOTNET_MODULE_URL`                                                                                                                                                                                                                        |
| F#<br>`FSHARP`                     | wasm-dotnet@0.1.0 / browser-wasm .NET module                                                      | `language=fsharp` compile target `browser-wasm`; stdin is currently blocked                                                                                                                                                                                                                                                                                                                                                                                                                          | `runtimeAssets.dotnet.moduleUrl` or `PUBLIC_WASM_DOTNET_MODULE_URL`; `programArgs`                                                                                                                                                                                                         |
| VB.NET<br>`VBNET`                  | wasm-dotnet@0.1.0 / browser-wasm .NET module                                                      | `language=vbnet` compile target `browser-wasm`; supports `stdin` and `programArgs`                                                                                                                                                                                                                                                                                                                                                                                                                   | `runtimeAssets.dotnet.moduleUrl` or `PUBLIC_WASM_DOTNET_MODULE_URL`                                                                                                                                                                                                                        |
...
| AssemblyScript<br>`ASSEMBLYSCRIPT` | @wasm-idle/runtime-assemblyscript@0.1.0 / assemblyscript@0.28.17 + @assemblyscript/loader@0.28.17 | AssemblyScript compiler emits WASM and runs through WASI/browser imports; supports `stdin`                                                                                                                                                                                                                                                                                                                                                                                                           | `programArgs`, `stdin`; compiler assets come from bundled package                                                                                                                                                                                                                          |
...
| Scheme<br>`LISP`                   | wasm-lisp@0.1.0 / Puppy Scheme WASM component                                                     | Puppy Scheme compiler/runtime; stdin is not wired                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `runtimeAssets.lisp.moduleUrl` or `PUBLIC_WASM_LISP_MODULE_URL`; `programArgs`                                                                                                                                                                                                             |
...
| Fortran<br>`FORTRAN`               | Netlib f2c 2022-09-09 + `@cowasm/f2c 1.0.0` libf2c + @seo-rii/wasm-llvm@0.4.0                     | runs `f2c.wasm` in WASI, compiles generated C with wasm-llvm, links `libf2c.a`, then executes the resulting WASI module with `stdin` and `programArgs`                                                                                                                                                                                                                                                                                                                                               | `runtimeAssets.fortran.baseUrl`/`f2cWasmUrl`/`libf2cUrl`/`f2cHeaderUrl`/`analyzerUrl` or `PUBLIC_WASM_FORTRAN_*`; `runtimeAssets.clang.baseUrl`/`loader` for the C backend; `activePath`, `workspaceFiles`, `compileArgs`                                                                  |
| COBOL<br>`COBOL`                   | GnuCOBOL 3.2 + GMP 6.3.0 + @seo-rii/wasm-llvm@0.4.0                                               | translates free-format COBOL with the real GnuCOBOL `cobc` frontend, compiles the generated C with wasm-llvm, links libcob/GMP, and executes the resulting WASI module with `stdin` and `programArgs`                                                                                                                                                                                                                                                                                                | `runtimeAssets.cobol.baseUrl` or `PUBLIC_WASM_COBOL_BASE_URL`; `runtimeAssets.clang.baseUrl`/`loader` for the C backend; `activePath`, `workspaceFiles`, `compileArgs`                                                                                                                     |
...
- `packages/core`: framework-neutral contracts, runtime asset keys, progress helpers, and playground
  binding helpers.
- `@seo-rii/wasm-llvm`: pinned external owner of shared memory/MemFS/tar/WASM helpers and the Clang,
  Objective-C, Swift, D/Rust Emscripten LLD, Nim LLVM 8, Rust LLVM worker, and TinyGo emception
  profiles. D and Rust share one validated LLVM 16.0.4 LLD JS/WASM/data bundle.
...
Powered by [`@seo-rii/wasm-llvm`](https://github.com/seo-rii/wasm-llvm), Pyodide, TeaVM, `wasm-rust`,
