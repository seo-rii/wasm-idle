...
export interface CompileDotnetDependencies {
  loadRuntime?: () => Promise<DotnetCompilerRuntime>;
  loadReferences?: (
    language: DotnetLanguage,
  ) => Promise<DotnetReferenceAssembly[]>;
}
...
function emitProgress(
  ...
  request.onProgress?.({
    ...
    total,
    percent:
      total <= 0 ? 100 : Math.min(100, Math.max(0, (completed / total) * 100)),
    ...(message ? { message } : {}),
  });
}
      ...
      severity:
        groups.severity?.toLowerCase() === "warning"
          ...
          : groups.severity?.toLowerCase() === "error"
            ...
            : "other",
      message:
        `${groups.code ? `${groups.code}: ` : ""}${groups.message || match[0]}`.trim(),
    });
  }
  return diagnostics;
}
...
function resolveReferenceBaseUrl(options: CreateDotnetCompilerOptions) {
  const runtimeBaseUrl = new URL(
    options.runtimeBaseUrl || "./runtime/",
    import.meta.url,
  );
...
export async function compileDotnet(
  ...
  emitProgress(
    ...
    1,
    10,
    "loading .NET browser compiler runtime",
  );
  try {
    const runtime = await (
      dependencies.loadRuntime || loadDotnetCompilerRuntime
    )();
    ...
    emitProgress(
      ...
      5,
      10,
      `compiling ${languageLabel(language)} source`,
    );
