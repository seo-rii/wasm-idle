...
function ensureTrailingSlash(inputPath: string) {
	return inputPath.endsWith('/') ? inputPath : `${inputPath}/`;
}

function patchToolSource(source: string) {
	const mountNeedle = 'this.content={};this.root=a;';
	const mountReplacement =
		'this.content={};this.root=a;(globalThis.__jsoo_mounts||(globalThis.__jsoo_mounts=[])).push(this);';
	if (!source.includes(mountNeedle)) {
		throw new Error('failed to patch browser tool source for virtual fs mount exposure');
	}
	const createFileNeedle =
		/([A-Za-z$_][\w$]*)\.jsoo_create_file=([A-Za-z$_][\w$]*);\1\.jsoo_fs_tmp=\[\];return 0/;
	const forcedBrowserSource = source
		.replace('const isNode = globalThis.process?.versions?.node;', 'const isNode = false;')
		.replace(
			createFileNeedle,
			'(globalThis.__jsoo_created_files||(globalThis.__jsoo_created_files={}));$1.jsoo_create_file=(name,content)=>{globalThis.__jsoo_created_files[name]=content;return $2(name,content)};$1.jsoo_fs_tmp=[];return 0'
		);
...
function isBinaryenBridgeDebugEnabled(runtimeGlobal: RuntimeGlobal) {
	return runtimeGlobal['__wasm_bridge_debug'] === true;
}

function pushBinaryenBridgeMessage(runtimeGlobal: RuntimeGlobal, message: string) {
	const bridgeMessages = (runtimeGlobal['__wasm_bridge_messages'] as string[] | undefined) || [];
	bridgeMessages.push(message);
	runtimeGlobal['__wasm_bridge_messages'] = bridgeMessages;
}

function shellSplit(command: string) {
	const tokens: string[] = [];
	let current = '';
	let quote: "'" | '"' | '' = '';
...
function getMounts(runtimeGlobal: RuntimeGlobal) {
	return (runtimeGlobal['__jsoo_mounts'] as VirtualMount[] | undefined) || [];
}

function findMount(runtimeGlobal: RuntimeGlobal, targetPath: string) {
	const normalizedTarget = normalizePath(targetPath);
	let bestMatch: VirtualMount | undefined;
...
function readVirtualFile(runtimeGlobal: RuntimeGlobal, targetPath: string) {
	...
	const lookedUpEntry =
		typeof mount.lookup === 'function' ? mount.lookup(relativePath) : undefined;
...
function describeVirtualFile(runtimeGlobal: RuntimeGlobal, targetPath: string) {
	const mount = findMount(runtimeGlobal, targetPath);
	if (!mount) {
...
function listVirtualFiles(runtimeGlobal: RuntimeGlobal, prefixes: string[]) {
...
function collectVirtualFiles(runtimeGlobal: RuntimeGlobal, paths: string[]) {
	const files: Array<{ path: string; data: Uint8Array }> = [];
	const createdFiles = runtimeGlobal['__jsoo_created_files'] as
...
function writeVirtualFiles(
	...
	const createFile = runtimeGlobal['jsoo_create_file'] as
...
function ensureBinaryenCliDirectory(fs: Record<string, unknown>, targetPath: string) {
	...
	if (typeof fs.mkdirTree === 'function') {
		(fs.mkdirTree as (path: string) => void)(directory);
		return;
	}
	...
	for (const segment of segments) {
		current += `/${segment}`;
		try {
			(fs.mkdir as (path: string) => void)(current);
		} catch {
...
function parseBinaryenCommand(command: string) {
...
function runBinaryenTool(
	...
	if (
		...
		if (isBinaryenBridgeDebugEnabled(runtimeGlobal)) {
			pushBinaryenBridgeMessage(
				runtimeGlobal,
				`binaryen fast path: ${parsed.toolName} copied ${inputWasmPath} to ${outputWasmPath}`
			);
		}
		return 0;
	}
	...
	try {
		try {
			...
			const cliRuntime = runtimeGlobal['__binaryen_cli_runtime'] as
				| {
						...
						Module?: { arguments?: string[]; thisProgram?: string };
			...
			if (isBinaryenBridgeDebugEnabled(runtimeGlobal)) {
				for (const inputPath of parsed.inputPaths) {
					pushBinaryenBridgeMessage(
						runtimeGlobal,
						`binaryen input probe: ${describeVirtualFile(runtimeGlobal, inputPath)}`
					);
				}
			}
			for (const file of collectVirtualFiles(runtimeGlobal, parsed.inputPaths)) {
				if (isBinaryenBridgeDebugEnabled(runtimeGlobal)) {
					pushBinaryenBridgeMessage(
						runtimeGlobal,
						`binaryen input ready: ${file.path} (${file.data.byteLength} bytes, magic=${[...file.data.subarray(0, 4)].map((value) => value.toString(16).padStart(2, '0')).join(' ')})`
					);
				}
				ensureBinaryenCliDirectory(fs, file.path);
				(fs.writeFile as (path: string, data: Uint8Array) => void)(file.path, file.data);
			}
		...
		if (!fs || typeof fs.readFile !== 'function') {
			throw new Error(
				`static Binaryen tool did not leave behind a readable FS: ${parsed.toolName}`
			);
		}
			...
			writeVirtualFiles(runtimeGlobal, [
				{
					...
					data: new TextEncoder().encode(
						stderrParts.length > 0 ? `${stderrParts.join('\n')}\n` : ''
					)
				}
			]);
		}
		...
		for (const outputPath of parsed.outputPaths) {
			try {
				...
				if (isBinaryenBridgeDebugEnabled(runtimeGlobal)) {
					pushBinaryenBridgeMessage(
						runtimeGlobal,
						`binaryen output ready: ${outputPath} (${outputData.byteLength} bytes)`
					);
				}
			} catch {
				if (isBinaryenBridgeDebugEnabled(runtimeGlobal)) {
					...
					pushBinaryenBridgeMessage(
						runtimeGlobal,
						`binaryen output missing: ${outputPath}\ncli fs /: ${rootListing}\ncli fs /tmp: ${tmpListing}`
					);
				}
		...
		if (exitCode === 1 && outputFiles.length > 0 && stderrParts.length === 0) {
			if (isBinaryenBridgeDebugEnabled(runtimeGlobal)) {
				pushBinaryenBridgeMessage(
					runtimeGlobal,
					`binaryen normalized exit: ${parsed.toolName} reported 1 after producing ${outputFiles.length} output file(s)`
				);
			}
			exitCode = 0;
		}
		...
		const activeBinaryenCliRuntime = runtimeGlobal['__binaryen_cli_runtime'] as
			| {
					FS?: {
						quit?: () => void;
					};
...
self.addEventListener('message', async (event: MessageEvent<RunToolRequest>) => {
