...
export function createCompileHandler(options: CompileHandlerOptions = {}) {
	return async (
		...
		const compilePlan: CompilePlanCommand[] = [];
		...
		const compilePlanArtifact: CompileArtifact = {
		...
		fs.writeText(
			compilePlanArtifact.path,
			typeof compilePlanArtifact.data === 'string'
		...
		if (!options.system) {
			stderrParts.push(
				'compile pipeline is scaffolded, but no bytecode tool runner is configured yet'
			);
			return {
				success: false,
		...
		for (const command of compilePlan) {
			const result = await options.system(command.argv, {
				...
				env: {
					...
					WASM_OF_JS_OF_OCAML_BROWSER_FAST_BINARYEN:
						wasmBinaryenMode === 'full' ? '0' : '1'
				},
				fs
			});
			if (result.stdout) {
			...
			if (result.exitCode !== 0) {
				...
				for (let lineIndex = 0; lineIndex < stderrLines.length; lineIndex += 1) {
					...
					const primaryMessageIndex = messageLines.findIndex(
						(messageLine) =>
		...
		const artifacts: CompileArtifact[] = [compilePlanArtifact];
	...
	workerLikeGlobal.addEventListener('message', async (event) => {
		...
		try {
			const result = await handleCompile(
				(message as CompileWorkerRequestMessage).request,
				(message as CompileWorkerRequestMessage).manifest
			);
			workerLikeGlobal.postMessage?.({
				type: 'result',
				result
			} satisfies CompileWorkerResultMessage);
		} catch (error) {
			workerLikeGlobal.postMessage?.({
				type: 'error',
