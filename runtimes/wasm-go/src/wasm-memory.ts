...
function skipLimits(bytes: Uint8Array, offset: number, label: string) {
	...
	if ((flags.value & 0x04) !== 0 || (flags.value & 0x08) !== 0) {
		throw new Error(`${label} uses unsupported memory64 or custom-page limits`);
	}
...
export function capGoWasmMemory(bytes: Uint8Array, maxBytes: number, label: string) {
	if (
		!Number.isSafeInteger(maxBytes) ||
		maxBytes < WASM_PAGE_BYTES ||
		maxBytes > 65_536 * WASM_PAGE_BYTES
	) {
		throw new Error(`${label} memory limit must be between one page and 4 GiB`);
	}
	if (
...
export function assertGoInstanceMemoryLimit(
	instance: WebAssembly.Instance,
	maxBytes: number,
	label: string
) {
	for (const value of Object.values(instance.exports)) {
		if (value instanceof WebAssembly.Memory && value.buffer.byteLength > maxBytes) {
			throw new Error(
				`${label} memory ${value.buffer.byteLength} exceeds the hard limit ${maxBytes}`
			);
		}
	}
}
