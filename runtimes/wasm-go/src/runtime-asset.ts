...
export function parseRuntimePackIndex(value: unknown): RuntimePackIndex {
	...
	if (
		root.format !== 'wasm-go-runtime-pack-index-v1' &&
	...
	if (root.format === 'wasm-go-runtime-pack-index-v1') {
		const entries = parsedEntries.map(
			(entry) =>
				({
					runtimePath: entry.runtimePath,
					offset: entry.offset,
					length: entry.length
				}) satisfies RuntimeIdentityPackIndexEntry
		);
		return {
	...
	const entries = parsedEntries.map((entry, index) => {
		const decodedLength = expectNonNegativeInteger(
			entry.object.decodedLength,
			...
			...(entry.object.baseRuntimePath !== undefined
				? {
						baseRuntimePath: expectString(
							entry.object.baseRuntimePath,
							`root.entries[${index}].baseRuntimePath`
						)
					}
				: {})
		} satisfies RuntimeDeltaPackIndexEntry;
	});
...
type LoadedRuntimePackEntry = {
	runtimePath: string;
	bytes: Uint8Array;
};

function runtimePackKey(baseUrl: string | URL, pack: RuntimeAssetPackReference) {
...
async function loadRuntimePackEntriesRecursive(
	...
	pack: RuntimeAssetPackReference,
	fetchImpl: typeof fetch,
	ancestorPacks: ReadonlySet<string>,
	reportProgressForPack: (pack: RuntimeAssetPackReference) =>
		| {
	...
	if (index.format === 'wasm-go-runtime-pack-index-v1') {
		if (pack.delta !== undefined) {
			throw new Error(
				`runtime pack index ${pack.index} is an identity pack but its reference declares a delta`
			);
		}
	...
	if (!pack.delta || pack.delta.format !== 'copy-literal-v1') {
		throw new Error(
			`runtime delta pack index ${pack.index} requires a copy-literal-v1 base reference`
		);
	}

	const baseEntries = await loadRuntimePackEntriesRecursive(
		baseUrl,
		pack.delta.base,
		fetchImpl,
		nestedAncestorPacks,
	...
	const decodedEntries: LoadedRuntimePackEntry[] = [];
	for (const entry of index.entries) {
		...
		if (entry.baseRuntimePath !== undefined && !baseEntry) {
			throw new Error(
				`runtime delta entry ${entry.runtimePath} references missing base runtime path ${baseRuntimePath}`
			);
		}
		...
		while (encodedOffset < encodedBytes.byteLength) {
			...
			if (operation === 0) {
				if (encodedBytes.byteLength - encodedOffset < 4) {
					throw new Error(
						`malformed runtime delta stream for ${entry.runtimePath} at ${operationOffset}: truncated literal length`
					);
				}
				...
				if (literalLength > encodedBytes.byteLength - encodedOffset) {
					throw new Error(
						`malformed runtime delta stream for ${entry.runtimePath} at ${operationOffset}: literal exceeds encoded entry length`
					);
				}
				if (literalLength > decodedBytes.byteLength - decodedOffset) {
					throw new Error(
						`malformed runtime delta stream for ${entry.runtimePath} at ${operationOffset}: literal exceeds decodedLength ${entry.decodedLength}`
					);
				}
				decodedBytes.set(
			...
			if (operation === 1) {
				if (encodedBytes.byteLength - encodedOffset < 8) {
					throw new Error(
						`malformed runtime delta stream for ${entry.runtimePath} at ${operationOffset}: truncated copy range`
					);
				}
				...
				if (!baseEntry) {
					throw new Error(
						`malformed runtime delta stream for ${entry.runtimePath} at ${operationOffset}: copy references missing base runtime path ${baseRuntimePath}`
					);
				}
				if (
					...
					throw new Error(
						`malformed runtime delta stream for ${entry.runtimePath} at ${operationOffset}: copy range ${baseOffset}+${copyLength} exceeds base length ${baseEntry.bytes.byteLength}`
					);
				}
				if (copyLength > decodedBytes.byteLength - decodedOffset) {
					throw new Error(
						`malformed runtime delta stream for ${entry.runtimePath} at ${operationOffset}: copy exceeds decodedLength ${entry.decodedLength}`
					);
				}
				decodedBytes.set(
			...
			throw new Error(
				`malformed runtime delta stream for ${entry.runtimePath} at ${operationOffset}: unknown operation ${operation}`
			);
		}
		if (decodedOffset !== entry.decodedLength) {
			throw new Error(
				`malformed runtime delta stream for ${entry.runtimePath}: decoded ${decodedOffset} bytes, expected ${entry.decodedLength}`
			);
		}
		decodedEntries.push({
			runtimePath: entry.runtimePath,
...
export async function loadRuntimePackEntries(
	...
	const indexWeights = new Map(
		[...packs].map(([key, reference]) => [key, Math.max(reference.fileCount, 1)])
	);
	...
	const reportProgressForPack = (currentPack: RuntimeAssetPackReference) => {
		const key = runtimePackKey(baseUrl, currentPack);
		return {
			...(reportIndexProgress
				? {
						index: (loaded: number, total?: number) => {
							const fraction =
								total && total > 0
							...
							indexFractions.set(
								key,
								Math.max(indexFractions.get(key) || 0, fraction)
							);
			...
			...(reportAssetProgress
				? {
						asset: (loaded: number, total?: number) => {
							const fraction =
								total && total > 0
									? Math.min(Math.max(loaded / total, 0), 1)
									: loaded > 0
							...
							assetFractions.set(
								key,
								Math.max(assetFractions.get(key) || 0, fraction)
							);
							let aggregateLoaded = 0;
