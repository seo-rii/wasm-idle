...
class MirroredBitcodeOpenDirectory extends OpenDirectory {
	...
	private readonly sharedWorkspace: SharedWorkspaceStore;
	private readonly sharedPathPrefix: string;

	constructor(
		dir: Directory,
		mirroredBitcodePath: string,
		sharedWorkspace: SharedWorkspaceStore,
	...
	private resolveSharedWorkspacePath(pathStr: string) {
	...
	private installSharedWorkspaceFile(pathStr: string, file: SharedWorkspaceFile) {
	...
	private installSharedParentDirectories(pathStr: string) {
	...
	private installSharedDirectory(pathStr: string) {
	...
	path_lookup(pathStr: string, dirflags: number) {
	...
	path_open(
		...
		const shared = this.sharedWorkspace.open(
			workspacePath,
	...
	path_create_directory(pathStr: string) {
		const workspacePath = this.resolveSharedWorkspacePath(pathStr);
		if (workspacePath === null) {
	...
	path_filestat_get(flags: number, pathStr: string) {
		const mirrored = this.resolveMirroredBitcodeFile(pathStr);
		if (
			mirrored instanceof MirroredBitcodeFile &&
		...
		if (
			localLookup.inode_obj instanceof SharedWorkspaceFile &&
	...
	path_remove_directory(pathStr: string) {
		const workspacePath = this.resolveSharedWorkspacePath(pathStr);
...
class MirroredBitcodeDirectory extends Directory {
	...
	path_open(oflags: number) {
		return {
			ret: wasi.ERRNO_SUCCESS,
			fd_obj: new MirroredBitcodeOpenDirectory(
				this,
				this.mirroredBitcodePath,
				this.sharedWorkspace,
				this.sharedPathPrefix
			)
		};
	}
}
...
export async function buildPreopenedDirectories(
	...
	const tmpDirectory = new MirroredBitcodePreopenDirectory(
		'/tmp',
		tmpRoot,
		'',
		sharedWorkspace,
