...
export interface RuntimeDeltaPackIndexEntry {
	runtimePath: string;
	offset: number;
	length: number;
...
export function parseRuntimePackIndex(value: unknown): RuntimePackIndex {
	...
	const entries = root.entries.map(
		(entry, index): RuntimePackIndexEntry | RuntimeDeltaPackIndexEntry => {
			const object = expectObject(entry, `root.entries[${index}]`);
			const parsedEntry: RuntimePackIndexEntry = {
			...
			return {
				...parsedEntry,
				decodedLength: expectNonNegativeInteger(
					object.decodedLength,
					`root.entries[${index}].decodedLength`
				),
				...(object.baseRuntimePath === undefined
					? {}
					: {
							baseRuntimePath: expectString(
								object.baseRuntimePath,
								`root.entries[${index}].baseRuntimePath`
							)
						})
			};
		}
	...
	if (isDelta) {
		const decodedTotalBytes = expectNonNegativeInteger(
			root.decodedTotalBytes,
			'root.decodedTotalBytes'
		);
		const deltaEntries = entries as RuntimeDeltaPackIndexEntry[];
		let entryDecodedTotalBytes = 0;
		for (const entry of deltaEntries) {
			if (entry.decodedLength > decodedTotalBytes - entryDecodedTotalBytes) {
				throw new Error(
					`invalid root.decodedTotalBytes in wasm-rust runtime pack index: entry lengths exceed ${decodedTotalBytes}`
				);
			}
		...
		if (entryDecodedTotalBytes !== decodedTotalBytes) {
			throw new Error(
				`invalid root.decodedTotalBytes in wasm-rust runtime pack index: expected ${decodedTotalBytes} but entries decode to ${entryDecodedTotalBytes}`
			);
		}
		return {
			format: 'wasm-rust-runtime-delta-pack-index-v1',
	...
	return {
		...
		fileCount,
		totalBytes,
		entries: entries as RuntimePackIndexEntry[]
	};
}
...
async function loadRuntimePackEntriesRecursive(
	runtimeBaseUrl: string | URL,
	pack: RuntimeAssetPackReference,
	fetchImpl: typeof fetch,
	onProgressForPack: (
	...
	if (ancestorIndexUrls.has(indexUrl)) {
		throw new Error(
			`invalid wasm-rust runtime pack ${pack.index}: cyclic delta base reference`
		);
	}
	...
	try {
		...
		const baseEntriesPromise = pack.delta
			? loadRuntimePackEntriesRecursive(
					runtimeBaseUrl,
					pack.delta.base,
					fetchImpl,
					onProgressForPack,
		...
		if (index.format === 'wasm-rust-runtime-pack-index-v1') {
			if (pack.delta) {
				throw new Error(
					`invalid wasm-rust runtime pack ${pack.index}: delta reference requires a delta pack index`
				);
			}
			if (
				pack.decodedTotalBytes !== undefined &&
				pack.decodedTotalBytes !== index.totalBytes
			) {
				throw new Error(
					`invalid wasm-rust runtime pack ${pack.index}: expected ${pack.decodedTotalBytes} decoded bytes but got ${index.totalBytes}`
				);
			}
		...
		if (!pack.delta) {
			throw new Error(
				`invalid wasm-rust runtime pack ${pack.index}: delta pack index requires a delta reference`
			);
		}
		if (pack.delta.format !== 'copy-literal-v1') {
			throw new Error(
				`invalid wasm-rust runtime pack ${pack.index}: unsupported delta format ${String(pack.delta.format)}`
			);
		}
		if (
			pack.decodedTotalBytes !== undefined &&
			...
			throw new Error(
				`invalid wasm-rust runtime pack ${pack.index}: expected ${pack.decodedTotalBytes} decoded bytes but got ${index.decodedTotalBytes}`
			);
		}
		if (packBytes.byteLength !== index.totalBytes) {
			throw new Error(
				`invalid wasm-rust runtime delta pack ${pack.asset}: expected exactly ${index.totalBytes} encoded bytes but got ${packBytes.byteLength}`
			);
		}

		const baseEntryByRuntimePath = new Map(
			(baseEntries || []).map((entry) => [entry.runtimePath, entry] as const)
		);
		const decodedEntries: RuntimePackAssetEntry[] = [];
		...
		for (const entry of index.entries) {
			...
			if (entry.baseRuntimePath && !baseEntry) {
				throw new Error(
					`invalid wasm-rust runtime delta pack ${pack.index}: base runtime path ${entry.baseRuntimePath} for ${entry.runtimePath} was not found`
				);
			}
			...
			try {
				...
				throw new Error(
					`invalid wasm-rust runtime delta pack ${pack.index}: decoded length ${entry.decodedLength} for ${entry.runtimePath} is too large`
				);
			}
			let encodedOffset = 0;
			let decodedOffset = 0;
			while (encodedOffset < encodedBytes.byteLength) {
				...
				if (operation === 0) {
					if (encodedBytes.byteLength - encodedOffset < 4) {
						throw new Error(
							`invalid wasm-rust runtime delta pack ${pack.asset}: truncated literal operation at offset ${operationOffset} for ${entry.runtimePath}`
						);
					}
					...
					if (literalLength > encodedBytes.byteLength - encodedOffset) {
						throw new Error(
							`invalid wasm-rust runtime delta pack ${pack.asset}: literal operation at offset ${operationOffset} exceeds the encoded entry for ${entry.runtimePath}`
						);
					}
					if (literalLength > decodedBytes.byteLength - decodedOffset) {
						throw new Error(
							`invalid wasm-rust runtime delta pack ${pack.asset}: literal operation at offset ${operationOffset} exceeds decoded length ${entry.decodedLength} for ${entry.runtimePath}`
						);
					}
					decodedBytes.set(
						encodedBytes.subarray(encodedOffset, encodedOffset + literalLength),
						decodedOffset
					);
				...
				if (operation === 1) {
					if (encodedBytes.byteLength - encodedOffset < 8) {
						throw new Error(
							`invalid wasm-rust runtime delta pack ${pack.asset}: truncated copy operation at offset ${operationOffset} for ${entry.runtimePath}`
						);
					}
					...
					if (!baseEntry) {
						throw new Error(
							`invalid wasm-rust runtime delta pack ${pack.index}: copy operation for ${entry.runtimePath} requires baseRuntimePath`
						);
					}
					if (
						baseOffset > baseEntry.bytes.byteLength ||
						copyLength > baseEntry.bytes.byteLength - baseOffset
					) {
						throw new Error(
							`invalid wasm-rust runtime delta pack ${pack.asset}: copy range ${baseOffset}+${copyLength} exceeds ${baseEntry.bytes.byteLength} bytes for base runtime path ${entry.baseRuntimePath}`
						);
					}
					if (copyLength > decodedBytes.byteLength - decodedOffset) {
						throw new Error(
							`invalid wasm-rust runtime delta pack ${pack.asset}: copy operation at offset ${operationOffset} exceeds decoded length ${entry.decodedLength} for ${entry.runtimePath}`
						);
					}
					decodedBytes.set(
						baseEntry.bytes.subarray(baseOffset, baseOffset + copyLength),
						decodedOffset
					);
				...
				throw new Error(
					`invalid wasm-rust runtime delta pack ${pack.asset}: unknown operation ${String(operation)} at offset ${operationOffset} for ${entry.runtimePath}`
				);
			}
			if (decodedOffset !== entry.decodedLength) {
				throw new Error(
					`invalid wasm-rust runtime delta pack ${pack.asset}: decoded ${decodedOffset} bytes but expected ${entry.decodedLength} for ${entry.runtimePath}`
				);
			}
		...
		if (decodedTotalBytes !== index.decodedTotalBytes) {
			throw new Error(
				`invalid wasm-rust runtime delta pack ${pack.asset}: decoded ${decodedTotalBytes} bytes but expected ${index.decodedTotalBytes}`
			);
		}
...
export async function loadRuntimePackEntries(
	...
	const packs = new Map<string, RuntimeAssetPackReference>();
	...
	while (pendingPacks.length > 0) {
		...
		if (currentPack.delta) pendingPacks.push(currentPack.delta.base);
	}
	const fractions = new Map([...packs.keys()].map((key) => [key, 0]));
	const weights = new Map(
		[...packs].map(([key, reference]) => [key, Math.max(reference.totalBytes, 1)])
	);
	const aggregateTotal = [...weights.values()].reduce((total, weight) => total + weight, 0);
	const onProgressForPack = (currentPack: RuntimeAssetPackReference) => {
		if (!onProgress) return undefined;
		const key = resolveVersionedAssetUrl(runtimeBaseUrl, currentPack.index).toString();
		return ({ loaded, total }: { loaded: number; total?: number }) => {
			const fraction =
				total && total > 0 ? Math.min(Math.max(loaded / total, 0), 1) : loaded > 0 ? 1 : 0;
