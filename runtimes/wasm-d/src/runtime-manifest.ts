...
export function parseRuntimeManifest(value: unknown): RuntimeManifestV1 {
	...
	if (root.manifestVersion !== 1) throw new Error('invalid root.manifestVersion in wasm-d runtime manifest');
	...
	if (wasmTarget.artifactFormat !== 'wasi-core-wasm') {
		throw new Error('invalid root.targets.wasm32-wasi.artifactFormat in wasm-d runtime manifest');
	}
	const execution = expectObject(wasmTarget.execution, 'root.targets.wasm32-wasi.execution');
	if (execution.kind !== 'wasi-preview1') {
		throw new Error('invalid root.targets.wasm32-wasi.execution.kind in wasm-d runtime manifest');
	}
	return {
		manifestVersion: 1,
		name: 'wasm-d',
