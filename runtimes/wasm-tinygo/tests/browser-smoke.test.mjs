...
test(
	'browser smoke completes TinyGo bootstrap flow through test hooks',
	{ timeout: 600000 },
	async (t) => {
...
`
		...
		try {
			...
			if (
				...
				t.skip(
					`browser smoke skipped: loopback listen is not permitted in this sandbox\n${message}`
				);
				return;
			}
			throw error;
		...
		if (typeof previewPort !== 'number') {
			throw new Error(
				`browser smoke failed to reserve a preview port: ${String(previewPort)}`
			);
		}
		...
		await writeFile(
			path.join(bridgeWorkDir, 'helper', 'helper.go'),
			browserWorkspaceFiles['helper/helper.go']
		);
...
`
		...
		aliasOnlyDriverBridgeManifest.realFrontendAnalysis =
		...
		driftedAnalysisInputBridgeManifest.frontendAnalysisInput.buildContext.target =
			'mismatch-target';
		const driftedFrontendProbeAnalysisInputBridgeManifest = JSON.parse(
			JSON.stringify(driverBridgeManifest)
		);
		...
		driftedDriverBridgeManifest.frontendAnalysis = JSON.parse(
		...
		try {
			...
			if (
				...
				t.skip(
					`browser smoke skipped: preview server is not permitted in this sandbox\n${message}`
				);
				return;
			}
			throw error;
		...
		try {
			...
			if (
				...
				t.skip(
					`browser smoke skipped: Chromium launch is not permitted in this sandbox\n${message}`
				);
				return;
			}
			throw error;
		...
		await page.waitForFunction(
			() =>
				...
				typeof window.__wasmTinygoTestHooks?.readFrontendAnalysisInputManifest ===
					'function' &&
		...
		const upstreamProbe = await page.evaluate(() =>
			window.__wasmTinygoTestHooks.runUpstreamProbe()
		);
		...
		const frontendProbe = await page.evaluate(() =>
			window.__wasmTinygoTestHooks.runUpstreamFrontendProbe()
		);
		assert.equal(frontendProbe.requestedTarget, 'wasip1');
		assert.equal(
			frontendProbe.mainImportPath,
			driverBridgeManifest.entryPackage?.importPath ?? 'command-line-arguments'
		);
		assert.equal(
			frontendProbe.mainPackageName,
			driverBridgeManifest.entryPackage?.name ?? 'main'
		);
		...
		assert.equal(
			frontendProbe.packages.some(
				(pkg) => pkg.importPath === 'example.com/browserprobe/helper'
			),
			true
		);
		assert.equal(
		...
		assert.match(
			activity ?? '',
			/backend source=bridge host artifact target=wasip1 runnable=true path=\/working\/out\.wasm/
		);
		assert.match(
			activity ?? '',
			/bridge host artifact command=.*tinygo.* build -target wasip1 .* -o .*tinygo-bridge-execution\.wasm/
		);
		assert.match(
			activity ?? '',
			/patched upstream TinyGo WASI probe verified target=wasip1 triple=wasm32-unknown-wasi scheduler=asyncify/
		);
		assert.match(
			activity ?? '',
			/patched upstream TinyGo WASI frontend probe matched analysis input packages=[1-9]\d* main=example\.com\/browserprobe/
		);
		assert.match(
			activity ?? '',
			/patched upstream TinyGo WASI frontend probe matched frontend analysis packages=[1-9]\d* main=example\.com\/browserprobe/
		);
		assert.match(
			activity ?? '',
			/patched upstream TinyGo WASI frontend probe matched frontend real adapter packages=[1-9]\d* main=example\.com\/browserprobe/
		);
		assert.match(
			activity ?? '',
		...
		assert.match(
			activity ?? '',
			/frontend final artifact source=bridge host artifact target=wasip1 output=\/working\/out\.wasm/
		);
		assert.match(
			activity ?? '',
		...
		assert.match(
			activity ?? '',
			/execution artifact ready: .*\/(?:main|out)\.wasm \([\d,]+ bytes\)/
		);
		assert.match(
			activity ?? '',
			/execution artifact entrypoint=(?:_(?:start|initialize)|main)/
		);
		assert.match(activity ?? '', /browser-ok/);
		assert.match(activity ?? '', /execution artifact completed exitCode=0/);
		assert.match(
			activity ?? '',
			/driver tinygo-style planner validated target=wasm optimize=-Oz scheduler=asyncify panic=trap/
		);
		assert.match(
			activity ?? '',
			/frontend input bridge verified target=wasm llvm=wasm32-unknown-wasi scheduler=asyncify packages=[1-9]\d*/
		);
		assert.match(
			activity ?? '',
			/frontend analysis input bridge verified target=wasm llvm=wasm32-unknown-wasi scheduler=asyncify packages=[1-9]\d*/
		);
		...
		assert.match(
			activity ?? '',
			/frontend analysis verified target=wasm llvm=wasm32-unknown-wasi groups=6 compileUnits=[1-9]\d* allCompile=[1-9]\d*/
		);
		assert.match(activity ?? '', /frontend build mode=frontend/);
		assert.match(activity ?? '', /frontend build source=real-adapter/);
		assert.match(
			activity ?? '',
			/frontend real adapter verified target=wasm llvm=wasm32-unknown-wasi groups=4 compileUnits=[1-9]\d* allCompile=[1-9]\d*/
		);
		assert.match(
			activity ?? '',
			/frontend real adapter bridge verified target=wasm llvm=wasm32-unknown-wasi groups=4 compileUnits=[1-9]\d* allCompile=[1-9]\d* alias=direct source=canonical/
		);
		assert.match(activity ?? '', /frontend real adapter source=bridge/);
		assert.match(
			activity ?? '',
			/frontend real adapter seam verified target=wasm llvm=wasm32-unknown-wasi groups=4 compileUnits=[1-9]\d* allCompile=[1-9]\d* alias=direct/
		);
		assert.match(activity ?? '', /frontend analysis tinygo frontend prepared analysis handoff/);
		assert.match(
			activity ?? '',
			/frontend bridge verified target=wasm llvm=wasm32-unknown-wasi program=main imports=1 packages=[1-9]\d*/
		);
		assert.match(
			activity ?? '',
			/frontend bridge coverage compileUnits=[1-9]\d* graphPackages=[1-9]\d* coveredPackages=[1-9]\d*\/[1-9]\d* compileUnitFiles=[1-9]\d* coveredFiles=[1-9]\d*\/[1-9]\d* depOnly=[1-9]\d* standard=[1-9]\d* local=2 alias=direct/
		);
		...
		assert.doesNotMatch(
			activity ?? '',
			/frontend real adapter tinygo frontend prepared real adapter handoff/
		);
		...
		assert.doesNotMatch(
			activity ?? '',
			/\$ \/usr\/bin\/wasm-ld .*\/working\/tinygo-work\/.*\.bc .* -o \/working\/out\.wasm/
		);
		assert.doesNotMatch(
			activity ?? '',
			/\$ \/usr\/bin\/clang .*tinygo-bootstrap\.c .* -o .*tinygo-bootstrap\.o/
		);
		assert.doesNotMatch(
			activity ?? '',
		...
		assert.match(
			hookBootPlanError ?? '',
			/wasm-tinygo test hook action already running: booting/
		);
		...
		assert.deepEqual(
			redundantReadyExecuteFrontendAnalysisInputManifest,
		...
		const busyUiPlanUnhandledRejections = await page.evaluate(
			() => window.__codexUnhandledRejections
		);
		...
		const failingUiPlanUnhandledRejections = await page.evaluate(
			() => window.__codexUnhandledRejections
		);
		...
		await page.evaluate(
			(manifest) => window.__wasmTinygoTestHooks.setDriverBridgeManifest(manifest),
			driftedAnalysisInputBridgeManifest
		);
		await page.evaluate(() => window.__wasmTinygoTestHooks.boot());
		await page.evaluate(() => {
			document
		...
		const failingUiExecuteUnhandledRejections = await page.evaluate(
			() => window.__codexUnhandledRejections
		);
		...
		assert.match(
			failingUiExecuteActivity ?? '',
			/build execution failed: frontend analysis input did not match real TinyGo driver bridge/
		);
		...
		await page.waitForFunction(
			() =>
				/verified|failed/.test(
					document.querySelector('[data-phase="verify"] .phase-value')?.textContent ?? ''
				),
			null,
			{ timeout: 120000 }
		);
		...
		assert.deepEqual(
			repeatedExecuteFrontendAnalysisInputManifest,
			expectedFrontendAnalysisInputManifest
		);
		assert.match(repeatedExecutePhases.join('\n'), /build execution\s+[\d,]+ bytes/);
		assert.match(repeatedExecutePhases.join('\n'), /front-end verification\s+verified/);
		assert.equal(
			(repeatedExecuteActivity?.match(/frontend analysis input source=bridge/g) ?? []).length,
			2
		);
		assert.equal(
			(repeatedExecuteActivity?.match(/frontend final artifact compiled module=ok/g) ?? [])
				.length,
			2
		);
		...
		const concurrentExecuteStatuses = await page.evaluate(
			async ({ workspaceFiles, manifest }) => {
				window.__wasmTinygoTestHooks.setBuildRequestOverrides({ scheduler: 'asyncify' });
				window.__wasmTinygoTestHooks.setWorkspaceFiles(workspaceFiles);
		...
		assert.deepEqual(concurrentExecuteStatuses, [
			'fulfilled',
			'wasm-tinygo test hook action already running: executing'
		]);
		...
		await page.evaluate(() => window.__wasmTinygoTestHooks.boot());
		const delayedUiExecuteMutationError = await page.evaluate(
			(workspaceFiles) =>
				new Promise((resolve) => {
					document.querySelector('[data-action="execute"]')?.click();
					setTimeout(() => {
		...
		assert.match(
			delayedUiExecuteMutationError ?? '',
			/wasm-tinygo test hook action already running: executing/
		);
		assert.deepEqual(
			delayedInvalidationFrontendAnalysisInputManifest,
			expectedFrontendAnalysisInputManifest
		);
		...
		assert.deepEqual(
			forcedResetFrontendAnalysisInputManifest,
			expectedFrontendAnalysisInputManifest
		);
		...
		assert.match(
			aliasOnlyActivity ?? '',
			/frontend real adapter bridge verified target=wasm llvm=wasm32-unknown-wasi groups=4 compileUnits=[1-9]\d* allCompile=[1-9]\d* alias=direct source=compat-alias/
		);
		assert.match(
			aliasOnlyActivity ?? '',
			/patched upstream TinyGo WASI frontend probe matched frontend real adapter packages=[1-9]\d* main=example\.com\/browserprobe/
		...
		assert.match(
			driftedActivity ?? '',
			/build execution failed: frontend analysis buildContext did not match real TinyGo analysis adapter/
		);

		await page.route('**/api/tinygo/compile', (route) =>
		...
		assert.match(
			...
			/execution artifact ready: \/working\/out\.wasm \([\d,]+ bytes\)/
		);
		assert.match(
			staticFallbackActivity ?? '',
			/execution artifact entrypoint=(?:_(?:start|initialize)|main)/
		...
		assert.match(
			staticExecutionActivity ?? '',
			/build artifact ready: \/working\/out\.wasm \([\d,]+ bytes\)/
		);
		assert.match(
			staticExecutionActivity ?? '',
			/execution artifact ready: \/working\/out\.wasm \([\d,]+ bytes\)/
		...
		assert.match(
			staticExecutionActivity ?? '',
			/frontend analysis tinygo frontend prepared analysis handoff/
		);
		assert.match(
			staticExecutionActivity ?? '',
			/frontend analysis verified target=wasm llvm=wasm32-unknown-wasi groups=6 compileUnits=[1-9]\d* allCompile=[1-9]\d*/
		...
		const unsupportedStaticLanguagePhases = await page
			.locator('[data-phase]')
			.allTextContents();
		const unsupportedStaticLanguageActivity = await page
		...
		assert.doesNotMatch(
			unsupportedStaticLanguageActivity ?? '',
			/attempting browser-side relink to produce a runnable execution artifact/
		);

		await page.evaluate(() => window.__wasmTinygoTestHooks.reset());
		await page.evaluate(() =>
			window.__wasmTinygoTestHooks.setBuildRequestOverrides({
				scheduler: 'asyncify',
				target: 'bogus-target'
			})
		);
		...
		assert.match(unsupportedTargetPhases.join('\n'), /build driver plan\s+failed/);
		assert.match(
			unsupportedTargetActivity ?? '',
			/build driver failed: unsupported target: "bogus-target"/
		);
