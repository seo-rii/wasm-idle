...
const runFrontendProbe = async ({
	...
	const tinygoRoot = new PreopenDirectory(
		tinygoRootPath,
		buildDirectoryContents(tinygoRootEntries)
	);
...
test('build-tinygo-upstream-frontend-probe parses a real driver bridge package graph with shipped TinyGo source snapshot', async (t) => {
	...
	const bridgeWorkDir = await mkdtemp(
		path.join(tmpdir(), 'wasm-tinygo-upstream-frontend-bridge-')
	);
	t.after(async () => {
...
`
	...
	await writeFile(
		...
		workspaceFiles['helper/helper.go']
	);
	...
	const packageList = (browserManifest.packageGraph ?? []).map((packageInfo) => {
		...
		const isWorkspacePackage =
			packageDir === '/workspace' || packageDir.startsWith('/workspace/');
		return {
			Dir: packageDir,
	...
	for (const packageInfo of browserManifest.packageGraph ?? []) {
		...
		for (const goFile of packageInfo.goFiles ?? []) {
			...
			assetReads.push(
				readFile(path.join(result.gorootPath, relativeAssetPath), 'utf8').then(
					(contents) => [relativeAssetPath, contents]
				)
			);
		}
	}
	...
	assert.equal(payload.requestedTarget, 'wasip1');
	assert.equal(
		payload.mainImportPath,
		browserManifest.entryPackage?.importPath ?? 'command-line-arguments'
	);
