...
test('fetch-emception-worker reuses an existing worker file when download fails', async (t) => {
	...
	const child = spawn(process.execPath, [scriptPath], {
		...
		env: {
			...process.env,
			WASM_TINYGO_EMCEPTION_WORKER_URL:
				'http://127.0.0.1:9/emception.worker.bundle.worker.js',
...
test('fetch-emception-worker downloads the worker and referenced runtime assets into the local vendor directory', async (t) => {
	...
	assert.deepEqual(
		await readFile(path.join(path.dirname(outputPath), 'tool.wasm')),
		assetBodies['tool.wasm']
	);
	assert.equal(
		await readFile(path.join(path.dirname(outputPath), 'libsupport.a'), 'utf8'),
		'support archive\n'
	);
	assert.equal(
		await readFile(path.join(path.dirname(outputPath), 'cache-package.brotli'), 'utf8'),
		'brotli payload\n'
	);
});
