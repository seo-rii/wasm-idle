...
import {
	...
	toolchainIsReady
} from '../scripts/tinygo-toolchain-paths.mjs';
...
const ensureTinyGoToolchainReady = async () => {
	...
	tinyGoToolchainReadyPromise = (async () => {
		...
		const fetchScriptPath = new URL('../scripts/fetch-tinygo-toolchain.mjs', import.meta.url)
			.pathname;
...
`
	...
	const probe = spawn(
		process.execPath,
		[new URL('../scripts/probe-tinygo-host.mjs', import.meta.url).pathname],
		{
			cwd,
			env: {
				...process.env,
...
test('real TinyGo driver bridge records synthetic frontend handoff facts for the same entry package', async (t) => {
	...
	const bridge = spawn(
		process.execPath,
		[new URL('../scripts/probe-tinygo-driver-bridge.mjs', import.meta.url).pathname],
		{
			cwd,
			env: {
				...process.env,
	...
	assert.equal(
		bridgeManifest.frontendHandoff.programImportPath,
		'example.com/wasm-tinygo/hostprobe'
	);
	assert.equal(bridgeManifest.frontendHandoff.programImportAlias, 'direct');
	assert.equal(
		bridgeManifest.frontendHandoff.bridgeEntryImportPath,
		'example.com/wasm-tinygo/hostprobe'
	);
	assert.deepEqual(bridgeManifest.frontendHandoff.bridgeEntryGoFiles, [
	...
	assert.equal(
		bridgeManifest.frontendRealAdapter.compileUnitManifestPath,
	...
	assert.equal(
		bridgeManifest.frontendRealAdapter.buildContext.scheduler,
		bridgeManifest.scheduler
	);
	assert.equal(
		bridgeManifest.frontendRealAdapter.packageGraph.length,
		bridgeManifest.frontendRealAdapter.compileUnits.length
	);
	...
	assert.equal(
		bridgeManifest.packageGraph.length,
		bridgeManifest.frontendHandoff.graphPackageCount
	);
});
...
`
	...
	await writeFile(
		path.join(workspaceDir, 'helper', 'helper.go'),
		`package helper
...
`
	...
	assert.deepEqual(bridgeManifest.frontendHandoff.programFiles, [
		path.join(workspaceDir, 'main.go')
	]);
	...
	assert.deepEqual(bridgeManifest.frontendHandoff.bridgeEntryImports, [
		'example.com/bridgeprobe/helper'
	]);
	assert.equal(bridgeManifest.frontendAnalysis, undefined);
	...
	assert.equal(
		bridgeManifest.frontendRealAdapter.compileUnits.length,
		bridgeManifest.frontendHandoff.compileUnitCount
	);
	assert.equal(
		bridgeManifest.frontendRealAdapter.allCompileFiles.length,
		bridgeManifest.frontendHandoff.compileUnitFileCount
	);
	assert.deepEqual(bridgeManifest.realFrontendAnalysis, bridgeManifest.frontendRealAdapter);
	...
	const normalizedRealAnalysisPaths = [
		...
		...(normalizedBridgeManifest.frontendRealAdapter?.compileUnits ?? []).flatMap(
			(compileUnit) => [compileUnit.packageDir ?? '', ...(compileUnit.files ?? [])]
		)
	];
	assert.equal(
		normalizedRealAnalysisPaths.every(
			(filePath) =>
				filePath.startsWith('/workspace') || filePath.startsWith('/working/.tinygo-root')
		),
		true
	);
});
