...
import {
	compileTinyGoHostSource,
	compileTinyGoHostWorkspace
} from '../scripts/tinygo-host-compiler.mjs';
...
`
	...
	assert.match(
		invocations[2],
		/build -target wasip1 -opt z -scheduler tasks -panic trap -o .*main\.wasm .*main\.go$/
	);
	...
	assert.deepEqual(result.targetInfo.buildTags, [
		'tinygo.wasm',
		'tinygo',
		'purego',
		'gc.precise',
...
`
	...
	assert.match(
		invocations[2],
		/build -target wasip1 -opt z -scheduler asyncify -panic trap -o .*main\.wasm .*main\.go$/
	);
	assert.equal(invocations[3], fakeTinyGoRoot);
	assert.equal(invocations[4], path.join(tempDir, 'workspace'));
	assert.equal(invocations[5], 'info -target wasip1 -scheduler asyncify');
	assert.equal(
		await readFile(path.join(tempDir, 'workspace', 'go.mod'), 'utf8'),
		'module example.com/browserprobe\n\ngo 1.22\n'
	);
	assert.equal(
		await readFile(path.join(tempDir, 'workspace', 'helper', 'helper.go'), 'utf8'),
		'package helper\n\nfunc Answer() int { return 42 }\n'
	);
...
`
	...
	const [tinygoRoot, invocation] = (await readFile(fakeTinyGoLogPath, 'utf8'))
		.trimEnd()
		.split('\n');
	...
	assert.match(
		invocation,
		new RegExp(
			`build -target wasip1 -o ${outputPath.replaceAll(/[.*+?^${}()|[\]\\]/g, '\\$&')}`
		)
	);
...
`
	...
	assert.match(
		invocations[2],
		new RegExp(
			`build -target wasip1 -opt z -scheduler tasks -panic trap -o ${outputPath.replaceAll(/[.*+?^${}()|[\]\\]/g, '\\$&')} ${entryPath.replaceAll(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`
		)
	);
	assert.equal(invocations[3], fakeTinyGoRoot);
	...
	const manifest = JSON.parse(
		await readFile(path.join(workspaceDir, 'tinygo-host-probe.json'), 'utf8')
	);
	assert.equal(manifest.targetInfo.scheduler, 'tasks');
	assert.deepEqual(manifest.targetInfo.buildTags, [
		'tinygo.wasm',
		'tinygo',
		'purego',
		'gc.precise',
