...
const runProbe = async ({ mode, rootPath, entries, resultFileName }) => {
	...
	const wasi = new WASI(
		['tinygo-driver'],
		[`WASM_TINYGO_MODE=${mode}`],
		[new OpenFile(new File([])), stdout, stderr, rootDirectory]
	);
	...
	const compileRequest = compileRequestNode
		? JSON.parse(textDecoder.decode(compileRequestNode.data))
		: null;
	return { exitCode, result, compileRequest, logs, rootFiles };
};

const runDriver = async ({ source, files, request }) =>
	runProbe({
		mode: 'driver',
		rootPath: '/workspace',
		entries: {
...
const runFrontend = async ({ input, analysisResult, adapterResult, files }) =>
	runProbe({
		mode: 'frontend',
		rootPath: '/working',
		entries: {
...
const runFrontendAnalysis = async ({ input, files }) =>
	runProbe({
...
const runFrontendFromAnalysis = async ({ analysisResult, files }) =>
	runProbe({
...
const runFrontendRealAdapter = async ({ input, files }) =>
	runProbe({
...
const runFrontendRealAdapterFromAnalysis = async ({ analysisResult, files }) =>
	runProbe({
...
const runFrontendFromRealAdapter = async ({ analysisResult, adapterResult, files }) =>
	runProbe({
...
const runBackend = async ({ input, files }) =>
	runProbe({
		mode: 'backend',
		rootPath: '/working',
		entries: {
...
test('wasi driver writes tinygo metadata for valid source', async () => {
	...
	const manifestFile = execution.result.files.find(
		(file) => file.path === '/working/tinygo-bootstrap.json'
	);
	const frontendInputFile = execution.result.files.find(
	...
	const frontendAnalysisExecution = await runFrontendAnalysis({
		input: frontendInput,
		files: {}
	});
	assert.equal(frontendAnalysisExecution.exitCode, 0);
	assert.equal(frontendAnalysisExecution.result.ok, true);
	const frontendExecution = await runFrontend({
		analysisResult: frontendAnalysisExecution.result,
		files: {}
	});
	assert.equal(frontendExecution.exitCode, 0);
	assert.equal(frontendExecution.result.ok, true);
	const compileUnitManifest = JSON.parse(
		frontendExecution.result.generatedFiles.find(
...
test('wasi driver honors go:build constraints when loading package files', async () => {
	const execution = await runDriver({
		files: {
			...
			'browser.go':
				'//go:build tinygo.wasm && scheduler.tasks\n\npackage main\n\nfunc browserOnly() {}\n',
...
test('wasi driver tracks local replace module imports', async () => {
	...
	assert.equal(execution.exitCode, 0);
	assert.equal(execution.result.ok, true);
	assert.deepEqual(execution.result.metadata.replacedModuleImports, ['example.com/lib/pkg']);
	assert.deepEqual(execution.result.metadata.importedPackageFiles, [
...
test('wasi driver tracks workspace module imports from go.work', async () => {
	...
	assert.equal(execution.exitCode, 0);
	assert.equal(execution.result.ok, true);
	assert.deepEqual(execution.result.metadata.workspaceModuleImports, ['example.com/lib/pkg']);
	assert.deepEqual(execution.result.metadata.importedPackageFiles, [
...
test('wasi driver tracks workspace module replace imports', async () => {
	...
	assert.equal(execution.result.ok, true);
	assert.deepEqual(execution.result.metadata.workspaceModuleImports, ['example.com/lib/pkg']);
	assert.deepEqual(execution.result.metadata.replacedModuleImports, ['example.com/dep/value']);
	assert.deepEqual(execution.result.metadata.importedPackageFiles, [
		'/workspace/libs/lib/pkg/pkg.go',
...
test('wasi driver writes diagnostics for non-local go.work replace directives', async () => {
	...
	assert.equal(execution.result.ok, false);
	assert.match(
		execution.result.diagnostics[0],
		/non-local replace directive is not supported yet/
	);
});
...
test('wasi driver writes diagnostics when a local package is excluded by target constraints', async () => {
	...
	assert.match(
		execution.result.diagnostics[0],
		/no files matching current target\/build constraints/
	);
});
...
test('wasi frontend emits synthetic artifacts from frontend-analysis handoff', async () => {
	...
	assert.equal(
		execution.result.generatedFiles[0].contents.includes('/working/tinygo-bootstrap.json'),
		false
	);
	assert.equal(
		execution.result.generatedFiles[0].contents.includes('/working/tinygo-frontend-input.json'),
		false
	);
	assert.equal(
		execution.result.generatedFiles[1].contents.includes('/working/tinygo-bootstrap.json'),
		false
	);
	assert.equal(
		execution.result.generatedFiles[1].contents.includes('/working/tinygo-frontend-input.json'),
		false
	);
	...
	assert.match(
		execution.result.diagnostics[0],
		/frontend prepared bootstrap compile request for wasm/
	);
...
test('wasi frontend prefers frontend-real-adapter handoff when present', async () => {
	...
	assert.equal(execution.exitCode, 0);
	assert.equal(execution.result.ok, true);
	const compileUnitFile = execution.result.generatedFiles.find(
		(file) => file.path === '/working/tinygo-compile-unit.json'
	);
...
test('wasi frontend-analysis writes normalized handoff analysis', async () => {
	...
	assert.equal(
		execution.result.analysis.compileUnitManifestPath,
...
test('wasi frontend-analysis-build consumes normalized analysis handoff', async () => {
	...
	const directExecution = await runFrontend({
		analysisResult: analysisExecution.result,
		files: {}
	});
	...
	const directAdapterResult = JSON.parse(
		directExecution.rootFiles['tinygo-frontend-real-adapter.json']
	);
...
test('wasi frontend-real-adapter-analysis consumes normalized analysis handoff', async () => {
	...
	assert.equal(
		execution.result.adapter.compileUnitManifestPath,
		'/working/tinygo-compile-unit.json'
	);
...
test('wasi backend consumes backend input', async () => {
	...
	assert.deepEqual(loweredCommandBatchManifest.compileCommands, [
		...
		},
		{
			argv: [
				...
				'-msign-ext',
				'-c',
				'/working/tinygo-lowered/stdlib-000.c',
				'-o',
	...
	assert.deepEqual(commandArtifactManifest, {
		artifactOutputPath: '/working/out.wasm',
		artifactKind: 'probe',
		bitcodeFiles: ['/working/tinygo-work/program-000.bc', '/working/tinygo-work/stdlib-000.bc'],
		entrypoint: null,
		reason: 'missing-wasi-entrypoint',
		runnable: false
	});
