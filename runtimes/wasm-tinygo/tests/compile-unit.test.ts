...
test('verifyCompileUnitManifestAgainstCompileRequest preserves explicit execution ldflags while toolPlan appends probe-only flags', () => {
	...
	assert.deepEqual(verification.toolchain.ldflags, ['--stack-first', '--import-memory']);
...
test('verifyTinyGoHostProbeManifestAgainstDriverMetadata accepts matching real TinyGo target facts', () => {
	...
	assert.deepEqual(verification.driverBuildTags, [
		'gc.precise',
		'scheduler.tasks',
		'serial.none',
		'tinygo.unicore',
	...
	assert.deepEqual(verification.hostBuildTags, [
		'gc.precise',
		'purego',
		'scheduler.tasks',
		'serial.none',
	...
	const verification = verifyFrontendAnalysisAgainstDriverBridgeManifest(
		{
			...
			compileGroups: [
				...
				{
					name: 'all-compile',
					files: ['/workspace/main.go', '/working/.tinygo-root/src/fmt/print.go']
				}
			],
			compileUnits: [
				{
			...
			verifyFrontendAnalysisAgainstRealDriverBridgeManifest(
				{
					entryFile: '/workspace/main.go',
					compileUnitManifestPath: '/working/tinygo-compile-unit.json',
					allCompileFiles: ['/workspace/helper/helper.go', '/workspace/main.go'],
					compileGroups: [
						{
							name: 'program',
							files: ['/workspace/main.go']
						...
						{
							name: 'all-compile',
							files: ['/workspace/helper/helper.go', '/workspace/main.go']
						}
					],
					compileUnits: [
						{
				...
				},
				{
					frontendRealAdapter: {
						entryFile: '/workspace/main.go',
						compileUnitManifestPath: '/working/tinygo-compile-unit.json',
						allCompileFiles: ['/workspace/main.go', '/workspace/other/helper.go'],
						compileGroups: [
							{
								name: 'program',
								files: ['/workspace/main.go']
							...
							{
								name: 'all-compile',
								files: ['/workspace/main.go', '/workspace/other/helper.go']
							}
						],
						compileUnits: [
							{
			...
			verifyCompileUnitManifestAgainstDriverBridgeManifest(
				{
					...
					sourceSelection: {
						program: ['/workspace/main.go'],
						allCompile: ['/workspace/helper/helper.go', '/workspace/main.go']
					},
					compileUnits: [
						{
							kind: 'program',
...
test('normalizeTinyGoDriverBridgeManifestForBrowser promotes alias-only realFrontendAnalysis input into frontendRealAdapter', () => {
	...
	assert.equal(
		verification.frontendRealAdapter?.compileUnitManifestPath,
		'/working/tinygo-compile-unit.json'
	);
});
...
test('verifyWorkItemsManifestAgainstLoweringManifest accepts a normalized work-item graph', () => {
	...
	assert.equal(
		verification.workItems[0]?.bitcodeOutputPath,
					...
					compileCommands: [
						{
							argv: [
								...
								'-emit-llvm',
								'-c',
								'/working/tinygo-lowered/program-001.c',
								'-o',
...
test('verifyBackendResultManifestAgainstBackendInputAndLoweredBitcodeManifest accepts a normalized backend result', () => {
	...
	assert.equal(
		verification.loweredArtifact.objectFiles[0],
		'/working/tinygo-lowered/program-000.o'
	);
});
	...
	assert.throws(
		() =>
			verifyLoweredBitcodeManifestAgainstLoweringPlanAndLoweredSourcesManifest(
				...
				{
					bitcodeFiles: ['/working/tinygo-work/program-001.bc']
				}
			),
					...
					compileCommands: [
						{
							argv: [
								...
								'/working/tinygo-lowered/program-000.c',
								'-o',
								'/working/tinygo-lowered/program-001.o'
							],
							cwd: '/working'
						}
					],
