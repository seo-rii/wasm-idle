...
test('fetch-tinygo-toolchain reports a clear error when the archive extractor is unavailable', async (t) => {
	const tempDir = await mkdtemp(
		path.join(tmpdir(), 'wasm-tinygo-fetch-toolchain-missing-extractor-')
	);
	t.after(async () => {
		await rm(tempDir, { recursive: true, force: true });
	});
...
`
	...
	assert.equal(
		manifest.rootPath,
		path.join(cacheDir, 'extract', 'usr', 'local', 'lib', 'tinygo')
	);
	const [loggedArchivePath, extractPath] = (await readFile(dpkgLogPath, 'utf8'))
		.trimEnd()
		.split('\n');
...
test('resolveTinyGoToolchainPaths chooses archive layouts per platform', () => {
	...
	assert.equal(
		windowsPaths.binPath,
		path.join(cacheDir, 'extract', 'tinygo', 'bin', 'tinygo.exe')
	);
