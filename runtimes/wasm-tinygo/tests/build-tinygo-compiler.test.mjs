...
`
	...
	const patchedBuilderSource = await readFile(
		path.join(sourceRoot, 'builder', 'build.go'),
		'utf8'
	);
	const patchedBuilderCommandsSource = await readFile(
		path.join(sourceRoot, 'builder', 'commands.go'),
		'utf8'
	);
	const patchedBuilderCCSource = await readFile(
		path.join(sourceRoot, 'builder', 'cc.go'),
		'utf8'
	);
	const patchedGoenvSource = await readFile(path.join(sourceRoot, 'goenv', 'goenv.go'), 'utf8');
	const patchedLoaderSource = await readFile(
		path.join(sourceRoot, 'loader', 'loader.go'),
		'utf8'
	);
	const builderLLVMDefaultSource = await readFile(
		path.join(sourceRoot, 'builder', 'llvm_version_default.go'),
		'utf8'
	);
	const builderLLVMWasip1Source = await readFile(
		path.join(sourceRoot, 'builder', 'llvm_version_wasip1.go'),
		'utf8'
	);
	const goenvLLVMDefaultSource = await readFile(
		path.join(sourceRoot, 'goenv', 'llvm_version_default.go'),
		'utf8'
	);
	const goenvLLVMWasip1Source = await readFile(
		path.join(sourceRoot, 'goenv', 'llvm_version_wasip1.go'),
		'utf8'
	);
	const flockStubSource = await readFile(
		path.join(sourceRoot, 'wasmbridge', 'flock', 'flock.go'),
		'utf8'
	);
	const cgoStubSource = await readFile(
		path.join(sourceRoot, 'wasmbridge', 'cgo', 'cgo.go'),
		'utf8'
	);
	...
	assert.match(
		cgoStubSource,
		/func Process\(files \[\]\*ast\.File, dir, importPath string, fset \*token\.FileSet/
	);
...
`
	...
	await writeFile(
		path.join(staleBridgeDir, 'backend.go'),
	...
	const localBackendSource = await readFile(
		new URL('../internal/tinygobackend/backend.go', import.meta.url),
		'utf8'
	);
...
`
	...
	assert.doesNotMatch(
		manifest.patchedEntryFailureReason ?? '',
