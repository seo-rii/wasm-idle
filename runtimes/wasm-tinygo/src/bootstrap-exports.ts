...
const deriveEmbeddedManifestSelection = (manifestSource: string) => {
	...
	const runtimeSupportFilesFromManifest = Array.isArray(manifest.sourceSelection?.runtimeSupport)
		...
		: null;
	const materializedFiles = Array.isArray(manifest.materializedFiles)
		? manifest.materializedFiles
		: null;
...
export const readTinyGoBootstrapManifest = (
...
export const verifyTinyGoBootstrapArtifactExpectation = (
	...
	try {
		const expectedManifest = JSON.parse(expectedEmbeddedManifestSource) as Record<
			string,
			unknown
		>;
		for (const field of [
	...
	return {
		...
		reason:
			embeddedManifestSource === expectedEmbeddedManifestSource
				? ''
				: 'bootstrap embedded manifest mismatch'
	};
};
