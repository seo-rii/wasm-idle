...
export const normalizeTinyGoDriverBridgeManifestForBrowser = (
	...
	if (normalizedFrontendAnalysisInput) {
		...
		if (
			inputEntryFile !== '' &&
			hostWorkspaceRoot !== '' &&
			(inputEntryFile === hostWorkspaceRoot ||
				inputEntryFile.startsWith(`${hostWorkspaceRoot}/`))
		) {
					...
					? {
							targetAssets: inputSourceSelection.targetAssets.map((filePath) => {
								if (
									filePath !== '' &&
									hostWorkspaceRoot !== '' &&
									(filePath === hostWorkspaceRoot ||
										filePath.startsWith(`${hostWorkspaceRoot}/`))
								) {
								...
								if (
									filePath !== '' &&
									hostTinyGoRoot !== '' &&
									(filePath === hostTinyGoRoot ||
										filePath.startsWith(`${hostTinyGoRoot}/`))
								) {
		...
		const packageGraph = (normalizedFrontendAnalysisInput.packageGraph ?? []).map(
			(packageInfo) => {
				let packageDir = packageInfo.dir ?? '';
				if (
					packageDir !== '' &&
					hostWorkspaceRoot !== '' &&
					(packageDir === hostWorkspaceRoot ||
						packageDir.startsWith(`${hostWorkspaceRoot}/`))
				) {
					packageDir = `${browserWorkspaceRoot}${packageDir.slice(hostWorkspaceRoot.length)}`;
				} else if (
					packageDir !== '' &&
		...
		normalizedFrontendAnalysisInput = {
			...
			...(inputSourceSelection !== undefined
				? { sourceSelection: inputSourceSelection }
				: {}),
	...
	if (normalizedFrontendAnalysis) {
		...
		if (
			analysisEntryFile !== '' &&
			hostWorkspaceRoot !== '' &&
			(analysisEntryFile === hostWorkspaceRoot ||
				analysisEntryFile.startsWith(`${hostWorkspaceRoot}/`))
		) {
		...
		if (
			...
			hostTinyGoRoot !== '' &&
			(compileUnitManifestPath === hostTinyGoRoot ||
				compileUnitManifestPath.startsWith(`${hostTinyGoRoot}/`))
		) {
		...
		const allCompileFiles = (normalizedFrontendAnalysis.allCompileFiles ?? []).map(
			(filePath) => {
				if (
					filePath !== '' &&
					hostWorkspaceRoot !== '' &&
		...
		const compileGroups = (normalizedFrontendAnalysis.compileGroups ?? []).map(
			(compileGroup) => ({
				...compileGroup,
		...
		if (analysisToolchain) {
			...
			analysisToolchain = {
				...analysisToolchain,
				...(analysisToolchain.artifactOutputPath !== undefined
					? { artifactOutputPath }
					: {}),
				...(analysisToolchain.objectOutputPath !== undefined ? { objectOutputPath } : {}),
				...(analysisToolchain.translationUnitPath !== undefined
					? { translationUnitPath }
					: {})
			};
		}
	...
	let normalizedFrontendRealAdapter =
		manifest.frontendRealAdapter ?? manifest.realFrontendAnalysis;
	if (normalizedFrontendRealAdapter) {
		...
		const allCompileFiles = (normalizedFrontendRealAdapter.allCompileFiles ?? []).map(
			(filePath) => {
				if (
					filePath !== '' &&
					hostWorkspaceRoot !== '' &&
		...
		const compileGroups = (normalizedFrontendRealAdapter.compileGroups ?? []).map(
			(compileGroup) => ({
				...compileGroup,
				files: (compileGroup.files ?? []).map((filePath) => {
					if (
		...
		const compileUnits = (normalizedFrontendRealAdapter.compileUnits ?? []).map(
			(compileUnit) => {
				let packageDir = compileUnit.packageDir ?? '';
				if (
					packageDir !== '' &&
		...
		const packageGraph = (normalizedFrontendRealAdapter.packageGraph ?? []).map(
			(packageInfo) => {
				let packageDir = packageInfo.dir ?? '';
				if (
					packageDir !== '' &&
		...
		if (realAdapterToolchain) {
			...
			if (
				realAdapterToolchain.artifactOutputPath !== undefined &&
				artifactOutputPath !== ''
			) {
				const artifactOutputPathSlashIndex = artifactOutputPath.lastIndexOf('/');
				artifactOutputPath = `/working/${artifactOutputPathSlashIndex >= 0 ? artifactOutputPath.slice(artifactOutputPathSlashIndex + 1) : artifactOutputPath}`;
			...
			if (
				realAdapterToolchain.translationUnitPath !== undefined &&
				translationUnitPath !== ''
			) {
				const translationUnitPathSlashIndex = translationUnitPath.lastIndexOf('/');
				translationUnitPath = `/working/${translationUnitPathSlashIndex >= 0 ? translationUnitPath.slice(translationUnitPathSlashIndex + 1) : translationUnitPath}`;
			...
			realAdapterToolchain = {
				...
				...(realAdapterToolchain.artifactOutputPath !== undefined
					? { artifactOutputPath }
					: {}),
				...(realAdapterToolchain.objectOutputPath !== undefined
					? { objectOutputPath }
					: {}),
				...(realAdapterToolchain.translationUnitPath !== undefined
					? { translationUnitPath }
	...
	if (manifest.frontendRealAdapter && manifest.realFrontendAnalysis) {
		...
		if (
			JSON.stringify(normalizedFrontendRealAdapter ?? null) !==
			JSON.stringify(normalizedLegacyFrontendRealAnalysis ?? null)
		) {
...
const verifyUpstreamFrontendProbeAgainstPackageGraph = (
	...
	const graphPackages = packageGraph.filter(
		(packageInfo) => (packageInfo.importPath ?? '') !== ''
	);
	...
	for (const packageInfo of graphPackages) {
		...
		const expectedFileCount =
			importPath === 'unsafe' ? 0 : (packageInfo.files?.goFiles ?? []).length;
...
export const verifyUpstreamFrontendProbeAgainstFrontendAnalysisInputManifest = (
	...
	const programPackages = packageGraph.filter(
...
const defaultTargetProfiles: Record<
	string,
	{ llvmTarget: string; linker: string; cflags: string[]; ldflags: string[] }
> = {
	wasm: {
...
const normalizePackageFactsByKind = (kind: string, depOnly?: boolean, standard?: boolean) => {
...
const isTinyGoRootStdlibPackage = (kind: string, packageDir: string, standard: boolean) =>
...
export const buildToolPlanFromCompileUnitManifest = (
	...
	const compileCwd =
		translationUnitPathIndex > 0
			? translationUnitPath.slice(0, translationUnitPathIndex)
			: '/working';
	const linkCwd =
		objectOutputPathIndex > 0 ? objectOutputPath.slice(0, objectOutputPathIndex) : compileCwd;
	const translationUnitName =
		translationUnitPathIndex >= 0
			? translationUnitPath.slice(translationUnitPathIndex + 1)
			: translationUnitPath;
	const objectOutputName =
		objectOutputPathIndex >= 0
			? objectOutputPath.slice(objectOutputPathIndex + 1)
			: objectOutputPath;
...
export const verifyTinyGoHostProbeManifestAgainstDriverMetadata = (
	...
	if (
		driverMetadata.output !== undefined &&
		(manifest.artifact?.path ?? '') !== driverMetadata.output
	) {
	...
	const commandOptimize =
		optimizeFlagIndex < 0 || optimizeFlagIndex + 1 >= commandArgs.length
			? undefined
			: commandArgs[optimizeFlagIndex + 1];
	if (
		commandOptimize !== undefined &&
		optimizeValue !== '' &&
		commandOptimize !== optimizeValue
	) {
		throw new Error('real TinyGo host probe command did not match driver metadata');
	}
	...
	const commandPanicStrategy =
		panicFlagIndex < 0 || panicFlagIndex + 1 >= commandArgs.length
			? undefined
			: commandArgs[panicFlagIndex + 1];
	if (
	...
	if (
		...
		commandArgs[outputFlagIndex + 1] !==
			(driverMetadata.output ?? manifest.artifact?.path ?? '') ||
		commandArgs[commandArgs.length - 1] !==
			(driverMetadata.entry ?? command[command.length - 1] ?? '')
	) {
		throw new Error('real TinyGo host probe command did not match driver metadata');
	}

	if (
		(driverMetadata.llvmTarget !== undefined &&
			targetInfo.llvmTriple !== driverMetadata.llvmTarget) ||
		...
		(driverMetadata.scheduler !== undefined &&
			targetInfo.scheduler !== driverMetadata.scheduler)
	) {
...
export const verifyFrontendInputManifestAgainstDriverBridgeManifest = (
	...
	if (
		(bridgeManifest.goarch ?? '') !== '' &&
		(buildContext.goarch ?? '') !== bridgeManifest.goarch
	) {
		throw new Error('frontend input build context did not match real TinyGo driver bridge');
	}
	...
	if (driverBuildTags.length !== 0) {
		...
		for (const [index, buildTag] of buildTags.entries()) {
			if (driverBuildTags[index] !== buildTag) {
				throw new Error(
					'frontend input build context did not match real TinyGo driver bridge'
				);
			}
		}
	} else {
...
export const verifyFrontendAnalysisInputManifestAgainstDriverBridgeManifest = (
	...
	if ((bridgeAnalysisInput.compileUnits ?? []).length !== 0) {
		throw new Error(
			'real TinyGo driver bridge frontend analysis input compileUnits are not supported'
		);
	}
	...
	const synthesizeCompileUnits = (inputManifest: TinyGoFrontendInputManifest) =>
		(inputManifest.packageGraph ?? []).map((packageInfo) => {
			...
			return {
				...
				packageDir,
				files: goFiles.map((goFile) =>
					packageDir === '' ? goFile : `${packageDir.replace(/\/$/, '')}/${goFile}`
				),
	...
	try {
		verification = verifyFrontendInputManifestAgainstDriverBridgeManifest(
			...
			},
			{
				...bridgeManifest,
				driverBuildTags:
					bridgeAnalysisInput.buildContext?.buildTags ??
					bridgeAnalysisInput.buildTags ??
					bridgeManifest.driverBuildTags,
				...
				target:
					bridgeAnalysisInput.buildContext?.target ??
					bridgeAnalysisInput.toolchain?.target ??
					bridgeManifest.target,
				llvmTriple:
	...
	if (
		...
		JSON.stringify(manifestToolchain.cflags ?? []) !==
			JSON.stringify(bridgeToolchain.cflags ?? []) ||
		JSON.stringify(manifestToolchain.ldflags ?? []) !==
			JSON.stringify(bridgeToolchain.ldflags ?? []) ||
		(manifestToolchain.translationUnitPath ?? '') !==
			(bridgeToolchain.translationUnitPath ?? '') ||
...
export const verifyFrontendAnalysisAgainstDriverBridgeManifest = (
	...
	const bridgeLLVMTarget =
		bridgeAnalysis.toolchain?.llvmTarget ?? bridgeManifest.llvmTriple ?? '';
	if (
		bridgeLLVMTarget === '' ||
		manifestLLVMTarget === '' ||
		manifestLLVMTarget !== bridgeLLVMTarget
	) {
	...
	const bridgeBuildContextBuildTags = [
		...(bridgeBuildContext.buildTags ?? bridgeManifest.driverBuildTags ?? [])
	].sort();
	if (
		(buildContext.target ?? '') !== '' ||
		(buildContext.llvmTarget ?? '') !== '' ||
		...
		if (
			(buildContext.target ?? '') !==
				(bridgeBuildContext.target ?? bridgeManifest.target ?? '') ||
			(buildContext.llvmTarget ?? '') !==
				(bridgeBuildContext.llvmTarget ?? bridgeManifest.llvmTriple ?? '') ||
			(buildContext.goos ?? '') !== (bridgeBuildContext.goos ?? bridgeManifest.goos ?? '') ||
			(buildContext.goarch ?? '') !==
				(bridgeBuildContext.goarch ?? bridgeManifest.goarch ?? '') ||
			(buildContext.gc ?? '') !== (bridgeBuildContext.gc ?? bridgeManifest.gc ?? '') ||
			(buildContext.scheduler ?? '') !==
				(bridgeBuildContext.scheduler ?? bridgeManifest.scheduler ?? '')
		) {
			throw new Error(
		...
		if (
			(bridgeBuildContext.modulePath ?? '') !== '' &&
			(buildContext.modulePath ?? '') !== (bridgeBuildContext.modulePath ?? '')
		) {
			throw new Error(
				'frontend analysis buildContext did not match real TinyGo driver bridge'
			);
	...
	if (
		(manifest.compileUnitManifestPath ?? '') !== (bridgeAnalysis.compileUnitManifestPath ?? '')
	) {
		throw new Error(
	...
	if (allCompileFiles.length !== bridgeAllCompileFiles.length) {
		throw new Error(
			'frontend analysis allCompileFiles did not match real TinyGo driver bridge'
		);
	}
	...
	for (const [index, compileGroup] of compileGroups.entries()) {
		...
		if (!bridgeCompileGroup || (compileGroup.name ?? '') !== (bridgeCompileGroup.name ?? '')) {
			throw new Error(
				'frontend analysis compileGroups did not match real TinyGo driver bridge'
			);
		}
	...
	for (const compileUnit of compileUnits) {
		...
		if (!bridgeCompileUnit && (compileUnit.kind ?? '') === 'program') {
			if (compileUnitImportPath === 'command-line-arguments') {
				...
				bridgeCompileUnit =
					bridgeCompileUnitImportPath === ''
						? undefined
						: bridgeCompileUnitsByImportPath.get(bridgeCompileUnitImportPath);
		...
		if (!bridgeCompileUnit) {
			throw new Error(
				'frontend analysis compileUnits did not match real TinyGo driver bridge'
			);
		}
		...
		const compileUnitImportPathMatches =
			...
			((compileUnit.kind ?? '') === 'program' &&
				((compileUnitImportPath === 'command-line-arguments' &&
					(bridgeCompileUnit.importPath ?? '') ===
						(bridgeManifest.entryPackage?.importPath ?? '')) ||
	...
	for (const packageInfo of packageGraph) {
		...
		if (
			!bridgePackageInfo &&
			packageImportPath === 'command-line-arguments' &&
			!(packageInfo.depOnly ?? false)
		) {
			bridgeImportPath = bridgeManifest.entryPackage?.importPath ?? '';
			bridgePackageInfo =
				bridgeImportPath === ''
					? undefined
					: bridgePackageGraphByImportPath.get(bridgeImportPath);
		}
	...
	const programCompileUnit = compileUnits.find(
		(compileUnit) => (compileUnit.kind ?? '') === 'program'
	);
...
export const verifyFrontendAnalysisAgainstRealDriverBridgeManifest = (
	...
	if (
		realLLVMTarget === '' ||
		manifestLLVMTarget === '' ||
		manifestLLVMTarget !== realLLVMTarget
	) {
	...
	const manifestBuildContextBuildTags = [...(buildContext.buildTags ?? [])].sort();
	const realBuildContextBuildTags = [
		...(realBuildContext.buildTags ?? bridgeManifest.driverBuildTags ?? [])
	].sort();
	if (
		(buildContext.target ?? '') !== '' ||
		(buildContext.llvmTarget ?? '') !== '' ||
		...
		if (
			(buildContext.target ?? '') !==
				(realBuildContext.target ?? bridgeManifest.target ?? '') ||
			(buildContext.llvmTarget ?? '') !==
				(realBuildContext.llvmTarget ?? bridgeManifest.llvmTriple ?? '') ||
			(buildContext.goos ?? '') !== (realBuildContext.goos ?? bridgeManifest.goos ?? '') ||
			(buildContext.goarch ?? '') !==
				(realBuildContext.goarch ?? bridgeManifest.goarch ?? '') ||
			(buildContext.gc ?? '') !== (realBuildContext.gc ?? bridgeManifest.gc ?? '') ||
			(buildContext.scheduler ?? '') !==
				(realBuildContext.scheduler ?? bridgeManifest.scheduler ?? '')
		) {
			throw new Error(
		...
		if (
			(realBuildContext.modulePath ?? '') !== '' &&
			(buildContext.modulePath ?? '') !== (realBuildContext.modulePath ?? '')
		) {
			throw new Error(
				'frontend analysis buildContext did not match real TinyGo analysis adapter'
			);
	...
	if ((manifest.compileUnitManifestPath ?? '') !== (realAnalysis.compileUnitManifestPath ?? '')) {
		throw new Error(
			'frontend analysis compileUnitManifestPath did not match real TinyGo analysis adapter'
		);
	}
	if (
	...
	if (
		...
		if (
			...
			JSON.stringify(manifestToolchain.cflags ?? []) !==
				JSON.stringify(realToolchain.cflags ?? []) ||
			JSON.stringify(manifestToolchain.ldflags ?? []) !==
				JSON.stringify(realToolchain.ldflags ?? []) ||
			(manifestToolchain.translationUnitPath ?? '') !==
				(realToolchain.translationUnitPath ?? '') ||
	...
	if (compileUnits.length !== realCompileUnits.length) {
		throw new Error(
			'frontend analysis compileUnits did not match real TinyGo analysis adapter'
		);
	}
	...
	for (const compileGroupName of selectedCompileGroupNames) {
		...
		if (!compileGroup || !realCompileGroup) {
			throw new Error(
				'frontend analysis compileGroups did not match real TinyGo analysis adapter'
			);
		}
		const compileGroupFiles = [...(compileGroup.files ?? [])].sort();
	...
	const manifestProgramCompileUnit = compileUnits.find(
		(compileUnit) => (compileUnit.kind ?? '') === 'program'
	);
	const realProgramCompileUnit = realCompileUnits.find(
		(compileUnit) => (compileUnit.kind ?? '') === 'program'
	);
	...
	if (
		(manifestProgramCompileUnit.importPath ?? '') !== (realProgramCompileUnit.importPath ?? '')
	) {
	...
	if (
		(manifestProgramCompileUnit.modulePath ?? '') !==
			(realProgramCompileUnit.modulePath ?? '') ||
		(manifestProgramCompileUnit.packageName ?? '') !==
			(realProgramCompileUnit.packageName ?? '') ||
		(manifestProgramCompileUnit.packageDir ?? '') !==
			(realProgramCompileUnit.packageDir ?? '') ||
	...
	for (const packageInfo of packageGraph) {
		...
		if (
			...
			const realProgramCompileUnit = (realAnalysis.compileUnits ?? []).find(
				(compileUnit) => (compileUnit.kind ?? '') === 'program'
			);
			...
			realPackageInfo =
				realImportPath === ''
					? undefined
					: realPackageGraphByImportPath.get(realImportPath);
		}
		if (!realPackageInfo) {
			throw new Error(
				'frontend analysis packageGraph did not match real TinyGo analysis adapter'
...
export const verifyFrontendRealAdapterAgainstFrontendAnalysis = (
	...
	verifyFrontendAnalysisAgainstRealDriverBridgeManifest(analysisManifest, {
		driverBuildTags:
			manifest.buildContext?.buildTags ?? analysisManifest.buildContext?.buildTags ?? [],
		...
		llvmTriple:
			...
			'',
		scheduler:
			manifest.buildContext?.scheduler ?? analysisManifest.buildContext?.scheduler ?? '',
		target:
...
export const verifyCompileUnitManifestAgainstDriverBridgeManifest = (
	...
	if (
		(bridgeManifest.artifactOutputPath ?? '') !== '' &&
		normalizedToolchain.artifactOutputPath !== bridgeManifest.artifactOutputPath
	) {
		throw new Error(
			'frontend compile unit artifact output did not match real TinyGo driver bridge'
		);
	}
	if (
		(bridgeManifest.entryFile ?? '') !== '' &&
		manifest.entryFile !== bridgeManifest.entryFile
	) {
		throw new Error('frontend compile unit entry file did not match real TinyGo driver bridge');
	}

	const programCompileUnits = compileUnits.filter(
		(compileUnit) => compileUnit.kind === 'program'
	);
	...
	const bridgeEntryGoFiles = (bridgeEntryPackage.goFiles ?? [])
		.map((file) => {
	...
	if (
		(bridgeEntryPackage.name ?? '') !== '' &&
		(programCompileUnit.packageName ?? '') !== bridgeEntryPackage.name
	) {
		throw new Error(
			'frontend compile unit program package did not match real TinyGo driver bridge'
		);
	...
	if (
		(bridgeEntryPackage.dir ?? '') !== '' &&
		(programCompileUnit.packageDir ?? '') !== bridgeEntryPackage.dir
	) {
		throw new Error(
			'frontend compile unit program package did not match real TinyGo driver bridge'
		);
	...
	if (
		(bridgeEntryPackage.modulePath ?? '') !== '' &&
		(programCompileUnit.modulePath ?? '') !== (bridgeEntryPackage.modulePath ?? '')
	) {
		throw new Error(
			'frontend compile unit program package did not match real TinyGo driver bridge'
		);
	...
	if (
		bridgeEntryGoFiles.length !== 0 &&
		JSON.stringify(programFiles) !== JSON.stringify(bridgeEntryGoFiles)
	) {
		throw new Error(
			'frontend compile unit program package did not match real TinyGo driver bridge'
		);
	}
	const compileUnitFileCount = compileUnits.reduce(
		(count, compileUnit) => count + (compileUnit.files ?? []).length,
		0
	);
	const bridgeFileKeys = new Set<string>();
	for (const goFile of bridgeEntryPackage.goFiles ?? []) {
		const bridgeEntryImportPath =
			bridgeEntryPackage.importPath ??
			programCompileUnit.importPath ??
	...
	for (const importPath of bridgeEntryImports) {
		if (!availableCompileUnitImports.has(importPath)) {
			throw new Error(
				'frontend compile unit imports did not cover real TinyGo driver bridge imports'
			);
		}
	}
	const bridgePackageGraphImportPaths = [
		...new Set(
			(bridgeManifest.packageGraph ?? [])
				.map((packageInfo) => packageInfo.importPath ?? '')
				.filter(
					(importPath) =>
						importPath !== '' && importPath !== (bridgeEntryPackage.importPath ?? '')
				)
		)
	].sort();
	if (bridgePackageGraphImportPaths.length !== 0) {
		...
		for (const compileUnit of compileUnits.filter(
			...
			if (Array.isArray(compileUnit.imports)) {
				...
				if (
					...
					JSON.stringify(compileUnitDirectImports) !==
					JSON.stringify(bridgePackageDirectImports)
				) {
					throw new Error(
						'frontend compile unit package graph did not match real TinyGo driver bridge'
					);
			...
			if (
				(bridgePackage.name ?? '') !== '' &&
				(compileUnit.packageName ?? '') !== bridgePackage.name
			) {
				throw new Error(
					'frontend compile unit package graph did not match real TinyGo driver bridge'
				);
			...
			if (
				(bridgePackage.modulePath ?? '') !== '' &&
				(compileUnit.modulePath ?? '') !== (bridgePackage.modulePath ?? '')
			) {
				throw new Error(
					'frontend compile unit package graph did not match real TinyGo driver bridge'
				);
	...
	const graphPackageCount =
		bridgeGraphPackages.length !== 0
			...
			: ((bridgeEntryPackage.importPath ?? '') !== '' ? 1 : 0) +
				bridgePackageGraphImportPaths.length;
	const programImportAlias =
		(programCompileUnit.importPath ?? '') === 'command-line-arguments' &&
		(bridgeEntryPackage.importPath ?? '') !== '' &&
		(programCompileUnit.importPath ?? '') !== bridgeEntryPackage.importPath
	...
	return {
		...
		depOnlyPackageCount: bridgeGraphPackages.filter(
			(packageInfo) => packageInfo.depOnly === true
		).length,
		entryFile: manifest.entryFile ?? '',
		graphPackageCount,
		llvmTarget: normalizedToolchain.llvmTarget,
		localPackageCount: compileUnits.filter(
			(compileUnit) => (compileUnit.kind ?? '') !== 'stdlib'
		).length,
		programImportAlias,
		programFiles,
		...
		standardPackageCount: bridgeGraphPackages.filter(
			(packageInfo) => packageInfo.standard === true
		).length,
...
export const verifyCompileUnitManifestAgainstCompileRequest = (
	...
	if (
		(compileRequest.cflags !== undefined &&
			JSON.stringify(normalizedToolchain.cflags) !== JSON.stringify(compileRequest.cflags)) ||
		(compileRequest.ldflags !== undefined &&
			JSON.stringify(normalizedToolchain.ldflags) !==
				JSON.stringify(compileRequest.ldflags)) ||
		(compileRequest.target !== undefined &&
			normalizedToolchain.target !== compileRequest.target) ||
		(compileRequest.llvmTarget !== undefined &&
			normalizedToolchain.llvmTarget !== compileRequest.llvmTarget) ||
		(compileRequest.linker !== undefined &&
			normalizedToolchain.linker !== compileRequest.linker) ||
	...
	if (
		...
		throw new Error(
			'frontend compile unit source selection was missing normalized compile inputs'
		);
	}
	...
	const compileUnits =
		Array.isArray(manifest.compileUnits) && manifest.compileUnits.length !== 0
			? manifest.compileUnits.map((compileUnit) => ({
					...
					...(Array.isArray(compileUnit.imports)
						? { imports: [...compileUnit.imports] }
						: {}),
	...
	if (compileUnits.length !== 0) {
		...
		for (const compileUnit of compileUnits) {
			if (
				...
				(compileUnit.imports !== undefined &&
					(!Array.isArray(compileUnit.imports) ||
						compileUnit.imports.some((importPath) => importPath === '')))
			) {
				throw new Error(
					'frontend compile unit source selection was missing normalized compile inputs'
				);
			...
			for (const filePath of compileUnit.files) {
				...
				if (
					fileDir !== compileUnit.packageDir ||
					!allCompileFileSet.has(filePath) ||
					seenCompileFiles.has(filePath)
				) {
					throw new Error(
						'frontend compile unit source selection was missing normalized compile inputs'
					);
		...
		if (
			seenCompileFiles.size !== allCompileFiles.length ||
			!programFiles.includes(manifest.entryFile)
		) {
			throw new Error(
				'frontend compile unit source selection was missing normalized compile inputs'
			);
		...
		programFiles.push(
			...(Array.isArray(manifest.sourceSelection?.program)
				...
				: allCompileFiles.filter((filePath) => {
						if (
							importedPackageFiles.includes(filePath) ||
							stdlibPackageFiles.includes(filePath)
						) {
							return false;
						}
						const slashIndex = filePath.lastIndexOf('/');
	...
	const targetAssetFiles = Array.isArray(manifest.sourceSelection?.targetAssets)
		...
		: materializedFiles.filter((filePath) =>
				filePath.startsWith('/working/.tinygo-root/targets/')
			);
	...
	const runtimeSupportFiles = Array.isArray(manifest.sourceSelection?.runtimeSupport)
		...
		: materializedFiles.filter((filePath) => {
				if (
					generatedFileSet.has(filePath) ||
					stdlibFileSet.has(filePath) ||
					targetAssetSet.has(filePath)
				) {
					return false;
				}
				return true;
	...
	if (
		...
		(compileRequest.runtimeSupportFiles !== undefined &&
			JSON.stringify(runtimeSupportFiles) !==
				JSON.stringify(compileRequest.runtimeSupportFiles)) ||
		(compileRequest.programFiles !== undefined &&
			JSON.stringify(programFiles) !== JSON.stringify(compileRequest.programFiles)) ||
		(compileRequest.importedPackageFiles !== undefined &&
			JSON.stringify(importedPackageFiles) !==
				JSON.stringify(compileRequest.importedPackageFiles)) ||
		(compileRequest.stdlibPackageFiles !== undefined &&
			JSON.stringify(stdlibPackageFiles) !==
				JSON.stringify(compileRequest.stdlibPackageFiles)) ||
	...
	if (
		compileRequest.toolPlan?.length &&
		JSON.stringify(toolPlan) !== JSON.stringify(compileRequest.toolPlan)
	) {
...
export const verifyIntermediateManifestAgainstCompileUnitManifest = (
	...
	if (
		JSON.stringify(compileUnitVerification.toolchain) !==
		JSON.stringify(intermediateManifest.toolchain ?? {})
	) {
		throw new Error('frontend intermediate toolchain did not match compile unit manifest');
	}
	if (
		JSON.stringify(compileUnitVerification.sourceSelection) !==
		JSON.stringify(intermediateManifest.sourceSelection ?? {})
	) {
		throw new Error(
			'frontend intermediate source selection did not match compile unit manifest'
		);
	}
	if (
		JSON.stringify(compileUnitVerification.compileUnits) !==
		JSON.stringify(intermediateManifest.compileUnits ?? [])
	) {
...
export const verifyLoweringManifestAgainstIntermediateManifest = (
	...
	if (
		JSON.stringify(intermediateManifest.toolchain ?? {}) !==
		JSON.stringify(loweringManifest.toolchain ?? {})
	) {
	...
	if (
		JSON.stringify(intermediateManifest.compileUnits ?? []) !==
		JSON.stringify(loweringManifest.compileUnits ?? [])
	) {
...
export const verifyWorkItemsManifestAgainstLoweringManifest = (
	...
	if (
		JSON.stringify(loweringManifest.toolchain ?? {}) !==
		JSON.stringify(workItemsManifest.toolchain ?? {})
	) {
	...
	const workItems = (loweringManifest.compileUnits ?? []).map((compileUnit) => {
		...
		const packageFacts = normalizePackageFactsByKind(
			kind,
			compileUnit.depOnly,
			compileUnit.standard
		);
...
export const verifyLoweredCommandBatchAgainstCompileUnitAndLoweredSourcesManifest = (
	...
	if ((compileUnitManifest.entryFile ?? '') !== (loweredSourcesManifest.entryFile ?? '')) {
		throw new Error(
			'frontend lowered command batch entry file did not match compile-unit and lowered sources manifests'
		);
	}
	if ((compileUnitManifest.optimizeFlag ?? '') !== (loweredSourcesManifest.optimizeFlag ?? '')) {
		throw new Error(
			'frontend lowered command batch optimize flag did not match compile-unit and lowered sources manifests'
		);
	}
	...
	const compileCommands = (loweredSourcesManifest.units ?? []).map((unit) => {
		...
		const objectOutputPath = loweredSourcePath.endsWith('.c')
			? `${loweredSourcePath.slice(0, -2)}.o`
			: `${loweredSourcePath}.o`;
		return {
			argv: [
				'/usr/bin/clang',
	...
	if (
		...
		JSON.stringify(commandBatchManifest.compileCommands ?? [])
	) {
		throw new Error(
	...
	const linkCommand = {
		argv: [
			...
			...(loweredSourcesManifest.units ?? []).map((unit) => {
				const loweredSourcePath = unit.loweredSourcePath ?? '';
				return loweredSourcePath.endsWith('.c')
					? `${loweredSourcePath.slice(0, -2)}.o`
					: `${loweredSourcePath}.o`;
			}),
			'-o',
	...
	if (JSON.stringify(linkCommand) !== JSON.stringify(commandBatchManifest.linkCommand ?? {})) {
		throw new Error(
			'frontend lowered command batch link command did not match compile-unit and lowered sources manifests'
		);
	}
...
export const verifyLoweringPlanAgainstWorkItemsManifest = (
	...
	if (
		JSON.stringify(executionLinkJob) !==
		JSON.stringify(loweringPlanManifest.executionLinkJob ?? {})
	) {
		throw new Error(
			'frontend lowering plan execution link job did not match work items manifest'
		);
	}
...
const deriveLinkBitcodeInputsFromBackendInputManifest = (
	...
	if (
		...
		JSON.stringify(bitcodeInputs) !==
			JSON.stringify(backendInputManifest.linkJob?.bitcodeInputs ?? [])
	) {
		throw new Error(errorMessage);
	}
...
const deriveExecutionLinkBitcodeInputsFromBackendInputManifest = (
	...
	if (
		...
		JSON.stringify(bitcodeInputs) !==
			JSON.stringify(backendInputManifest.executionLinkJob?.bitcodeInputs ?? [])
	) {
		throw new Error(errorMessage);
	}
	return bitcodeInputs;
...
const deriveOptimizeFlagFromBackendInputManifest = (
	...
	const optimizeFlag =
		(backendInputManifest.compileJobs ?? []).reduce<string | null>((derived, compileJob) => {
...
export const verifyBackendInputManifestAgainstLoweringPlanAndLoweredSourcesManifest = (
	...
	const normalizedLoweringPlanCompileJobs = (loweringPlanManifest.compileJobs ?? []).map(
		(compileJob) => {
			const packageFacts = normalizePackageFactsByKind(
				compileJob.kind ?? '',
				compileJob.depOnly,
	...
	const normalizedBackendInputCompileJobs = (backendInputManifest.compileJobs ?? []).map(
		(compileJob) => {
			const packageFacts = normalizePackageFactsByKind(
				compileJob.kind ?? '',
				compileJob.depOnly,
	...
	if (
		JSON.stringify(normalizedLoweringPlanCompileJobs) !==
		JSON.stringify(normalizedBackendInputCompileJobs)
	) {
		throw new Error(
			'frontend backend input did not match lowering plan and lowered sources manifests'
		);
...
export const verifyCommandBatchAgainstBackendInputManifest = (
	...
	const compileCommands = (backendInputManifest.compileJobs ?? []).map((compileJob) => {
		...
		if (
			...
			JSON.stringify(compileJob.files ?? []) !==
				JSON.stringify(loweredUnit.sourceFiles ?? []) ||
	...
	if (
		commandArtifactManifest?.runnable === true &&
		commandArtifactManifest.entrypoint === 'main'
	) {
...
export const verifyBackendResultManifestAgainstBackendInputAndLoweredBitcodeManifest = (
	...
	if (
		JSON.stringify(
			(backendResultManifest.generatedFiles ?? []).map((file) => file.path ?? '')
	...
	try {
		...
		loweredIRManifest = JSON.parse(
			backendResultManifest.generatedFiles?.[
				(backendResultManifest.generatedFiles?.length ?? 0) - 5
			]?.contents ?? '{}'
		) as TinyGoLoweredIRManifest;
		loweredCommandBatchManifest = JSON.parse(
			backendResultManifest.generatedFiles?.[
				(backendResultManifest.generatedFiles?.length ?? 0) - 4
			]?.contents ?? '{}'
		) as TinyGoCommandBatchManifest;
		loweredArtifactManifest = JSON.parse(
			backendResultManifest.generatedFiles?.[
				(backendResultManifest.generatedFiles?.length ?? 0) - 3
			]?.contents ?? '{}'
		) as TinyGoLoweredArtifactManifest;
		commandArtifactManifest = JSON.parse(
			backendResultManifest.generatedFiles?.[
				(backendResultManifest.generatedFiles?.length ?? 0) - 2
			]?.contents ?? '{}'
		) as TinyGoCommandArtifactManifest;
		commandBatchManifest = JSON.parse(
			backendResultManifest.generatedFiles?.[
				(backendResultManifest.generatedFiles?.length ?? 0) - 1
			]?.contents ?? '{}'
		) as TinyGoCommandBatchManifest;
	} catch {
		throw new Error('frontend backend result did not match backend input manifest');
	...
	if (
		JSON.stringify(
			(loweredIRManifest.units ?? []).map((unit) => ({
				id: unit.id ?? '',
				kind: unit.kind ?? '',
				importPath: unit.importPath ?? '',
				modulePath: unit.modulePath ?? '',
	...
	for (const unit of loweredIRManifest.units ?? []) {
		...
		if (
			JSON.stringify(
				(unit.placeholderBlocks ?? []).map((block) => ({
					stage: block.stage ?? '',
					index: block.index ?? 0,
					value: block.value ?? '',
		...
		for (const [importIndex, loweredImport] of (unit.imports ?? []).entries()) {
			...
			const importSignature =
				importAlias !== '' ? `${importAlias}=${importPath}` : importPath;
	...
	if (
		JSON.stringify(loweredBitcodeManifest.bitcodeFiles ?? []) !==
		JSON.stringify(loweredBitcodeManifestFromResult.bitcodeFiles ?? [])
	) {
		throw new Error('frontend backend result did not match backend input manifest');
	}
...
export const buildLoweredBitcodeCompileCommandsFromLoweringPlanAndLoweredSourcesManifest = (
	...
	const compileCommands = (loweringPlanManifest.compileJobs ?? []).map((compileJob) => {
		...
		if (
			...
			JSON.stringify(compileJob.files ?? []) !==
				JSON.stringify(loweredSourceUnit.sourceFiles ?? []) ||
...
export const verifyLoweredBitcodeManifestAgainstLoweringPlanAndLoweredSourcesManifest = (
	...
	if (
		JSON.stringify(bitcodeFiles) !== JSON.stringify(loweredBitcodeManifest.bitcodeFiles ?? [])
	) {
		throw new Error(
...
export const verifyCommandArtifactManifestAgainstBackendInputAndLoweredBitcodeManifest = (
	...
	const artifactOutputPath =
		backendInputManifest.executionLinkJob?.artifactOutputPath ??
		backendInputManifest.linkJob?.artifactOutputPath ??
		'';
	if (artifactOutputPath !== (commandArtifactManifest.artifactOutputPath ?? '')) {
		throw new Error(
			'frontend command artifact manifest did not match backend input and lowered bitcode manifest'
		);
	}
	if (
		JSON.stringify(
			backendInputManifest.executionLinkJob !== undefined
				? (backendInputManifest.executionLinkJob.bitcodeInputs ??
						loweredBitcodeManifest.bitcodeFiles ??
						[])
				: (backendInputManifest.linkJob?.bitcodeInputs ??
						loweredBitcodeManifest.bitcodeFiles ??
						[])
		) !== JSON.stringify(commandArtifactManifest.bitcodeFiles ?? [])
	) {
