...
export type TinyGoBuildRequestOverrides = Partial<
	Pick<TinyGoBuildRequest, 'optimize' | 'panic' | 'scheduler' | 'target'>
>;
...
type TinyGoRuntimeAction =
	| 'booting'
	| 'planning'
	| 'executing'
	| 'upstream-probe'
	| 'upstream-frontend-probe';
...
const describePureBrowserFallbackFailure = (
	...
	const userUnits = loweredIRVerification.units.filter(
		(unit) => (unit.kind ?? '') === 'program' || (unit.kind ?? '') === 'imported'
	);
	...
	if (
		userUnits.some((unit) => (unit.functions ?? []).some((functionInfo) => functionInfo.method))
	) {
		addBlocker('methods');
	}
	if (
		userUnits.some((unit) =>
			(unit.types ?? []).some((typeInfo) => (typeInfo.kind ?? '') === 'struct')
		)
	) {
	...
	if (
		userUnits.some((unit) =>
			(unit.types ?? []).some((typeInfo) => (typeInfo.kind ?? '') === 'interface')
		)
	) {
	...
	if (
		userUnits.some((unit) =>
			(unit.types ?? []).some((typeInfo) => (typeInfo.kind ?? '') === 'map')
		)
	) {
	...
	if (
		userUnits.some((unit) =>
			(unit.types ?? []).some((typeInfo) => (typeInfo.kind ?? '') === 'chan')
		)
	) {
	...
	if (
		userUnits.some((unit) =>
			(unit.types ?? []).some((typeInfo) => (typeInfo.kind ?? '') === 'array')
		)
	) {
	...
	if (
		userUnits.some((unit) =>
			(unit.types ?? []).some((typeInfo) => (typeInfo.kind ?? '') === 'slice')
		)
	) {
	...
	if (
		userUnits.some((unit) =>
			(unit.types ?? []).some((typeInfo) => (typeInfo.kind ?? '') === 'pointer')
		)
	) {
...
export const createTinyGoRuntime = (options: TinyGoRuntimeOptions): TinyGoRuntime => {
	...
	const appendLog = (message: string, tone: Exclude<PhaseTone, 'idle'> | 'idle' = 'idle') => {
		const stamp = (options.now?.() ?? new Date()).toLocaleTimeString('ko-KR', {
			hour12: false
		});
	...
	const loadCompilerManifest = async (): Promise<TinyGoCompilerManifest | null> => {
		if (compilerManifestPromise === null) {
			compilerManifestPromise = (async () => {
				try {
					const manifestBytes = await loadAssetBytes(
						'tools/tinygo-compiler.json',
						'tinygo-compiler.json'
					);
	...
	const loadCompilerModuleBytes = async () => {
		try {
			...
			const compilerBytes = await loadAssetBytes(
			...
			if (compilerManifest?.artifactKind === 'compiler') {
				...
				const blockers =
					Array.isArray(compilerManifest?.blockers) &&
					compilerManifest.blockers.length > 0
	...
	const runUpstreamProbe = async (): Promise<TinyGoUpstreamProbeResult> => {
		const [probeBytes, targetSource, zversionSource, armSource] = await Promise.all([
			...
			loadAssetBytes(
				'tools/tinygo-upstream-probe/targets/wasip1.json',
				'tinygo-upstream-probe target wasip1.json'
			),
			loadAssetBytes(
				'tools/tinygo-upstream-probe/src/runtime/internal/sys/zversion.go',
				'tinygo-upstream-probe zversion.go'
			),
			loadAssetBytes(
				'tools/tinygo-upstream-probe/src/device/arm/arm.go',
				'tinygo-upstream-probe arm.go'
			)
		]);
	...
	const verifyAndLogUpstreamFrontendProbe = (
		...
		const driverBridgeManifest =
			injectedDriverBridgeManifest === null
				? null
				: cloneJsonValue(injectedDriverBridgeManifest);
		if ((options?.verifyDriverBridge ?? true) && driverBridgeManifest?.packageGraph?.length) {
			const bridgeVerification = verifyUpstreamFrontendProbeAgainstDriverBridgeManifest(
				result,
		...
		if (
			(options?.verifyAnalysisInput ?? true) &&
			driverBridgeManifest?.frontendAnalysisInput?.packageGraph?.length
		) {
			const analysisInputVerification =
				verifyUpstreamFrontendProbeAgainstFrontendAnalysisInputManifest(
					result,
		...
		if (options?.frontendAnalysisManifest?.packageGraph?.length) {
			const frontendAnalysisVerification =
				verifyUpstreamFrontendProbeAgainstFrontendAnalysisManifest(
					result,
		...
		if (options?.frontendRealAdapterManifest?.packageGraph?.length) {
			const frontendRealAdapterVerification =
				verifyUpstreamFrontendProbeAgainstFrontendRealAdapterManifest(
					result,
	...
	const runUpstreamFrontendProbe = async (
		...
		const buildContextModulePath =
			driverBridgeManifest?.frontendAnalysisInput?.buildContext?.modulePath ?? '';
		...
		const normalizePackageDirForProbe = (packageInfo: {
			...
			if (
				packageDir === browserTinyGoRoot ||
				packageDir.startsWith(`${browserTinyGoRoot}/`)
			) {
				return packageDir;
			}
		...
		const packageList = driverBridgeManifest?.packageGraph?.length
			? driverBridgeManifest.packageGraph.map((packageInfo) => {
					...
					const name =
						packageInfo.name ??
						(importPath !== '' ? (importPath.split('/').pop() ?? 'main') : 'main');
			...
			: (() => {
					const goFiles = Object.keys(workspaceFiles).filter(
						(filePath) => filePath.endsWith('.go') && !filePath.includes('/')
					);
					return [
						{
							Dir: '/workspace',
		...
		const [probeBytes, targetSource] = await Promise.all([
			loadAssetBytes(
				...
				'tinygo-upstream-frontend-probe.wasm'
			),
			loadAssetBytes(
		...
		const tinygoRootFileReads = await Promise.all(
			[...requiredTinyGoRootFiles]
				.sort()
				.map(
					async (relativeAssetPath) =>
						[
							relativeAssetPath,
							textDecoder.decode(
		...
		if (exitCode !== 0) {
			throw new Error(
				`upstream frontend probe exited with code ${exitCode}: ${stderrLines.join('\n')}`
			);
		}
		if (stderrLines.length) {
			throw new Error(stderrLines.join('\n'));
	...
	const planBuild = async (
		...
		try {
			...
			const workspace = new PreopenDirectory(
				'/workspace',
				buildDirectoryContentsFromTextEntries(workspaceFiles, textEncoder)
			);
			const stdout = ConsoleStdout.lineBuffered((line) =>
				appendLog(`driver ${line}`, 'running')
			);
			const stderr = ConsoleStdout.lineBuffered((line) =>
				appendLog(`driver ${line}`, 'error')
			);
			const wasi = new WASI(
				['tinygo-driver'],
			...
			if (!(resultNode instanceof File)) {
				throw new Error(
					`tinygo driver did not write /workspace/tinygo-result.json (exit ${driverExitCode})`
				);
			}
			...
			if (buildStateVersionAtStart === buildStateVersion) {
				...
				appendLog(
					`driver planned ${result.plan.length} step(s) for ${result.artifact ?? '/working/out.wasm'}`,
					'success'
				);
				appendLog(
					`driver workspace files=${Object.keys(workspaceFiles).length - 1}`,
					'idle'
				);
			...
			if (buildStateVersionAtStart === buildStateVersion) {
				...
				appendLog(
					`build driver failed: ${error instanceof Error ? error.message : String(error)}`,
					'error'
				);
			}
			throw error;
		...
		try {
			...
			const buildRequestOverrides =
				injectedBuildRequestOverrides === null
					? null
					: cloneJsonValue(injectedBuildRequestOverrides);
			...
			const driverBridgeManifest =
				injectedDriverBridgeManifest === null
					...
					: cloneJsonValue(injectedDriverBridgeManifest);
			const workspaceFiles =
				injectedWorkspaceFiles === null ? null : cloneJsonValue(injectedWorkspaceFiles);
			const result =
				lastBuildResult ??
				(await planBuild({
			...
			const plannerManifestSource = result.files.find(
				(file) => file.path === '/working/tinygo-bootstrap.json'
			)?.contents;
			const frontendInputSource = result.files.find(
				(file) => file.path === '/working/tinygo-frontend-input.json'
			)?.contents;
			...
			let frontendLoweredIRVerification:
				| ReturnType<
						typeof verifyBackendResultManifestAgainstBackendInputAndLoweredBitcodeManifest
				  >['loweredIR']
				| null = null;
			let frontendLoweredArtifactVerification:
				| ReturnType<
						typeof verifyBackendResultManifestAgainstBackendInputAndLoweredBitcodeManifest
				  >['loweredArtifact']
				| null = null;
			let frontendLoweredBitcodeManifestVerification: ReturnType<
				typeof verifyLoweredBitcodeManifestAgainstLoweringPlanAndLoweredSourcesManifest
			> | null = null;
			let frontendCommandArtifactVerification:
				| ReturnType<
						typeof verifyBackendResultManifestAgainstBackendInputAndLoweredBitcodeManifest
				  >['commandArtifact']
				| null = null;
			let frontendLoweredBitcodeCompileCommands: ToolInvocation[] | null = null;
			let frontendCompileUnitVerification: ReturnType<
				typeof verifyCompileUnitManifestAgainstCompileRequest
			> | null = null;
			...
			if (frontendInputSource) {
				const parsedFrontendInputManifest = JSON.parse(
					frontendInputSource
				) as TinyGoFrontendInputManifest;
				const {
					compileUnits: _frontendAnalysisCompileUnits,
				...
				if (driverBridgeManifest) {
					const frontendInputVerification =
						verifyFrontendInputManifestAgainstDriverBridgeManifest(
							parsedFrontendInputManifest,
							driverBridgeManifest
						);
					...
					if (driverBridgeManifest.frontendAnalysisInput) {
						const frontendAnalysisInputVerification =
							verifyFrontendAnalysisInputManifestAgainstDriverBridgeManifest(
				...
				if (driverBridgeManifest?.upstreamFrontendProbe) {
					...
					appendLog(
						'frontend analysis input upstream frontend probe source=bridge',
						'success'
					);
				} else if (driverBridgeManifest?.packageGraph?.length) {
					upstreamFrontendProbeResult = await runUpstreamFrontendProbe(null, null);
					appendLog(
						'frontend analysis input upstream frontend probe source=browser',
						'success'
					);
				}
				...
				for (const file of filesToMaterialize) {
					...
					for (const [index, part] of parts.entries()) {
						...
						const existing = currentDirectory.get(part);
						if (existing instanceof Directory) {
							currentDirectory = existing.contents as unknown as Map<
								string,
								File | Directory
							>;
							continue;
						...
						const directory = new Directory(new Map());
						currentDirectory.set(part, directory);
						currentDirectory = directory.contents as unknown as Map<
							string,
							File | Directory
						>;
					}
				...
				working.dir.contents.set(
					...
					new File(
						textEncoder.encode(JSON.stringify(frontendAnalysisInputManifest, null, 2))
					)
				);
				...
				const bridgeFrontendRealAdapterManifest =
					driverBridgeManifest?.frontendRealAdapter ??
					driverBridgeManifest?.realFrontendAnalysis;
				if (!bridgeFrontendAnalysisManifest) {
					const frontendAnalysisStdout = ConsoleStdout.lineBuffered((line) =>
						appendLog(`frontend analysis ${line}`, 'running')
					);
					const frontendAnalysisStderr = ConsoleStdout.lineBuffered((line) =>
						appendLog(`frontend analysis ${line}`, 'error')
					);
					const frontendAnalysisWasi = new WASI(
						...
						[
							...
							frontendAnalysisStdout,
							frontendAnalysisStderr,
							working
						]
					);
					const frontendAnalysisInstance = await instantiateWasiModule(
						frontendModuleBytes,
						{
					...
					try {
						frontendAnalysisExitCode = frontendAnalysisWasi.start(
							frontendAnalysisInstance as {
					...
					const frontendAnalysisResultNode = working.dir.contents.get(
						'tinygo-frontend-analysis.json'
					);
					if (!(frontendAnalysisResultNode instanceof File)) {
						throw new Error(
							`tinygo frontend-analysis did not write /working/tinygo-frontend-analysis.json (exit ${frontendAnalysisExitCode})`
						);
					}
					...
					if (
						...
						parsedFrontendInputManifest.toolchain?.target
					) {
					...
					if (
						...
						parsedFrontendInputManifest.buildContext?.llvmTarget
					) {
						throw new Error(
							'frontend analysis llvmTarget did not match frontend input'
						);
					}
					if (
						frontendAnalysisResult.analysis.compileUnitManifestPath !==
						'/working/tinygo-compile-unit.json'
					) {
						throw new Error(
							'frontend analysis did not preserve the compile unit manifest path'
						);
					}
					if (
						upstreamFrontendProbeResult &&
						!frontendAnalysisResult.analysis.upstreamFrontendProbe
					) {
						throw new Error(
							'frontend analysis did not preserve upstream frontend probe facts'
						);
					}
				...
				if (bridgeFrontendRealAdapterManifest && driverBridgeManifest) {
					const realFrontendAnalysisSource = driverBridgeManifest.frontendRealAdapter
						? 'canonical'
						: 'compat-alias';
					if (bridgeFrontendAnalysisManifest) {
						const frontendAnalysisVerification =
					...
					const realFrontendAnalysisVerification =
						verifyFrontendAnalysisAgainstRealDriverBridgeManifest(
							frontendRealAdapterManifest,
							driverBridgeManifest
						);
					...
					if (frontendAnalysisManifest) {
						const frontendRealAdapterSeamVerification =
							verifyFrontendRealAdapterAgainstFrontendAnalysis(
								frontendRealAdapterManifest,
								frontendAnalysisManifest
							);
									...
									JSON.stringify(
										{
											...
											diagnostics: []
										} satisfies TinyGoFrontEndAnalysisResult,
										null,
										2
									)
								)
								...
								JSON.stringify(
									{
										...
										diagnostics: []
									} satisfies TinyGoFrontEndAdapterResult,
									null,
									2
								)
							)
					...
					const frontendRealAdapterStdout = ConsoleStdout.lineBuffered((line) =>
						appendLog(`frontend real adapter ${line}`, 'running')
					);
					const frontendRealAdapterStderr = ConsoleStdout.lineBuffered((line) =>
						appendLog(`frontend real adapter ${line}`, 'error')
					);
					const frontendRealAdapterWasi = new WASI(
						...
						[
							new OpenFile(new File([])),
							frontendRealAdapterStdout,
							frontendRealAdapterStderr,
							working
						]
					);
					const frontendRealAdapterInstance = await instantiateWasiModule(
						frontendModuleBytes,
						{
					...
					try {
						frontendRealAdapterExitCode = frontendRealAdapterWasi.start(
							frontendRealAdapterInstance as {
								exports: { memory: WebAssembly.Memory; _start: () => unknown };
							}
						);
					} catch (error) {
					...
					const frontendRealAdapterResultNode = working.dir.contents.get(
						'tinygo-frontend-real-adapter.json'
					);
					if (!(frontendRealAdapterResultNode instanceof File)) {
						throw new Error(
							`tinygo frontend-real-adapter did not write /working/tinygo-frontend-real-adapter.json (exit ${frontendRealAdapterExitCode})`
						);
					}
					...
					if (
						...
						throw new Error(
							'frontend real adapter target did not match frontend input'
						);
					}
					if (
						...
						throw new Error(
							'frontend real adapter llvmTarget did not match frontend input'
						);
					}
					if (
						frontendRealAdapterResult.adapter.compileUnitManifestPath !==
						'/working/tinygo-compile-unit.json'
					) {
						throw new Error(
							'frontend real adapter did not preserve the compile unit manifest path'
						);
					}
					if (
						(frontendRealAdapterResult.adapter.compileUnits?.length ?? 0) !==
						(parsedFrontendInputManifest.compileUnits?.length ?? 0)
					) {
						throw new Error(
							'frontend real adapter compile unit count did not match frontend input'
						);
					}
					if (
						(frontendRealAdapterResult.adapter.allCompileFiles?.length ?? 0) !==
						(parsedFrontendInputManifest.sourceSelection?.allCompile?.length ?? 0)
					) {
						throw new Error(
							'frontend real adapter all-compile count did not match frontend input'
						);
					}
					appendLog(
					...
					appendLog(
						`frontend real adapter source=${frontendAnalysisManifest ? 'analysis' : 'input'}`,
						'success'
					);
				}
				const frontendStdout = ConsoleStdout.lineBuffered((line) =>
					appendLog(`frontend ${line}`, 'running')
				);
				const frontendStderr = ConsoleStdout.lineBuffered((line) =>
					appendLog(`frontend ${line}`, 'error')
				);
				if (
					...
					if (upstreamFrontendProbeResult) {
						...
						upstreamFrontendProbeResult = await runUpstreamFrontendProbe(
							frontendAnalysisManifest,
							frontendRealAdapterManifest
						);
					}
				...
				try {
					frontendExitCode = frontendWasi.start(
						frontendInstance as {
							exports: { memory: WebAssembly.Memory; _start: () => unknown };
						}
					);
				...
				if (!(frontendResultNode instanceof File)) {
					throw new Error(
						`tinygo frontend did not write /working/tinygo-frontend-result.json (exit ${frontendExitCode})`
					);
				}
				frontendResult = JSON.parse(
				...
				const compileUnitManifest = frontendResult.generatedFiles?.find(
					(file) => file.path === '/working/tinygo-compile-unit.json'
				);
				if (!compileUnitManifest) {
					throw new Error(
						'frontend generated files did not include /working/tinygo-compile-unit.json'
					);
				}
				const intermediateManifest = frontendResult.generatedFiles?.find(
					(file) => file.path === '/working/tinygo-intermediate.json'
				);
				if (!intermediateManifest) {
					throw new Error(
						'frontend generated files did not include /working/tinygo-intermediate.json'
					);
				}
				const loweringManifest = frontendResult.generatedFiles?.find(
					(file) => file.path === '/working/tinygo-lowering-input.json'
				);
				if (!loweringManifest) {
					throw new Error(
						'frontend generated files did not include /working/tinygo-lowering-input.json'
					);
				}
				const workItemsManifest = frontendResult.generatedFiles?.find(
					(file) => file.path === '/working/tinygo-work-items.json'
				);
				if (!workItemsManifest) {
					throw new Error(
						'frontend generated files did not include /working/tinygo-work-items.json'
					);
				}
				const loweringPlanManifest = frontendResult.generatedFiles?.find(
					(file) => file.path === '/working/tinygo-lowering-plan.json'
				);
				if (!loweringPlanManifest) {
					throw new Error(
						'frontend generated files did not include /working/tinygo-lowering-plan.json'
					);
				}
				const backendInputManifest = frontendResult.generatedFiles?.find(
					(file) => file.path === '/working/tinygo-backend-input.json'
				);
				if (!backendInputManifest) {
					throw new Error(
						'frontend generated files did not include /working/tinygo-backend-input.json'
					);
				}
				appendLog(`frontend request manifest=${compileUnitManifest.path}`, 'idle');
				const parsedCompileUnitManifest = JSON.parse(
					compileUnitManifest.contents
				) as TinyGoCompileUnitManifest;
				const compileUnitVerification = verifyCompileUnitManifestAgainstCompileRequest(
					parsedCompileUnitManifest,
					{}
				);
				frontendCompileUnitVerification = compileUnitVerification;
				const parsedIntermediateManifest = JSON.parse(
					intermediateManifest.contents
				) as TinyGoIntermediateManifest;
				const intermediateVerification =
					verifyIntermediateManifestAgainstCompileUnitManifest(
						parsedCompileUnitManifest,
						parsedIntermediateManifest
					);
				const parsedLoweringManifest = JSON.parse(
					loweringManifest.contents
				) as TinyGoLoweringManifest;
				const loweringVerification = verifyLoweringManifestAgainstIntermediateManifest(
					parsedIntermediateManifest,
					parsedLoweringManifest
				);
				const parsedWorkItemsManifest = JSON.parse(
					workItemsManifest.contents
				) as TinyGoWorkItemsManifest;
				...
				const parsedLoweredSourcesManifest: TinyGoLoweredSourcesManifest = {
					...
					units: (parsedWorkItemsManifest.workItems ?? []).map((workItem) => ({
						...(() => {
							...
							return {
								...(typeof workItem.depOnly === 'boolean'
									? { depOnly: workItem.depOnly }
									: {}),
								...(typeof workItem.standard === 'boolean'
				...
				const loweredSourcesVerification =
					verifyLoweredSourcesManifestAgainstWorkItemsManifest(
						parsedWorkItemsManifest,
						parsedLoweredSourcesManifest
					);
				const parsedLoweringPlanManifest = JSON.parse(
					loweringPlanManifest.contents
				) as TinyGoLoweringPlanManifest;
				const loweringPlanVerification = verifyLoweringPlanAgainstWorkItemsManifest(
					parsedWorkItemsManifest,
					parsedLoweringPlanManifest
				);
				const parsedBackendInputManifest = JSON.parse(
					backendInputManifest.contents
				) as TinyGoBackendInputManifest;
				const backendInputVerification =
					verifyBackendInputManifestAgainstLoweringPlanAndLoweredSourcesManifest(
						parsedLoweringPlanManifest,
						parsedLoweredSourcesManifest,
				...
				if (
					...
					!frontendResult.generatedFiles?.some(
						(file) =>
							file.path === parsedCompileUnitManifest.toolchain?.translationUnitPath
					)
				) {
					throw new Error(
				...
				if (driverBridgeManifest) {
					...
					if ((driverBridgeManifest.toolchain?.version ?? '') !== '') {
						appendLog(
							`frontend bridge toolchain version=${driverBridgeManifest.toolchain?.version ?? ''}`,
							'idle'
						);
					}
				}
				...
				if (driverBridgeManifest?.hostArtifact?.bytesBase64) {
					...
					const hostArtifactEntrypoint =
						driverBridgeManifest.hostArtifact.entrypoint ??
					...
					bridgedHostArtifact = {
						...
						reason:
							hostArtifactEntrypoint === null
								...
								: driverBridgeManifest.hostArtifact.reason,
						runnable:
							driverBridgeManifest.hostArtifact.runnable ??
							hostArtifactEntrypoint !== null,
					...
					if ((driverBridgeManifest.hostArtifact.command ?? []).length !== 0) {
						appendLog(
							`bridge host artifact command=${(driverBridgeManifest.hostArtifact.command ?? []).join(' ')}`,
							'idle'
						);
					}
					appendLog(
						...
						for (const [index, part] of parts.entries()) {
							if (index === parts.length - 1) {
								currentDirectory.set(
									part,
									new File(textEncoder.encode(file.contents))
								);
								continue;
							}
							const existing = currentDirectory.get(part);
					...
					const backendWorkspace = new PreopenDirectory(
						'/workspace',
						backendWorkspaceContents
					);
					const backendStdout = ConsoleStdout.lineBuffered((line) =>
						appendLog(`backend ${line}`, 'running')
					);
					const backendStderr = ConsoleStdout.lineBuffered((line) =>
						appendLog(`backend ${line}`, 'error')
					);
					...
					try {
						backendExitCode = backendWasi.start(
							backendInstance as {
								exports: { memory: WebAssembly.Memory; _start: () => unknown };
							}
						);
					...
					const backendResultNode = backendWorking.dir.contents.get(
						'tinygo-backend-result.json'
					);
					if (!(backendResultNode instanceof File)) {
						throw new Error(
							`tinygo backend did not write /working/tinygo-backend-result.json (exit ${backendExitCode})`
						);
					}
					const backendResult = JSON.parse(
					...
					const backendResultVerification =
						verifyBackendResultManifestAgainstBackendInputAndLoweredBitcodeManifest(
							parsedBackendInputManifest,
							{
								bitcodeFiles: backendInputVerification.compileJobs.map(
									(compileJob) => compileJob.bitcodeOutputPath ?? ''
								)
							} as TinyGoLoweredBitcodeManifest,
							backendResult
						);
					frontendLoweredBitcodeManifestVerification =
						backendResultVerification.loweredBitcodeManifest;
					if (backendResultVerification.generatedFiles.length) {
						...
						await materializeGeneratedFiles(
							writableFileSystem,
							backendResultVerification.generatedFiles
						);
					}
					const commandBatchVerification = backendResultVerification.commandBatch;
					const loweredCommandBatchVerification =
						backendResultVerification.loweredCommandBatch;
					...
					for (const step of [
						...loweredCommandBatchVerification.compileCommands,
						loweredCommandBatchVerification.linkCommand
					]) {
						appendLog(`$ ${step.argv.join(' ')}`, 'running');
						const stepResult = await runtime._run_process_impl(step.argv, {
							cwd: step.cwd
						});
						if (stepResult.returncode !== 0) {
							...
							appendLog(
								`lowered build step failed with exit code ${stepResult.returncode}`,
								'error'
							);
							return;
						}
					...
					for (const step of loweredBitcodeCompileCommands) {
						...
						for (const [index, segment] of segments.entries()) {
							...
							if (
								...
								loweredBitcodeOutputDirectories.has(currentPath)
							) {
								continue;
							}
							try {
					...
					for (const step of loweredBitcodeCompileCommands) {
						...
						if (stepResult.returncode !== 0) {
							...
							appendLog(
								`lowered bitcode step failed with exit code ${stepResult.returncode}`,
								'error'
							);
							return;
						}
					...
					appendLog(
						`$ ${commandBatchVerification.linkCommand.argv.join(' ')}`,
						'running'
					);
					...
					if (commandLinkResult.returncode !== 0) {
						...
						setPhase('smoke', 'failed', 'error');
						appendLog(
							`final artifact link failed with exit code ${commandLinkResult.returncode}`,
							'error'
						);
						return;
					}
				...
				for (const step of result.plan) {
					...
					if (stepResult.returncode !== 0) {
						...
						appendLog(
							`build step failed with exit code ${stepResult.returncode}`,
							'error'
						);
						return;
					}
			...
			const artifactPath =
				...
				'/working/out.wasm';
			const artifact =
				bridgedHostArtifact?.bytes ?? (await fileSystem.readFile(artifactPath));
			const artifactBytes =
				typeof artifact === 'string' ? textEncoder.encode(artifact) : artifact;
			...
			const manifestArtifactKind =
				bridgedHostArtifact?.artifactKind ??
				frontendCommandArtifactVerification?.artifactKind;
			...
			lastBuildArtifactKind =
				bridgedHostArtifact?.artifactKind ??
				frontendCommandArtifactVerification?.artifactKind ??
				(hasBootstrapManifestExports ? 'bootstrap' : 'execution');
			lastBuildArtifactEntrypoint =
				bridgedHostArtifact?.entrypoint ??
				frontendCommandArtifactVerification?.entrypoint ??
			...
			lastBuildArtifactRunnable =
				bridgedHostArtifact?.runnable ??
				frontendCommandArtifactVerification?.runnable ??
				lastBuildArtifactEntrypoint !== null;
			lastBuildArtifactReason = lastBuildArtifactRunnable
				? undefined
				: (bridgedHostArtifact?.reason ??
			...
			appendLog(
				`build artifact ready: ${artifactPath} (${size.toLocaleString()} bytes)`,
				'success'
			);
			...
			if (frontendResult) {
				if (bridgedHostArtifact) {
					...
					appendLog(
						'frontend lowered verification skipped: bridge host artifact bypassed synthetic backend lowering',
						'idle'
					);
				} else {
					if (!frontendCommandArtifactVerification) {
						throw new Error(
							'missing command artifact verification for final artifact output'
						);
					}
					const finalArtifactVerification = verifyTinyGoFinalArtifactFile(
					...
					if (!frontendLoweredArtifactVerification) {
						throw new Error(
							'missing lowered artifact verification for lowered object outputs'
						);
					}
					const loweredObjectVerification = verifyTinyGoLoweredObjectFiles(
						frontendLoweredArtifactVerification,
						await Promise.all(
							frontendLoweredArtifactVerification.objectFiles.map(
								async (objectFile) => ({
									path: objectFile,
					...
					if (!frontendLoweredBitcodeCompileCommands) {
						throw new Error(
							'missing lowered bitcode compile commands for lowered bitcode outputs'
						);
					}
					if (!frontendLoweredBitcodeManifestVerification) {
						throw new Error(
							'missing lowered bitcode manifest verification for lowered bitcode outputs'
						);
					}
					const loweredBitcodeVerification = verifyTinyGoLoweredBitcodeFiles(
						frontendLoweredBitcodeManifestVerification.bitcodeFiles,
						await Promise.all(
							frontendLoweredBitcodeManifestVerification.bitcodeFiles.map(
								async (bitcodeFile) => ({
									path: bitcodeFile,
					...
					const loweredArtifact = await fileSystem.readFile(
						'/working/tinygo-lowered-out.wasm'
					);
					const loweredArtifactSize =
						typeof loweredArtifact === 'string'
							? loweredArtifact.length
							: loweredArtifact.byteLength;
					appendLog(
						`frontend lowered artifact ready: /working/tinygo-lowered-out.wasm (${loweredArtifactSize.toLocaleString()} bytes)`,
						'success'
					);
				}
			}
			try {
				...
				if (lastBuildArtifactRunnable) {
					...
					if (lastBuildArtifactKind !== 'bootstrap') {
						if (!frontendCommandArtifactVerification) {
							...
							appendLog(
								'frontend bootstrap probe skipped: final artifact is not bootstrap',
								'error'
							);
							return;
						}
						...
						if (!frontendBootstrapArtifactBytes) {
							setPhase('verify', 'failed', 'error');
							appendLog(
								'frontend bootstrap probe skipped: bootstrap artifact missing',
								'error'
							);
							return;
						}
						const bootstrapArtifactBytes = frontendBootstrapArtifactBytes;
						const bootstrapModule = (await WebAssembly.instantiate(
						...
						if (bootstrapManifest) {
							...
							const bootstrapAllFileCount = Array.isArray(
								parsedBootstrapManifest.sourceSelection?.allCompile
							)
							...
							if (bootstrapVerification?.ok) {
								...
								if (frontendResult) {
									if (!frontendLoweredSourcesManifest) {
										throw new Error(
											'missing lowered sources manifest for lowered artifact probe'
										);
									}
									const loweredSourceFileContents: Record<
										string,
										string | Uint8Array
									> = {};
									for (const unit of frontendLoweredSourcesManifest.units ?? []) {
										for (const sourceFile of unit.sourceFiles ?? []) {
											if (
												loweredSourceFileContents[sourceFile] !== undefined
											) {
												continue;
											}
											try {
												loweredSourceFileContents[sourceFile] =
													await fileSystem.readFile(sourceFile);
											} catch (error) {
												const workspaceRelativePath = sourceFile.startsWith(
													'/workspace/'
												)
												...
												if (
													...
													workspaceFiles !== null &&
													workspaceFiles[workspaceRelativePath] !==
														undefined
												) {
													loweredSourceFileContents[sourceFile] =
														workspaceFiles[workspaceRelativePath];
									...
									const loweredModuleBytes =
										typeof loweredArtifact === 'string'
											? textEncoder.encode(loweredArtifact)
											: loweredArtifact;
									const loweredModule = (await WebAssembly.instantiate(
				...
				setPhase('verify', 'failed', 'error');
				appendLog(
					`artifact probe failed: ${error instanceof Error ? error.message : String(error)}`,
					'error'
				);
			}
			...
			let executionArtifactReason:
				| 'bootstrap-artifact'
				| 'missing-wasi-entrypoint'
				| undefined = lastBuildArtifactReason;
			...
			if (
				...
				if (hostCompileResponse.ok) {
					...
					const hostArtifactBytes = Uint8Array.from(atob(artifactBase64), (char) =>
						char.charCodeAt(0)
					);
					const hostArtifactExportNames = WebAssembly.Module.exports(
						new WebAssembly.Module(hostArtifactBytes)
					...
					if (hostArtifactEntrypoint === null) {
						throw new Error(
							'TinyGo host compile returned a wasm artifact without a supported entrypoint'
						);
					}
			...
			if (
				...
				const executionLinkResult = await runtime._run_process_impl(executionLinkCommand, {
					cwd: '/working'
				});
				if (executionLinkResult.returncode !== 0) {
					...
					throw new Error(
						`execution artifact link failed with exit code ${executionLinkResult.returncode}`
					);
				}
				const relinkedExecutionArtifact = await fileSystem.readFile(
					relinkedExecutionArtifactPath
				);
				const relinkedExecutionArtifactBytes =
					typeof relinkedExecutionArtifact === 'string'
						? textEncoder.encode(relinkedExecutionArtifact)
						: relinkedExecutionArtifact;
				...
				const relinkedExecutionArtifactEntrypoint = executionArtifactExportNames.includes(
					'_start'
				)
					? '_start'
					: executionArtifactExportNames.includes('main') &&
						  executionArtifactExportNames.includes('_initialize')
						? '_initialize'
						: executionArtifactExportNames.includes('main')
							? 'main'
							: null;
				if (relinkedExecutionArtifactEntrypoint === null) {
					throw new Error(
						'execution artifact did not expose a supported WASI entrypoint'
					);
				}
			...
			if (
				...
				setPhase(
					'smoke',
					`${executionArtifactBytes.byteLength.toLocaleString()} bytes`,
					'success'
				);
				appendLog(
				...
				appendLog(
					`execution artifact entrypoint=${executionArtifactEntrypoint}`,
					'success'
				);
				const executionStdout = ConsoleStdout.lineBuffered((line) =>
					appendLog(line, 'success')
				);
				const executionStderr = ConsoleStdout.lineBuffered((line) =>
					appendLog(line, 'error')
				);
				...
				const { instance: executionInstance } = await WebAssembly.instantiate(
					executionArtifactBytes as BufferSource,
					{
				...
				try {
					if (executionArtifactEntrypoint === '_start') {
						...
						if (
							typeof executionExports._initialize !== 'function' &&
							typeof executionExports.__wasm_call_ctors === 'function'
						) {
...
export const createTinyGoBrowserRuntime = (
	options: TinyGoBrowserRuntimeOptions
): TinyGoBrowserRuntime => {
