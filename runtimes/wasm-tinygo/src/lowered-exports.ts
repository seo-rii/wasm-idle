import type {
	TinyGoLoweredArtifactManifest,
	TinyGoLoweredIRManifest,
	TinyGoLoweredSourcesManifest
} from './compile-unit';
...
export const verifyTinyGoLoweredArtifactExports = (
	...
	for (const unit of loweredSourcesManifest.units) {
		for (const sourceFile of unit.sourceFiles ?? []) {
			...
			while (true) {
				...
				for (
					let blankIndex = bodyStartIndex + 1;
					blankIndex < bodyEndIndex - 1;
					blankIndex += 1
				) {
	...
	for (const unit of loweredSourcesManifest.units) {
		...
		const loweredIRUnit = (loweredIRManifest?.units ?? []).find(
			(loweredUnit) => (loweredUnit.id ?? '') === unit.id
		);
		...
		if (!Number.isInteger(sourceFileCount) || sourceFileCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid source file count for ${exportName}`
			);
		}
		...
		if (!Number.isInteger(kindTag) || kindTag < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid kind tag for ${kindExportName}`
			);
		}
		const expectedKindTag =
			unit.kind === 'program'
				? 1
				: unit.kind === 'imported'
					? 2
		...
		if (typeof sourceHashExportValue !== 'function') {
			throw new Error(
				`frontend lowered artifact probe missing export ${sourceHashExportName}`
			);
		}
		const sourceHash = Number(sourceHashExportValue());
		if (!Number.isInteger(sourceHash) || sourceHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid source hash for ${sourceHashExportName}`
			);
		}
		...
		if (typeof importCountExportValue !== 'function') {
			throw new Error(
				`frontend lowered artifact probe missing export ${importCountExportName}`
			);
		}
		const importCount = Number(importCountExportValue());
		if (!Number.isInteger(importCount) || importCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid import count for ${importCountExportName}`
			);
		}
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(/^\s*import\s*\(([\s\S]*?)^\s*\)/gm)) {
				for (const importMatch of match[1].matchAll(
					/^\s*(?:(_|\.|[A-Za-z_]\w*)\s+)?"([^"]+)"/gm
				)) {
					if (importMatch[1] === '_') {
						expectedBlankImportCount += 1;
			...
			for (const importPath of importPaths) {
				for (const byte of textEncoder.encode(importPath)) {
					expectedImportPathHash =
						(expectedImportPathHash + byte * importPathHashPosition) >>> 0;
					importPathHashPosition += 1;
				}
				expectedImportPathHash =
					(expectedImportPathHash + 0x0a * importPathHashPosition) >>> 0;
				importPathHashPosition += 1;
			}
		}
		if (importCount !== expectedImportCount) {
			throw new Error(
				'frontend lowered artifact probe import count did not match lowered sources manifest'
			);
		}
		...
		if (typeof importPathHashExportValue !== 'function') {
			throw new Error(
				`frontend lowered artifact probe missing export ${importPathHashExportName}`
			);
		}
		const importPathHash = Number(importPathHashExportValue());
		if (!Number.isInteger(importPathHash) || importPathHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid import path hash for ${importPathHashExportName}`
			);
		}
		...
		const blankImportCount =
			typeof blankImportCountExportValue === 'function'
				? Number(blankImportCountExportValue())
				: expectedBlankImportCount;
		if (!Number.isInteger(blankImportCount) || blankImportCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid blank import count for ${blankImportCountExportName}`
			);
		}
		if (
			typeof blankImportCountExportValue === 'function' &&
			blankImportCount !== expectedBlankImportCount
		) {
			throw new Error(
		...
		const dotImportCount =
			typeof dotImportCountExportValue === 'function'
				? Number(dotImportCountExportValue())
				: expectedDotImportCount;
		if (!Number.isInteger(dotImportCount) || dotImportCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid dot import count for ${dotImportCountExportName}`
			);
		}
		if (
			typeof dotImportCountExportValue === 'function' &&
			dotImportCount !== expectedDotImportCount
		) {
			throw new Error(
		...
		const aliasedImportCount =
			typeof aliasedImportCountExportValue === 'function'
				? Number(aliasedImportCountExportValue())
				: expectedAliasedImportCount;
		if (!Number.isInteger(aliasedImportCount) || aliasedImportCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid aliased import count for ${aliasedImportCountExportName}`
			);
		}
		if (
			typeof aliasedImportCountExportValue === 'function' &&
			aliasedImportCount !== expectedAliasedImportCount
		) {
			throw new Error(
		...
		if (typeof functionCountExportValue !== 'function') {
			throw new Error(
				`frontend lowered artifact probe missing export ${functionCountExportName}`
			);
		}
		const functionCount = Number(functionCountExportValue());
		if (!Number.isInteger(functionCount) || functionCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid function count for ${functionCountExportName}`
			);
		}
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			expectedFunctionCount += (sourceText.match(/^\s*func(?:\s*\(|\s+[A-Za-z_])/gm) ?? [])
				.length;
		}
		...
		const expectedPlaceholderBlockCount = Array.isArray(loweredIRUnit?.placeholderBlocks)
			...
			: expectedImportCount +
				...
				(loweredIRUnit?.declarations ?? []).length;
		const placeholderBlockCount =
			typeof placeholderBlockCountExportValue === 'function'
				? Number(placeholderBlockCountExportValue())
				: expectedPlaceholderBlockCount;
		if (!Number.isInteger(placeholderBlockCount) || placeholderBlockCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid placeholder block count for ${placeholderBlockCountExportName}`
			);
		}
		if (
			typeof placeholderBlockCountExportValue === 'function' &&
			placeholderBlockCount !== expectedPlaceholderBlockCount
		) {
			throw new Error(
			...
			for (const placeholderBlock of loweredIRUnit?.placeholderBlocks ?? []) {
				for (const byte of textEncoder.encode(placeholderBlock.value ?? '')) {
					expectedPlaceholderBlockHash =
						(expectedPlaceholderBlockHash + byte * placeholderBlockHashPosition) >>> 0;
					placeholderBlockHashPosition += 1;
				}
				expectedPlaceholderBlockHash =
					(expectedPlaceholderBlockHash + 0x0a * placeholderBlockHashPosition) >>> 0;
				placeholderBlockHashPosition += 1;
			}
		...
		const placeholderBlockHash =
			typeof placeholderBlockHashExportValue === 'function'
				? Number(placeholderBlockHashExportValue())
				: expectedPlaceholderBlockHash;
		if (!Number.isInteger(placeholderBlockHash) || placeholderBlockHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid placeholder block hash for ${placeholderBlockHashExportName}`
			);
		}
		if (
			typeof placeholderBlockHashExportValue === 'function' &&
			loweredIRUnit &&
			placeholderBlockHash !== expectedPlaceholderBlockHash
		) {
			throw new Error(
		...
		const placeholderBlockSignatureHashExportValue =
			exportsObject[placeholderBlockSignatureHashExportName];
		...
		if (loweredIRUnit) {
			...
			if (
				...
				loweredIRUnit.placeholderBlocks.every(
					(placeholderBlock) => typeof placeholderBlock?.signature === 'string'
				)
			) {
				for (const placeholderBlock of loweredIRUnit.placeholderBlocks ?? []) {
					for (const byte of textEncoder.encode(placeholderBlock.signature ?? '')) {
						expectedPlaceholderBlockSignatureHash =
							(expectedPlaceholderBlockSignatureHash +
								byte * placeholderBlockSignatureHashPosition) >>>
							0;
						placeholderBlockSignatureHashPosition += 1;
					}
					expectedPlaceholderBlockSignatureHash =
						(expectedPlaceholderBlockSignatureHash +
							0x0a * placeholderBlockSignatureHashPosition) >>>
						0;
					placeholderBlockSignatureHashPosition += 1;
				}
			} else {
		...
		if (!Number.isInteger(placeholderBlockSignatureHash) || placeholderBlockSignatureHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid placeholder block signature hash for ${placeholderBlockSignatureHashExportName}`
			);
		}
		if (
			typeof placeholderBlockSignatureHashExportValue === 'function' &&
		...
		const placeholderBlockRuntimeHashExportValue =
			exportsObject[placeholderBlockRuntimeHashExportName];
		...
		if (!Number.isInteger(placeholderBlockRuntimeHash) || placeholderBlockRuntimeHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid placeholder block runtime hash for ${placeholderBlockRuntimeHashExportName}`
			);
		}
		if (
			typeof placeholderBlockRuntimeHashExportValue === 'function' &&
			loweredIRUnit &&
			placeholderBlockRuntimeHash !== expectedPlaceholderBlockHash
		) {
			throw new Error(
		...
		const loweringBlockCount =
			typeof loweringBlockCountExportValue === 'function'
				? Number(loweringBlockCountExportValue())
				: expectedLoweringBlockCount;
		if (!Number.isInteger(loweringBlockCount) || loweringBlockCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid lowering block count for ${loweringBlockCountExportName}`
			);
		}
		if (
			typeof loweringBlockCountExportValue === 'function' &&
			loweringBlockCount !== expectedLoweringBlockCount
		) {
			throw new Error(
		...
		if (loweredIRUnit) {
			...
			const expectedLoweringBlocks = Array.isArray(loweredIRUnit.loweringBlocks)
				...
				: [
						...(loweredIRUnit.imports ?? []).map(
							(loweredImport, loweredImportIndex) =>
						...
						...(loweredIRUnit.functions ?? []).map(
							(loweredFunction, loweredFunctionIndex) =>
								`tinygo_lower_unit_begin("${loweredIRUnit.id ?? ''}", "${loweredIRUnit.kind ?? ''}", "${loweredIRUnit.packageName ?? ''}", ${(loweredIRUnit.sourceFiles ?? []).length});` +
						...
						...(loweredIRUnit.declarations ?? []).map(
			...
			for (const loweringBlock of expectedLoweringBlocks) {
				for (const byte of textEncoder.encode(loweringBlock)) {
					expectedLoweringBlockHash =
						(expectedLoweringBlockHash + byte * loweringBlockHashPosition) >>> 0;
					loweringBlockHashPosition += 1;
				}
				expectedLoweringBlockHash =
					(expectedLoweringBlockHash + 0x0a * loweringBlockHashPosition) >>> 0;
				loweringBlockHashPosition += 1;
			}
		}
		const loweringBlockHash =
			typeof loweringBlockHashExportValue === 'function'
				? Number(loweringBlockHashExportValue())
				: expectedLoweringBlockHash;
		if (!Number.isInteger(loweringBlockHash) || loweringBlockHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid lowering block hash for ${loweringBlockHashExportName}`
			);
		}
		if (
			typeof loweringBlockHashExportValue === 'function' &&
			loweredIRUnit &&
			loweringBlockHash !== expectedLoweringBlockHash
		) {
			throw new Error(
		...
		const loweringBlockRuntimeHashExportValue =
			exportsObject[loweringBlockRuntimeHashExportName];
		...
		if (!Number.isInteger(loweringBlockRuntimeHash) || loweringBlockRuntimeHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid lowering block runtime hash for ${loweringBlockRuntimeHashExportName}`
			);
		}
		if (
			typeof loweringBlockRuntimeHashExportValue === 'function' &&
			loweredIRUnit &&
			loweringBlockRuntimeHash !== expectedLoweringBlockHash
		) {
			throw new Error(
		...
		const funcLiteralCount =
			typeof funcLiteralCountExportValue === 'function'
				? Number(funcLiteralCountExportValue())
				: expectedFuncLiteralCount;
		if (!Number.isInteger(funcLiteralCount) || funcLiteralCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid func literal count for ${funcLiteralCountExportName}`
			);
		}
		if (
			typeof funcLiteralCountExportValue === 'function' &&
			funcLiteralCount !== expectedFuncLiteralCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(
				/\bfunc(?:\s*\([^)]*\))?\s+[A-Za-z_]\w*(?:\s*\[[^\]]+\])?\s*\(([^)]*)\)\s*([^{\n]*)\{/g
			)) {
				const paramsText = match[1].trim();
				if (paramsText !== '') {
					expectedFuncParameterCount += paramsText
						.split(',')
						.map((part) => part.trim())
				...
				if (resultsText.startsWith('(') && resultsText.endsWith(')')) {
					...
					if (innerResultsText !== '') {
						expectedFuncResultCount += innerResultsText
							.split(',')
							.map((part) => part.trim())
							.filter((part) => part !== '').length;
					}
		...
		const funcParameterCount =
			typeof funcParameterCountExportValue === 'function'
				? Number(funcParameterCountExportValue())
				: expectedFuncParameterCount;
		if (!Number.isInteger(funcParameterCount) || funcParameterCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid func parameter count for ${funcParameterCountExportName}`
			);
		}
		if (
			typeof funcParameterCountExportValue === 'function' &&
			funcParameterCount !== expectedFuncParameterCount
		) {
			throw new Error(
				'frontend lowered artifact probe func parameter count did not match lowered sources manifest'
			);
		}
		const funcResultCount =
			typeof funcResultCountExportValue === 'function'
				? Number(funcResultCountExportValue())
				: expectedFuncResultCount;
		if (!Number.isInteger(funcResultCount) || funcResultCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid func result count for ${funcResultCountExportName}`
			);
		}
		if (
			typeof funcResultCountExportValue === 'function' &&
			funcResultCount !== expectedFuncResultCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(
				...
				for (const resultPart of innerResultsText
					.split(',')
					.map((part) => part.trim())
					.filter((part) => part !== '')) {
		...
		const variadicParameterCount =
			typeof variadicParameterCountExportValue === 'function'
				? Number(variadicParameterCountExportValue())
				: expectedVariadicParameterCount;
		if (!Number.isInteger(variadicParameterCount) || variadicParameterCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid variadic parameter count for ${variadicParameterCountExportName}`
			);
		}
		if (
			typeof variadicParameterCountExportValue === 'function' &&
			variadicParameterCount !== expectedVariadicParameterCount
		) {
			throw new Error(
		...
		const namedResultCount =
			typeof namedResultCountExportValue === 'function'
				? Number(namedResultCountExportValue())
				: expectedNamedResultCount;
		if (!Number.isInteger(namedResultCount) || namedResultCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid named result count for ${namedResultCountExportName}`
			);
		}
		if (
			typeof namedResultCountExportValue === 'function' &&
			namedResultCount !== expectedNamedResultCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(
				...
				expectedTypeParameterCount += match[1]
					.split(',')
					.map((part) => part.trim())
					.filter((part) => part !== '').length;
			}
		...
		const typeParameterCount =
			typeof typeParameterCountExportValue === 'function'
				? Number(typeParameterCountExportValue())
				: expectedTypeParameterCount;
		if (!Number.isInteger(typeParameterCount) || typeParameterCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid type parameter count for ${typeParameterCountExportName}`
			);
		}
		if (
			typeof typeParameterCountExportValue === 'function' &&
			typeParameterCount !== expectedTypeParameterCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			expectedGenericFunctionCount += (
				sourceText.match(/^\s*func(?:\s*\([^)]*\))?\s+[A-Za-z_]\w*\s*\[[^\]]+\]\s*\(/gm) ??
				[]
			).length;
		}
		const genericFunctionCount =
			typeof genericFunctionCountExportValue === 'function'
				? Number(genericFunctionCountExportValue())
				: expectedGenericFunctionCount;
		if (!Number.isInteger(genericFunctionCount) || genericFunctionCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid generic function count for ${genericFunctionCountExportName}`
			);
		}
		if (
			typeof genericFunctionCountExportValue === 'function' &&
			genericFunctionCount !== expectedGenericFunctionCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			expectedGenericTypeCount += (
				sourceText.match(/^\s*type\s+[A-Za-z_]\w*\s*\[[^\]]+\]/gm) ?? []
			).length;
		}
		const genericTypeCount =
			typeof genericTypeCountExportValue === 'function'
				? Number(genericTypeCountExportValue())
				: expectedGenericTypeCount;
		if (!Number.isInteger(genericTypeCount) || genericTypeCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid generic type count for ${genericTypeCountExportName}`
			);
		}
		if (
			typeof genericTypeCountExportValue === 'function' &&
			genericTypeCount !== expectedGenericTypeCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			const normalizedSourceText = sourceText
				...
				.replace(/"(?:\\.|[^"\\])*"/g, '""')
				.replace(/`[\s\S]*?`/g, '``');
			const callSearchSourceText = normalizedSourceText.replace(
				/\binterface\s*\{[\s\S]*?\}/g,
				'interface{}'
			);
			expectedCallExpressionCount +=
				[
					...callSearchSourceText.matchAll(/\b([A-Za-z_]\w*(?:\.[A-Za-z_]\w*)?)\s*\(/g)
				].filter((match) => match[1] !== 'func').length -
				(
					callSearchSourceText.match(
						/^\s*func(?:\s*\([^)]*\))?\s+[A-Za-z_]\w*(?:\s*\[[^\]]+\])?\s*\(/gm
					) ?? []
				).length;
		}
		const callExpressionCount =
			typeof callExpressionCountExportValue === 'function'
				? Number(callExpressionCountExportValue())
				: expectedCallExpressionCount;
		if (!Number.isInteger(callExpressionCount) || callExpressionCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid call expression count for ${callExpressionCountExportName}`
			);
		}
		if (
			typeof callExpressionCountExportValue === 'function' &&
			callExpressionCount !== expectedCallExpressionCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			expectedBuiltinCallCount += (
				sourceText.match(
					/\b(?:append|cap|clear|close|complex|copy|delete|imag|len|make|max|min|new|panic|print|println|real|recover)\s*\(/g
				) ?? []
			).length;
		}
		const builtinCallCount =
			typeof builtinCallCountExportValue === 'function'
				? Number(builtinCallCountExportValue())
				: expectedBuiltinCallCount;
		if (!Number.isInteger(builtinCallCount) || builtinCallCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid builtin call count for ${builtinCallCountExportName}`
			);
		}
		if (
			typeof builtinCallCountExportValue === 'function' &&
			builtinCallCount !== expectedBuiltinCallCount
		) {
			throw new Error(
		...
		const appendCallCount =
			typeof appendCallCountExportValue === 'function'
				? Number(appendCallCountExportValue())
				: expectedAppendCallCount;
		if (!Number.isInteger(appendCallCount) || appendCallCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid append call count for ${appendCallCountExportName}`
			);
		}
		if (
			typeof appendCallCountExportValue === 'function' &&
			appendCallCount !== expectedAppendCallCount
		) {
			throw new Error(
		...
		const lenCallCount =
			typeof lenCallCountExportValue === 'function'
				? Number(lenCallCountExportValue())
				: expectedLenCallCount;
		if (!Number.isInteger(lenCallCount) || lenCallCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid len call count for ${lenCallCountExportName}`
			);
		}
		if (
			typeof lenCallCountExportValue === 'function' &&
			lenCallCount !== expectedLenCallCount
		) {
			throw new Error(
		...
		const makeCallCount =
			typeof makeCallCountExportValue === 'function'
				? Number(makeCallCountExportValue())
				: expectedMakeCallCount;
		if (!Number.isInteger(makeCallCount) || makeCallCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid make call count for ${makeCallCountExportName}`
			);
		}
		if (
			typeof makeCallCountExportValue === 'function' &&
			makeCallCount !== expectedMakeCallCount
		) {
			throw new Error(
		...
		const capCallCount =
			typeof capCallCountExportValue === 'function'
				? Number(capCallCountExportValue())
				: expectedCapCallCount;
		if (!Number.isInteger(capCallCount) || capCallCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid cap call count for ${capCallCountExportName}`
			);
		}
		if (
			typeof capCallCountExportValue === 'function' &&
			capCallCount !== expectedCapCallCount
		) {
			throw new Error(
		...
		const copyCallCount =
			typeof copyCallCountExportValue === 'function'
				? Number(copyCallCountExportValue())
				: expectedCopyCallCount;
		if (!Number.isInteger(copyCallCount) || copyCallCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid copy call count for ${copyCallCountExportName}`
			);
		}
		if (
			typeof copyCallCountExportValue === 'function' &&
			copyCallCount !== expectedCopyCallCount
		) {
			throw new Error(
		...
		const panicCallCount =
			typeof panicCallCountExportValue === 'function'
				? Number(panicCallCountExportValue())
				: expectedPanicCallCount;
		if (!Number.isInteger(panicCallCount) || panicCallCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid panic call count for ${panicCallCountExportName}`
			);
		}
		if (
			typeof panicCallCountExportValue === 'function' &&
			panicCallCount !== expectedPanicCallCount
		) {
			throw new Error(
		...
		const recoverCallCount =
			typeof recoverCallCountExportValue === 'function'
				? Number(recoverCallCountExportValue())
				: expectedRecoverCallCount;
		if (!Number.isInteger(recoverCallCount) || recoverCallCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid recover call count for ${recoverCallCountExportName}`
			);
		}
		if (
			typeof recoverCallCountExportValue === 'function' &&
			recoverCallCount !== expectedRecoverCallCount
		) {
			throw new Error(
		...
		const newCallCount =
			typeof newCallCountExportValue === 'function'
				? Number(newCallCountExportValue())
				: expectedNewCallCount;
		if (!Number.isInteger(newCallCount) || newCallCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid new call count for ${newCallCountExportName}`
			);
		}
		if (
			typeof newCallCountExportValue === 'function' &&
			newCallCount !== expectedNewCallCount
		) {
			throw new Error(
		...
		const deleteCallCount =
			typeof deleteCallCountExportValue === 'function'
				? Number(deleteCallCountExportValue())
				: expectedDeleteCallCount;
		if (!Number.isInteger(deleteCallCount) || deleteCallCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid delete call count for ${deleteCallCountExportName}`
			);
		}
		if (
			typeof deleteCallCountExportValue === 'function' &&
			deleteCallCount !== expectedDeleteCallCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			expectedCompositeLiteralCount += (
				normalizedSourceText.match(
					/(?:=|:=|return\s+|,|\()\s*(?:\[\])?(?:[A-Za-z_]\w*(?:\s*\.\s*[A-Za-z_]\w*)?)\s*\{/g
				) ?? []
			).length;
			expectedCompositeLiteralCount += (
				normalizedSourceText.match(
					/(?:=|:=|return\s+|,|\()\s*map\s*\[[^\]]+\]\s*(?:[A-Za-z_]\w*(?:\s*\.\s*[A-Za-z_]\w*)?)\s*\{/g
				) ?? []
			).length;
		}
		const compositeLiteralCount =
			typeof compositeLiteralCountExportValue === 'function'
				? Number(compositeLiteralCountExportValue())
				: expectedCompositeLiteralCount;
		if (!Number.isInteger(compositeLiteralCount) || compositeLiteralCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid composite literal count for ${compositeLiteralCountExportName}`
			);
		}
		if (
			typeof compositeLiteralCountExportValue === 'function' &&
			compositeLiteralCount !== expectedCompositeLiteralCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			const normalizedSourceText = sourceText
				...
				.replace(/"(?:\\.|[^"\\])*"/g, '""')
				.replace(/`[\s\S]*?`/g, '``');
			expectedSelectorExpressionCount += (
				normalizedSourceText.match(/\b[A-Za-z_]\w*\s*\.\s*[A-Za-z_]\w*\b/g) ?? []
			).length;
		}
		const selectorExpressionCount =
			typeof selectorExpressionCountExportValue === 'function'
				? Number(selectorExpressionCountExportValue())
				: expectedSelectorExpressionCount;
		if (!Number.isInteger(selectorExpressionCount) || selectorExpressionCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid selector expression count for ${selectorExpressionCountExportName}`
			);
		}
		if (
			typeof selectorExpressionCountExportValue === 'function' &&
			selectorExpressionCount !== expectedSelectorExpressionCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of normalizedSourceText.matchAll(
				...
				for (const byte of textEncoder.encode(match[1])) {
					expectedSelectorNameHash =
						(expectedSelectorNameHash + byte * expectedSelectorNameHashPosition) >>> 0;
					expectedSelectorNameHashPosition += 1;
				}
				expectedSelectorNameHash =
					(expectedSelectorNameHash + 0x0a * expectedSelectorNameHashPosition) >>> 0;
				expectedSelectorNameHashPosition += 1;
			}
		}
		if (typeof selectorNameHashExportValue !== 'function') {
			if (expectedSelectorNameHash !== 0) {
				throw new Error(
					`frontend lowered artifact probe missing export ${selectorNameHashExportName}`
				);
			}
		}
		const selectorNameHash =
			typeof selectorNameHashExportValue === 'function'
				? Number(selectorNameHashExportValue())
				: 0;
		if (!Number.isInteger(selectorNameHash) || selectorNameHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid selector name hash for ${selectorNameHashExportName}`
			);
		}
		...
		const indexExpressionCount =
			typeof indexExpressionCountExportValue === 'function'
				? Number(indexExpressionCountExportValue())
				: expectedIndexExpressionCount;
		if (!Number.isInteger(indexExpressionCount) || indexExpressionCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid index expression count for ${indexExpressionCountExportName}`
			);
		}
		if (
			typeof indexExpressionCountExportValue === 'function' &&
			indexExpressionCount !== expectedIndexExpressionCount
		) {
			throw new Error(
		...
		const sliceExpressionCount =
			typeof sliceExpressionCountExportValue === 'function'
				? Number(sliceExpressionCountExportValue())
				: expectedSliceExpressionCount;
		if (!Number.isInteger(sliceExpressionCount) || sliceExpressionCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid slice expression count for ${sliceExpressionCountExportName}`
			);
		}
		if (
			typeof sliceExpressionCountExportValue === 'function' &&
			sliceExpressionCount !== expectedSliceExpressionCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of [
				...
				expectedKeyValueExpressionCount += (match[1].match(/\b[A-Za-z_]\w*\s*:/g) ?? [])
					.length;
			}
		}
		const keyValueExpressionCount =
			typeof keyValueExpressionCountExportValue === 'function'
				? Number(keyValueExpressionCountExportValue())
				: expectedKeyValueExpressionCount;
		if (!Number.isInteger(keyValueExpressionCount) || keyValueExpressionCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid key value expression count for ${keyValueExpressionCountExportName}`
			);
		}
		if (
			typeof keyValueExpressionCountExportValue === 'function' &&
			keyValueExpressionCount !== expectedKeyValueExpressionCount
		) {
			throw new Error(
		...
		const typeAssertionCount =
			typeof typeAssertionCountExportValue === 'function'
				? Number(typeAssertionCountExportValue())
				: expectedTypeAssertionCount;
		if (!Number.isInteger(typeAssertionCount) || typeAssertionCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid type assertion count for ${typeAssertionCountExportName}`
			);
		}
		if (
			typeof typeAssertionCountExportValue === 'function' &&
			typeAssertionCount !== expectedTypeAssertionCount
		) {
			throw new Error(
		...
		const blankIdentifierCount =
			typeof blankIdentifierCountExportValue === 'function'
				? Number(blankIdentifierCountExportValue())
				: expectedBlankIdentifierCount;
		if (!Number.isInteger(blankIdentifierCount) || blankIdentifierCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid blank identifier count for ${blankIdentifierCountExportName}`
			);
		}
		if (
			typeof blankIdentifierCountExportValue === 'function' &&
			blankIdentifierCount !== expectedBlankIdentifierCount
		) {
			throw new Error(
		...
		const blankAssignmentTargetCountExportValue =
			exportsObject[blankAssignmentTargetCountExportName];
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			const normalizedSourceText = sourceText
				...
				.replace(/"(?:\\.|[^"\\])*"/g, '""')
				.replace(/`[\s\S]*?`/g, '``');
			expectedBlankAssignmentTargetCount += (
				normalizedSourceText.match(/(?:^|[(,;{])\s*_\s*(?::=|=)/gm) ?? []
			).length;
		}
		const blankAssignmentTargetCount =
			typeof blankAssignmentTargetCountExportValue === 'function'
				? Number(blankAssignmentTargetCountExportValue())
				: expectedBlankAssignmentTargetCount;
		if (!Number.isInteger(blankAssignmentTargetCount) || blankAssignmentTargetCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid blank assignment target count for ${blankAssignmentTargetCountExportName}`
			);
		}
		if (
			typeof blankAssignmentTargetCountExportValue === 'function' &&
			blankAssignmentTargetCount !== expectedBlankAssignmentTargetCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			expectedUnaryExpressionCount += (
				normalizedSourceText.match(
					/(?:^|[=(:,{\[]\s*|return\s+|case\s+)(?:-(?!-)|\+(?!\+)|!|\^|\*|&)\s*(?:[A-Za-z_]\w*|\d+|\()/gm
				) ?? []
			).length;
		}
		const unaryExpressionCount =
			typeof unaryExpressionCountExportValue === 'function'
				? Number(unaryExpressionCountExportValue())
				: expectedUnaryExpressionCount;
		if (!Number.isInteger(unaryExpressionCount) || unaryExpressionCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid unary expression count for ${unaryExpressionCountExportName}`
			);
		}
		if (
			typeof unaryExpressionCountExportValue === 'function' &&
			unaryExpressionCount !== expectedUnaryExpressionCount
		) {
			throw new Error(
		...
		const binaryExpressionCount =
			typeof binaryExpressionCountExportValue === 'function'
				? Number(binaryExpressionCountExportValue())
				: expectedBinaryExpressionCount;
		if (!Number.isInteger(binaryExpressionCount) || binaryExpressionCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid binary expression count for ${binaryExpressionCountExportName}`
			);
		}
		if (
			typeof binaryExpressionCountExportValue === 'function' &&
			binaryExpressionCount !== expectedBinaryExpressionCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			expectedSendStatementCount += (
				sourceText.match(/\b(?!case\b)[A-Za-z_]\w*\s*<-\s*/g) ?? []
			).length;
		}
		const sendStatementCount =
			typeof sendStatementCountExportValue === 'function'
				? Number(sendStatementCountExportValue())
				: expectedSendStatementCount;
		if (!Number.isInteger(sendStatementCount) || sendStatementCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid send statement count for ${sendStatementCountExportName}`
			);
		}
		if (
			typeof sendStatementCountExportValue === 'function' &&
			sendStatementCount !== expectedSendStatementCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			expectedReceiveExpressionCount -= (
				sourceText.match(/\b(?!case\b)[A-Za-z_]\w*\s*<-\s*/g) ?? []
			).length;
		}
		const receiveExpressionCount =
			typeof receiveExpressionCountExportValue === 'function'
				? Number(receiveExpressionCountExportValue())
				: expectedReceiveExpressionCount;
		if (!Number.isInteger(receiveExpressionCount) || receiveExpressionCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid receive expression count for ${receiveExpressionCountExportName}`
			);
		}
		if (
			typeof receiveExpressionCountExportValue === 'function' &&
			receiveExpressionCount !== expectedReceiveExpressionCount
		) {
			throw new Error(
		...
		const defineStatementCount =
			typeof defineStatementCountExportValue === 'function'
				? Number(defineStatementCountExportValue())
				: expectedDefineStatementCount;
		if (!Number.isInteger(defineStatementCount) || defineStatementCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid define statement count for ${defineStatementCountExportName}`
			);
		}
		if (
			typeof defineStatementCountExportValue === 'function' &&
			defineStatementCount !== expectedDefineStatementCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			expectedAssignStatementCount += (
				sourceText.match(
					/(?<!\b(?:var|const)\s)\b[A-Za-z_]\w*(?:\s*,\s*[A-Za-z_]\w*)*\s*=(?!=)\s*/g
				) ?? []
			).length;
		}
		const assignStatementCount =
			typeof assignStatementCountExportValue === 'function'
				? Number(assignStatementCountExportValue())
				: expectedAssignStatementCount;
		if (!Number.isInteger(assignStatementCount) || assignStatementCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid assign statement count for ${assignStatementCountExportName}`
			);
		}
		if (
			typeof assignStatementCountExportValue === 'function' &&
			assignStatementCount !== expectedAssignStatementCount
		) {
			throw new Error(
		...
		const incStatementCount =
			typeof incStatementCountExportValue === 'function'
				? Number(incStatementCountExportValue())
				: expectedIncStatementCount;
		if (!Number.isInteger(incStatementCount) || incStatementCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid increment statement count for ${incStatementCountExportName}`
			);
		}
		if (
			typeof incStatementCountExportValue === 'function' &&
			incStatementCount !== expectedIncStatementCount
		) {
			throw new Error(
		...
		const decStatementCount =
			typeof decStatementCountExportValue === 'function'
				? Number(decStatementCountExportValue())
				: expectedDecStatementCount;
		if (!Number.isInteger(decStatementCount) || decStatementCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid decrement statement count for ${decStatementCountExportName}`
			);
		}
		if (
			typeof decStatementCountExportValue === 'function' &&
			decStatementCount !== expectedDecStatementCount
		) {
			throw new Error(
		...
		const returnStatementCount =
			typeof returnStatementCountExportValue === 'function'
				? Number(returnStatementCountExportValue())
				: expectedReturnStatementCount;
		if (!Number.isInteger(returnStatementCount) || returnStatementCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid return statement count for ${returnStatementCountExportName}`
			);
		}
		if (
			typeof returnStatementCountExportValue === 'function' &&
			returnStatementCount !== expectedReturnStatementCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(
				...
				expectedGoStatementCount += (
					match[1].match(/\bgo\s+[A-Za-z_]\w*(?:\.[A-Za-z_]\w*)?\s*\(/g) ?? []
				).length;
			}
		}
		const goStatementCount =
			typeof goStatementCountExportValue === 'function'
				? Number(goStatementCountExportValue())
				: expectedGoStatementCount;
		if (!Number.isInteger(goStatementCount) || goStatementCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid go statement count for ${goStatementCountExportName}`
			);
		}
		if (
			typeof goStatementCountExportValue === 'function' &&
			goStatementCount !== expectedGoStatementCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(
				...
				expectedDeferStatementCount += (
					match[1].match(/\bdefer\s+[A-Za-z_]\w*(?:\.[A-Za-z_]\w*)?\s*\(/g) ?? []
				).length;
			}
		}
		const deferStatementCount =
			typeof deferStatementCountExportValue === 'function'
				? Number(deferStatementCountExportValue())
				: expectedDeferStatementCount;
		if (!Number.isInteger(deferStatementCount) || deferStatementCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid defer statement count for ${deferStatementCountExportName}`
			);
		}
		if (
			typeof deferStatementCountExportValue === 'function' &&
			deferStatementCount !== expectedDeferStatementCount
		) {
			throw new Error(
		...
		const ifStatementCount =
			typeof ifStatementCountExportValue === 'function'
				? Number(ifStatementCountExportValue())
				: expectedIfStatementCount;
		if (!Number.isInteger(ifStatementCount) || ifStatementCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid if statement count for ${ifStatementCountExportName}`
			);
		}
		if (
			typeof ifStatementCountExportValue === 'function' &&
			ifStatementCount !== expectedIfStatementCount
		) {
			throw new Error(
		...
		const rangeStatementCount =
			typeof rangeStatementCountExportValue === 'function'
				? Number(rangeStatementCountExportValue())
				: expectedRangeStatementCount;
		if (!Number.isInteger(rangeStatementCount) || rangeStatementCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid range statement count for ${rangeStatementCountExportName}`
			);
		}
		if (
			typeof rangeStatementCountExportValue === 'function' &&
			rangeStatementCount !== expectedRangeStatementCount
		) {
			throw new Error(
		...
		const switchStatementCount =
			typeof switchStatementCountExportValue === 'function'
				? Number(switchStatementCountExportValue())
				: expectedSwitchStatementCount;
		if (!Number.isInteger(switchStatementCount) || switchStatementCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid switch statement count for ${switchStatementCountExportName}`
			);
		}
		if (
			typeof switchStatementCountExportValue === 'function' &&
			switchStatementCount !== expectedSwitchStatementCount
		) {
			throw new Error(
		...
		const typeSwitchStatementCountExportValue =
			exportsObject[typeSwitchStatementCountExportName];
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			expectedTypeSwitchStatementCount += (
				sourceText.match(/\bswitch\s+[^{}]*\.\(\s*type\s*\)/g) ?? []
			).length;
		}
		const typeSwitchStatementCount =
			typeof typeSwitchStatementCountExportValue === 'function'
				? Number(typeSwitchStatementCountExportValue())
				: expectedTypeSwitchStatementCount;
		if (!Number.isInteger(typeSwitchStatementCount) || typeSwitchStatementCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid type switch statement count for ${typeSwitchStatementCountExportName}`
			);
		}
		if (
			typeof typeSwitchStatementCountExportValue === 'function' &&
			typeSwitchStatementCount !== expectedTypeSwitchStatementCount
		) {
			throw new Error(
		...
		const typeSwitchCaseClauseCountExportValue =
			exportsObject[typeSwitchCaseClauseCountExportName];
		...
		const typeSwitchCaseClauseCount =
			typeof typeSwitchCaseClauseCountExportValue === 'function'
				? Number(typeSwitchCaseClauseCountExportValue())
				: expectedTypeSwitchCaseClauseCount;
		if (!Number.isInteger(typeSwitchCaseClauseCount) || typeSwitchCaseClauseCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid type switch case clause count for ${typeSwitchCaseClauseCountExportName}`
			);
		}
		if (
			typeof typeSwitchCaseClauseCountExportValue === 'function' &&
			typeSwitchCaseClauseCount !== expectedTypeSwitchCaseClauseCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(
				/\bswitch\s+([A-Za-z_]\w*)\s*:=\s*[^{}]*\.\(\s*type\s*\)/g
			)) {
				for (const byte of textEncoder.encode(match[1])) {
					expectedTypeSwitchGuardNameHash =
						(expectedTypeSwitchGuardNameHash +
							byte * expectedTypeSwitchGuardNameHashPosition) >>>
						0;
					expectedTypeSwitchGuardNameHashPosition += 1;
				}
				expectedTypeSwitchGuardNameHash =
					(expectedTypeSwitchGuardNameHash +
						0x0a * expectedTypeSwitchGuardNameHashPosition) >>>
					0;
				expectedTypeSwitchGuardNameHashPosition += 1;
			}
		}
		const typeSwitchGuardNameHash =
			typeof typeSwitchGuardNameHashExportValue === 'function'
				? Number(typeSwitchGuardNameHashExportValue())
				: expectedTypeSwitchGuardNameHash;
		if (!Number.isInteger(typeSwitchGuardNameHash) || typeSwitchGuardNameHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid type switch guard name hash for ${typeSwitchGuardNameHashExportName}`
			);
		}
		if (
			typeof typeSwitchGuardNameHashExportValue === 'function' &&
			typeSwitchGuardNameHash !== expectedTypeSwitchGuardNameHash
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(
				...
				for (const caseMatch of match[1].matchAll(/\bcase\s+([^:]+):/g)) {
					for (const typeExpression of caseMatch[1]
						.split(',')
						.map((part) => part.trim())
						.filter((part) => part !== '')) {
						for (const byte of textEncoder.encode(typeExpression)) {
							expectedTypeSwitchCaseTypeHash =
								(expectedTypeSwitchCaseTypeHash +
									byte * expectedTypeSwitchCaseTypeHashPosition) >>>
								0;
							expectedTypeSwitchCaseTypeHashPosition += 1;
						}
						expectedTypeSwitchCaseTypeHash =
							(expectedTypeSwitchCaseTypeHash +
								0x0a * expectedTypeSwitchCaseTypeHashPosition) >>>
							0;
						expectedTypeSwitchCaseTypeHashPosition += 1;
					}
				}
		...
		const typeSwitchCaseTypeHash =
			typeof typeSwitchCaseTypeHashExportValue === 'function'
				? Number(typeSwitchCaseTypeHashExportValue())
				: expectedTypeSwitchCaseTypeHash;
		if (!Number.isInteger(typeSwitchCaseTypeHash) || typeSwitchCaseTypeHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid type switch case type hash for ${typeSwitchCaseTypeHashExportName}`
			);
		}
		if (
			typeof typeSwitchCaseTypeHashExportValue === 'function' &&
			typeSwitchCaseTypeHash !== expectedTypeSwitchCaseTypeHash
		) {
			throw new Error(
		...
		const selectStatementCount =
			typeof selectStatementCountExportValue === 'function'
				? Number(selectStatementCountExportValue())
				: expectedSelectStatementCount;
		if (!Number.isInteger(selectStatementCount) || selectStatementCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid select statement count for ${selectStatementCountExportName}`
			);
		}
		if (
			typeof selectStatementCountExportValue === 'function' &&
			selectStatementCount !== expectedSelectStatementCount
		) {
			throw new Error(
		...
		const switchCaseClauseCount =
			typeof switchCaseClauseCountExportValue === 'function'
				? Number(switchCaseClauseCountExportValue())
				: expectedSwitchCaseClauseCount;
		if (!Number.isInteger(switchCaseClauseCount) || switchCaseClauseCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid switch case clause count for ${switchCaseClauseCountExportName}`
			);
		}
		if (
			typeof switchCaseClauseCountExportValue === 'function' &&
			switchCaseClauseCount !== expectedSwitchCaseClauseCount
		) {
			throw new Error(
		...
		const selectCommClauseCount =
			typeof selectCommClauseCountExportValue === 'function'
				? Number(selectCommClauseCountExportValue())
				: expectedSelectCommClauseCount;
		if (!Number.isInteger(selectCommClauseCount) || selectCommClauseCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid select comm clause count for ${selectCommClauseCountExportName}`
			);
		}
		if (
			typeof selectCommClauseCountExportValue === 'function' &&
			selectCommClauseCount !== expectedSelectCommClauseCount
		) {
			throw new Error(
		...
		const forStatementCount =
			typeof forStatementCountExportValue === 'function'
				? Number(forStatementCountExportValue())
				: expectedForStatementCount;
		if (!Number.isInteger(forStatementCount) || forStatementCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid for statement count for ${forStatementCountExportName}`
			);
		}
		if (
			typeof forStatementCountExportValue === 'function' &&
			forStatementCount !== expectedForStatementCount
		) {
			throw new Error(
		...
		const breakStatementCount =
			typeof breakStatementCountExportValue === 'function'
				? Number(breakStatementCountExportValue())
				: expectedBreakStatementCount;
		if (!Number.isInteger(breakStatementCount) || breakStatementCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid break statement count for ${breakStatementCountExportName}`
			);
		}
		if (
			typeof breakStatementCountExportValue === 'function' &&
			breakStatementCount !== expectedBreakStatementCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(/\bbreak\s+([A-Za-z_]\w*)/g)) {
				for (const byte of textEncoder.encode(match[1])) {
					expectedBreakLabelNameHash =
						(expectedBreakLabelNameHash + byte * expectedBreakLabelNameHashPosition) >>>
						0;
					expectedBreakLabelNameHashPosition += 1;
				}
				expectedBreakLabelNameHash =
					(expectedBreakLabelNameHash + 0x0a * expectedBreakLabelNameHashPosition) >>> 0;
				expectedBreakLabelNameHashPosition += 1;
			}
		}
		if (typeof breakLabelNameHashExportValue !== 'function') {
			if (expectedBreakLabelNameHash !== 0) {
				throw new Error(
					`frontend lowered artifact probe missing export ${breakLabelNameHashExportName}`
				);
			}
		}
		const breakLabelNameHash =
			typeof breakLabelNameHashExportValue === 'function'
				? Number(breakLabelNameHashExportValue())
				: 0;
		if (!Number.isInteger(breakLabelNameHash) || breakLabelNameHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid break label name hash for ${breakLabelNameHashExportName}`
			);
		}
		...
		const continueStatementCount =
			typeof continueStatementCountExportValue === 'function'
				? Number(continueStatementCountExportValue())
				: expectedContinueStatementCount;
		if (!Number.isInteger(continueStatementCount) || continueStatementCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid continue statement count for ${continueStatementCountExportName}`
			);
		}
		if (
			typeof continueStatementCountExportValue === 'function' &&
			continueStatementCount !== expectedContinueStatementCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(/\bcontinue\s+([A-Za-z_]\w*)/g)) {
				for (const byte of textEncoder.encode(match[1])) {
					expectedContinueLabelNameHash =
						(expectedContinueLabelNameHash +
							byte * expectedContinueLabelNameHashPosition) >>>
						0;
					expectedContinueLabelNameHashPosition += 1;
				}
				expectedContinueLabelNameHash =
					(expectedContinueLabelNameHash +
						0x0a * expectedContinueLabelNameHashPosition) >>>
					0;
				expectedContinueLabelNameHashPosition += 1;
			}
		}
		if (typeof continueLabelNameHashExportValue !== 'function') {
			if (expectedContinueLabelNameHash !== 0) {
				throw new Error(
					`frontend lowered artifact probe missing export ${continueLabelNameHashExportName}`
				);
			}
		}
		const continueLabelNameHash =
			typeof continueLabelNameHashExportValue === 'function'
				? Number(continueLabelNameHashExportValue())
				: 0;
		if (!Number.isInteger(continueLabelNameHash) || continueLabelNameHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid continue label name hash for ${continueLabelNameHashExportName}`
			);
		}
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			expectedLabeledStatementCount += (
				sourceText.match(/^\s*(?!default:)[A-Za-z_]\w*:\s*$/gm) ?? []
			).length;
		}
		const labeledStatementCount =
			typeof labeledStatementCountExportValue === 'function'
				? Number(labeledStatementCountExportValue())
				: expectedLabeledStatementCount;
		if (!Number.isInteger(labeledStatementCount) || labeledStatementCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid labeled statement count for ${labeledStatementCountExportName}`
			);
		}
		if (
			typeof labeledStatementCountExportValue === 'function' &&
			labeledStatementCount !== expectedLabeledStatementCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(/^\s*(?!default:)([A-Za-z_]\w*):\s*$/gm)) {
				for (const byte of textEncoder.encode(match[1])) {
					expectedLabelNameHash =
						(expectedLabelNameHash + byte * expectedLabelNameHashPosition) >>> 0;
					expectedLabelNameHashPosition += 1;
				}
				expectedLabelNameHash =
					(expectedLabelNameHash + 0x0a * expectedLabelNameHashPosition) >>> 0;
				expectedLabelNameHashPosition += 1;
			}
		}
		if (typeof labelNameHashExportValue !== 'function') {
			if (expectedLabelNameHash !== 0) {
				throw new Error(
					`frontend lowered artifact probe missing export ${labelNameHashExportName}`
				);
			}
		}
		const labelNameHash =
			typeof labelNameHashExportValue === 'function' ? Number(labelNameHashExportValue()) : 0;
		if (!Number.isInteger(labelNameHash) || labelNameHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid label name hash for ${labelNameHashExportName}`
			);
		}
		...
		const gotoStatementCount =
			typeof gotoStatementCountExportValue === 'function'
				? Number(gotoStatementCountExportValue())
				: expectedGotoStatementCount;
		if (!Number.isInteger(gotoStatementCount) || gotoStatementCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid goto statement count for ${gotoStatementCountExportName}`
			);
		}
		if (
			typeof gotoStatementCountExportValue === 'function' &&
			gotoStatementCount !== expectedGotoStatementCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(/\bgoto\s+([A-Za-z_]\w*)/g)) {
				for (const byte of textEncoder.encode(match[1])) {
					expectedGotoLabelNameHash =
						(expectedGotoLabelNameHash + byte * expectedGotoLabelNameHashPosition) >>>
						0;
					expectedGotoLabelNameHashPosition += 1;
				}
				expectedGotoLabelNameHash =
					(expectedGotoLabelNameHash + 0x0a * expectedGotoLabelNameHashPosition) >>> 0;
				expectedGotoLabelNameHashPosition += 1;
			}
		}
		if (typeof gotoLabelNameHashExportValue !== 'function') {
			if (expectedGotoLabelNameHash !== 0) {
				throw new Error(
					`frontend lowered artifact probe missing export ${gotoLabelNameHashExportName}`
				);
			}
		}
		const gotoLabelNameHash =
			typeof gotoLabelNameHashExportValue === 'function'
				? Number(gotoLabelNameHashExportValue())
				: 0;
		if (!Number.isInteger(gotoLabelNameHash) || gotoLabelNameHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid goto label name hash for ${gotoLabelNameHashExportName}`
			);
		}
		...
		const fallthroughStatementCountExportValue =
			exportsObject[fallthroughStatementCountExportName];
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			expectedFallthroughStatementCount += (sourceText.match(/\bfallthrough\b/g) ?? [])
				.length;
		}
		const fallthroughStatementCount =
			typeof fallthroughStatementCountExportValue === 'function'
				? Number(fallthroughStatementCountExportValue())
				: expectedFallthroughStatementCount;
		if (!Number.isInteger(fallthroughStatementCount) || fallthroughStatementCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid fallthrough statement count for ${fallthroughStatementCountExportName}`
			);
		}
		if (
			typeof fallthroughStatementCountExportValue === 'function' &&
			fallthroughStatementCount !== expectedFallthroughStatementCount
		) {
			throw new Error(
		...
		if (typeof methodCountExportValue !== 'function') {
			throw new Error(
				`frontend lowered artifact probe missing export ${methodCountExportName}`
			);
		}
		const methodCount = Number(methodCountExportValue());
		if (!Number.isInteger(methodCount) || methodCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid method count for ${methodCountExportName}`
			);
		}
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			expectedMethodCount += (
				sourceText.match(/^\s*func\s*\([^)]*\)\s*[A-Za-z_]\w*\s*\(/gm) ?? []
			).length;
		}
			...
			for (const match of sourceText.matchAll(
				...
				for (const byte of textEncoder.encode(match[1])) {
					expectedMethodNameHash =
						(expectedMethodNameHash + byte * expectedMethodNameHashPosition) >>> 0;
					expectedMethodNameHashPosition += 1;
				}
				expectedMethodNameHash =
					(expectedMethodNameHash + 0x0a * expectedMethodNameHashPosition) >>> 0;
				expectedMethodNameHashPosition += 1;
			}
		}
		const methodNameHash =
			typeof methodNameHashExportValue === 'function'
				? Number(methodNameHashExportValue())
				: expectedMethodNameHash;
		if (!Number.isInteger(methodNameHash) || methodNameHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid method name hash for ${methodNameHashExportName}`
			);
		}
		if (
			typeof methodNameHashExportValue === 'function' &&
			methodNameHash !== expectedMethodNameHash
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(
				/^\s*func\s*\([^)]*\)\s+[A-Za-z_]\w*(?:\s*\[[^\]]+\])?\s*(?:\([^)]*\)(?:\s*\([^)]*\)|\s+[^{\n]+)?)\s*\{/gm
			)) {
				for (const byte of textEncoder.encode(match[0].replace(/\s*\{$/, '').trim())) {
					expectedMethodSignatureHash =
						(expectedMethodSignatureHash +
							byte * expectedMethodSignatureHashPosition) >>>
						0;
					expectedMethodSignatureHashPosition += 1;
				}
				expectedMethodSignatureHash =
					(expectedMethodSignatureHash + 0x0a * expectedMethodSignatureHashPosition) >>>
					0;
				expectedMethodSignatureHashPosition += 1;
			}
		}
		const methodSignatureHash =
			typeof methodSignatureHashExportValue === 'function'
				? Number(methodSignatureHashExportValue())
				: expectedMethodSignatureHash;
		if (!Number.isInteger(methodSignatureHash) || methodSignatureHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid method signature hash for ${methodSignatureHashExportName}`
			);
		}
		if (
			typeof methodSignatureHashExportValue === 'function' &&
			methodSignatureHash !== expectedMethodSignatureHash
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(
				/^\s*func\s*\([^)]*\)\s+([A-Z]\w*)(?:\s*\[[^\]]+\])?\s*\(/gm
			)) {
				for (const byte of textEncoder.encode(match[1])) {
					expectedExportedMethodNameHash =
						(expectedExportedMethodNameHash +
							byte * expectedExportedMethodNameHashPosition) >>>
						0;
					expectedExportedMethodNameHashPosition += 1;
				}
				expectedExportedMethodNameHash =
					(expectedExportedMethodNameHash +
						0x0a * expectedExportedMethodNameHashPosition) >>>
					0;
				expectedExportedMethodNameHashPosition += 1;
			}
		}
		const exportedMethodNameHash =
			typeof exportedMethodNameHashExportValue === 'function'
				? Number(exportedMethodNameHashExportValue())
				: expectedExportedMethodNameHash;
		if (!Number.isInteger(exportedMethodNameHash) || exportedMethodNameHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid exported method name hash for ${exportedMethodNameHashExportName}`
			);
		}
		if (
			typeof exportedMethodNameHashExportValue === 'function' &&
			exportedMethodNameHash !== expectedExportedMethodNameHash
		) {
			throw new Error(
		...
		const exportedMethodSignatureHashExportValue =
			exportsObject[exportedMethodSignatureHashExportName];
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(
				/^\s*func\s*\([^)]*\)\s+([A-Z]\w*)(?:\s*\[[^\]]+\])?\s*(?:\([^)]*\)(?:\s*\([^)]*\)|\s+[^{\n]+)?)\s*\{/gm
			)) {
				for (const byte of textEncoder.encode(match[0].replace(/\s*\{$/, '').trim())) {
					expectedExportedMethodSignatureHash =
						(expectedExportedMethodSignatureHash +
							byte * expectedExportedMethodSignatureHashPosition) >>>
						0;
					expectedExportedMethodSignatureHashPosition += 1;
				}
				expectedExportedMethodSignatureHash =
					(expectedExportedMethodSignatureHash +
						0x0a * expectedExportedMethodSignatureHashPosition) >>>
					0;
				expectedExportedMethodSignatureHashPosition += 1;
			}
		}
		...
		if (!Number.isInteger(exportedMethodSignatureHash) || exportedMethodSignatureHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid exported method signature hash for ${exportedMethodSignatureHashExportName}`
			);
		}
		if (
			typeof exportedMethodSignatureHashExportValue === 'function' &&
			exportedMethodSignatureHash !== expectedExportedMethodSignatureHash
		) {
			throw new Error(
		...
		if (typeof exportedFunctionCountExportValue !== 'function') {
			throw new Error(
				`frontend lowered artifact probe missing export ${exportedFunctionCountExportName}`
			);
		}
		...
		if (!Number.isInteger(exportedFunctionCount) || exportedFunctionCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid exported function count for ${exportedFunctionCountExportName}`
			);
		}
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			expectedExportedFunctionCount += (
				sourceText.match(/^\s*func\s+[A-Z]\w*(?:\s*\[[^\]]+\])?\s*\(/gm) ?? []
			).length;
		}
		...
		const exportedFunctionNameHashExportValue =
			exportsObject[exportedFunctionNameHashExportName];
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(
				/^\s*func\s+([A-Z]\w*)(?:\s*\[[^\]]+\])?\s*\(/gm
			)) {
				for (const byte of textEncoder.encode(match[1])) {
					expectedExportedFunctionNameHash =
						(expectedExportedFunctionNameHash +
							byte * expectedExportedFunctionNameHashPosition) >>>
						0;
					expectedExportedFunctionNameHashPosition += 1;
				}
				expectedExportedFunctionNameHash =
					(expectedExportedFunctionNameHash +
						0x0a * expectedExportedFunctionNameHashPosition) >>>
					0;
				expectedExportedFunctionNameHashPosition += 1;
			}
		}
		...
		if (!Number.isInteger(exportedFunctionNameHash) || exportedFunctionNameHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid exported function name hash for ${exportedFunctionNameHashExportName}`
			);
		}
		if (
			typeof exportedFunctionNameHashExportValue === 'function' &&
		...
		if (typeof typeCountExportValue !== 'function') {
			throw new Error(
				`frontend lowered artifact probe missing export ${typeCountExportName}`
			);
		}
		const typeCount = Number(typeCountExportValue());
		if (!Number.isInteger(typeCount) || typeCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid type count for ${typeCountExportName}`
			);
		}
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(/^\s*type\s+([A-Za-z_]\w*)\b/gm)) {
				for (const byte of textEncoder.encode(match[1])) {
					expectedTypeNameHash =
						(expectedTypeNameHash + byte * expectedTypeNameHashPosition) >>> 0;
					expectedTypeNameHashPosition += 1;
				}
				expectedTypeNameHash =
					(expectedTypeNameHash + 0x0a * expectedTypeNameHashPosition) >>> 0;
				expectedTypeNameHashPosition += 1;
			}
		}
		const typeNameHash =
			typeof typeNameHashExportValue === 'function'
				? Number(typeNameHashExportValue())
				: expectedTypeNameHash;
		if (!Number.isInteger(typeNameHash) || typeNameHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid type name hash for ${typeNameHashExportName}`
			);
		}
		if (
			typeof typeNameHashExportValue === 'function' &&
			typeNameHash !== expectedTypeNameHash
		) {
			throw new Error(
		...
		if (typeof exportedTypeCountExportValue !== 'function') {
			throw new Error(
				`frontend lowered artifact probe missing export ${exportedTypeCountExportName}`
			);
		}
		const exportedTypeCount = Number(exportedTypeCountExportValue());
		if (!Number.isInteger(exportedTypeCount) || exportedTypeCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid exported type count for ${exportedTypeCountExportName}`
			);
		}
			...
			for (const match of sourceText.matchAll(/^\s*type\s+([A-Z]\w*)\b/gm)) {
				for (const byte of textEncoder.encode(match[1])) {
					expectedExportedTypeNameHash =
						(expectedExportedTypeNameHash +
							byte * expectedExportedTypeNameHashPosition) >>>
						0;
					expectedExportedTypeNameHashPosition += 1;
				}
				expectedExportedTypeNameHash =
					(expectedExportedTypeNameHash + 0x0a * expectedExportedTypeNameHashPosition) >>>
					0;
				expectedExportedTypeNameHashPosition += 1;
			}
		}
		...
		if (!Number.isInteger(exportedTypeNameHash) || exportedTypeNameHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid exported type name hash for ${exportedTypeNameHashExportName}`
			);
		}
		if (
			typeof exportedTypeNameHashExportValue === 'function' &&
		...
		if (typeof structTypeCountExportValue !== 'function') {
			throw new Error(
				`frontend lowered artifact probe missing export ${structTypeCountExportName}`
			);
		}
		const structTypeCount = Number(structTypeCountExportValue());
		if (!Number.isInteger(structTypeCount) || structTypeCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid struct type count for ${structTypeCountExportName}`
			);
		}
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			expectedStructTypeCount += (
				sourceText.match(/^\s*type\s+[A-Za-z_]\w*(?:\s*\[[^\]]+\])?\s+struct\b/gm) ?? []
			).length;
		}
		...
		if (typeof interfaceTypeCountExportValue !== 'function') {
			throw new Error(
				`frontend lowered artifact probe missing export ${interfaceTypeCountExportName}`
			);
		}
		const interfaceTypeCount = Number(interfaceTypeCountExportValue());
		if (!Number.isInteger(interfaceTypeCount) || interfaceTypeCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid interface type count for ${interfaceTypeCountExportName}`
			);
		}
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			expectedInterfaceTypeCount += (
				sourceText.match(/^\s*type\s+[A-Za-z_]\w*(?:\s*\[[^\]]+\])?\s+interface\b/gm) ?? []
			).length;
		}
		...
		const mapTypeCount =
			typeof mapTypeCountExportValue === 'function'
				? Number(mapTypeCountExportValue())
				: expectedMapTypeCount;
		if (!Number.isInteger(mapTypeCount) || mapTypeCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid map type count for ${mapTypeCountExportName}`
			);
		}
		if (
			typeof mapTypeCountExportValue === 'function' &&
			mapTypeCount !== expectedMapTypeCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			expectedChanTypeCount += (sourceText.match(/(?:<-\s*chan|chan\s*<-|\bchan\b)/g) ?? [])
				.length;
		}
		const chanTypeCount =
			typeof chanTypeCountExportValue === 'function'
				? Number(chanTypeCountExportValue())
				: expectedChanTypeCount;
		if (!Number.isInteger(chanTypeCount) || chanTypeCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid chan type count for ${chanTypeCountExportName}`
			);
		}
		if (
			typeof chanTypeCountExportValue === 'function' &&
			chanTypeCount !== expectedChanTypeCount
		) {
			throw new Error(
		...
		const sendOnlyChanTypeCount =
			typeof sendOnlyChanTypeCountExportValue === 'function'
				? Number(sendOnlyChanTypeCountExportValue())
				: expectedSendOnlyChanTypeCount;
		if (!Number.isInteger(sendOnlyChanTypeCount) || sendOnlyChanTypeCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid send-only chan type count for ${sendOnlyChanTypeCountExportName}`
			);
		}
		if (
			typeof sendOnlyChanTypeCountExportValue === 'function' &&
			sendOnlyChanTypeCount !== expectedSendOnlyChanTypeCount
		) {
			throw new Error(
		...
		const receiveOnlyChanTypeCountExportValue =
			exportsObject[receiveOnlyChanTypeCountExportName];
		...
		const receiveOnlyChanTypeCount =
			typeof receiveOnlyChanTypeCountExportValue === 'function'
				? Number(receiveOnlyChanTypeCountExportValue())
				: expectedReceiveOnlyChanTypeCount;
		if (!Number.isInteger(receiveOnlyChanTypeCount) || receiveOnlyChanTypeCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid receive-only chan type count for ${receiveOnlyChanTypeCountExportName}`
			);
		}
		if (
			typeof receiveOnlyChanTypeCountExportValue === 'function' &&
			receiveOnlyChanTypeCount !== expectedReceiveOnlyChanTypeCount
		) {
			throw new Error(
		...
		const arrayTypeCount =
			typeof arrayTypeCountExportValue === 'function'
				? Number(arrayTypeCountExportValue())
				: expectedArrayTypeCount;
		if (!Number.isInteger(arrayTypeCount) || arrayTypeCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid array type count for ${arrayTypeCountExportName}`
			);
		}
		if (
			typeof arrayTypeCountExportValue === 'function' &&
			arrayTypeCount !== expectedArrayTypeCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			expectedSliceTypeCount += [
				...sourceText.matchAll(
					/\[\](?=\s*(?:\[[^\]\n]*\]|[\*A-Za-z_]|map\s*\[|chan\b|<-|struct\b|interface\b))/gm
				)
			].length;
		}
		const sliceTypeCount =
			typeof sliceTypeCountExportValue === 'function'
				? Number(sliceTypeCountExportValue())
				: expectedSliceTypeCount;
		if (!Number.isInteger(sliceTypeCount) || sliceTypeCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid slice type count for ${sliceTypeCountExportName}`
			);
		}
		if (
			typeof sliceTypeCountExportValue === 'function' &&
			sliceTypeCount !== expectedSliceTypeCount
		) {
			throw new Error(
		...
		const pointerTypeCount =
			typeof pointerTypeCountExportValue === 'function'
				? Number(pointerTypeCountExportValue())
				: expectedPointerTypeCount;
		if (!Number.isInteger(pointerTypeCount) || pointerTypeCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid pointer type count for ${pointerTypeCountExportName}`
			);
		}
		if (
			typeof pointerTypeCountExportValue === 'function' &&
			pointerTypeCount !== expectedPointerTypeCount
		) {
			throw new Error(
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(
				/type\s+[A-Za-z_]\w*\s+struct\s*\{([^}]*)\}/g
			)) {
				for (const field of match[1]
					.split(/[;\n]/)
					...
					if (tagMatch) {
						...
						for (const byte of textEncoder.encode(tagMatch[1])) {
							expectedTaggedStructFieldTagHash =
								(expectedTaggedStructFieldTagHash +
									byte * expectedTaggedStructFieldTagHashPosition) >>>
								0;
							expectedTaggedStructFieldTagHashPosition += 1;
						}
						expectedTaggedStructFieldTagHash =
							(expectedTaggedStructFieldTagHash +
								0x0a * expectedTaggedStructFieldTagHashPosition) >>>
							0;
						expectedTaggedStructFieldTagHashPosition += 1;
					}
					...
					if (!/\s/.test(fieldWithoutTag)) {
						...
						for (const byte of textEncoder.encode(fieldWithoutTag)) {
							expectedEmbeddedStructFieldTypeHash =
								(expectedEmbeddedStructFieldTypeHash +
									byte * expectedEmbeddedStructFieldTypeHashPosition) >>>
								0;
							expectedEmbeddedStructFieldTypeHashPosition += 1;
						}
						expectedEmbeddedStructFieldTypeHash =
							(expectedEmbeddedStructFieldTypeHash +
								0x0a * expectedEmbeddedStructFieldTypeHashPosition) >>>
							0;
						expectedEmbeddedStructFieldTypeHashPosition += 1;
						continue;
					}
					const namedFieldMatch = fieldWithoutTag.match(
						/^([A-Za-z_]\w*(?:\s*,\s*[A-Za-z_]\w*)*)\s+/
					);
					if (!namedFieldMatch) {
						continue;
					}
					const fieldTypeMatch = fieldWithoutTag.match(
						/^([A-Za-z_]\w*(?:\s*,\s*[A-Za-z_]\w*)*)\s+(.+)$/
					);
					const fieldTypeSource = fieldTypeMatch?.[2]?.trim() ?? '';
					const fieldNames = namedFieldMatch[1]
						.split(',')
						.map((entry) => entry.trim())
						.filter((entry) => entry !== '');
					...
					for (const fieldName of fieldNames) {
						for (const byte of textEncoder.encode(fieldName)) {
							expectedStructFieldNameHash =
								(expectedStructFieldNameHash +
									byte * expectedStructFieldNameHashPosition) >>>
								0;
							expectedStructFieldNameHashPosition += 1;
						}
						expectedStructFieldNameHash =
							(expectedStructFieldNameHash +
								0x0a * expectedStructFieldNameHashPosition) >>>
							0;
						expectedStructFieldNameHashPosition += 1;
						for (const byte of textEncoder.encode(fieldTypeSource)) {
							expectedStructFieldTypeHash =
								(expectedStructFieldTypeHash +
									byte * expectedStructFieldTypeHashPosition) >>>
								0;
							expectedStructFieldTypeHashPosition += 1;
						}
						expectedStructFieldTypeHash =
							(expectedStructFieldTypeHash +
								0x0a * expectedStructFieldTypeHashPosition) >>>
							0;
						expectedStructFieldTypeHashPosition += 1;
					}
				}
		...
		if (typeof structFieldCountExportValue !== 'function') {
			if (expectedStructFieldCount !== 0) {
				throw new Error(
					`frontend lowered artifact probe missing export ${structFieldCountExportName}`
				);
			}
		}
		const structFieldCount =
			typeof structFieldCountExportValue === 'function'
				? Number(structFieldCountExportValue())
				: 0;
		if (!Number.isInteger(structFieldCount) || structFieldCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid struct field count for ${structFieldCountExportName}`
			);
		}
		...
		const embeddedStructFieldCountExportValue =
			exportsObject[embeddedStructFieldCountExportName];
		if (typeof embeddedStructFieldCountExportValue !== 'function') {
			if (expectedEmbeddedStructFieldCount !== 0) {
				throw new Error(
					`frontend lowered artifact probe missing export ${embeddedStructFieldCountExportName}`
				);
			}
		}
		const embeddedStructFieldCount =
			typeof embeddedStructFieldCountExportValue === 'function'
				? Number(embeddedStructFieldCountExportValue())
				: 0;
		if (!Number.isInteger(embeddedStructFieldCount) || embeddedStructFieldCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid embedded struct field count for ${embeddedStructFieldCountExportName}`
			);
		}
		...
		if (typeof taggedStructFieldCountExportValue !== 'function') {
			if (expectedTaggedStructFieldCount !== 0) {
				throw new Error(
					`frontend lowered artifact probe missing export ${taggedStructFieldCountExportName}`
				);
			}
		}
		const taggedStructFieldCount =
			typeof taggedStructFieldCountExportValue === 'function'
				? Number(taggedStructFieldCountExportValue())
				: 0;
		if (!Number.isInteger(taggedStructFieldCount) || taggedStructFieldCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid tagged struct field count for ${taggedStructFieldCountExportName}`
			);
		}
		...
		if (typeof structFieldNameHashExportValue !== 'function') {
			if (expectedStructFieldNameHash !== 0) {
				throw new Error(
					`frontend lowered artifact probe missing export ${structFieldNameHashExportName}`
				);
			}
		}
		const structFieldNameHash =
			typeof structFieldNameHashExportValue === 'function'
				? Number(structFieldNameHashExportValue())
				: 0;
		if (!Number.isInteger(structFieldNameHash) || structFieldNameHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid struct field name hash for ${structFieldNameHashExportName}`
			);
		}
		...
		if (typeof structFieldTypeHashExportValue !== 'function') {
			if (expectedStructFieldTypeHash !== 0) {
				throw new Error(
					`frontend lowered artifact probe missing export ${structFieldTypeHashExportName}`
				);
			}
		}
		const structFieldTypeHash =
			typeof structFieldTypeHashExportValue === 'function'
				? Number(structFieldTypeHashExportValue())
				: 0;
		if (!Number.isInteger(structFieldTypeHash) || structFieldTypeHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid struct field type hash for ${structFieldTypeHashExportName}`
			);
		}
		...
		const embeddedStructFieldTypeHashExportValue =
			exportsObject[embeddedStructFieldTypeHashExportName];
		if (typeof embeddedStructFieldTypeHashExportValue !== 'function') {
			if (expectedEmbeddedStructFieldTypeHash !== 0) {
				throw new Error(
					`frontend lowered artifact probe missing export ${embeddedStructFieldTypeHashExportName}`
				);
			}
		}
		const embeddedStructFieldTypeHash =
			typeof embeddedStructFieldTypeHashExportValue === 'function'
				? Number(embeddedStructFieldTypeHashExportValue())
				: 0;
		if (!Number.isInteger(embeddedStructFieldTypeHash) || embeddedStructFieldTypeHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid embedded struct field type hash for ${embeddedStructFieldTypeHashExportName}`
			);
		}
		...
		const taggedStructFieldTagHashExportValue =
			exportsObject[taggedStructFieldTagHashExportName];
		if (typeof taggedStructFieldTagHashExportValue !== 'function') {
			if (expectedTaggedStructFieldTagHash !== 0) {
				throw new Error(
					`frontend lowered artifact probe missing export ${taggedStructFieldTagHashExportName}`
				);
			}
		}
		const taggedStructFieldTagHash =
			typeof taggedStructFieldTagHashExportValue === 'function'
				? Number(taggedStructFieldTagHashExportValue())
				: 0;
		if (!Number.isInteger(taggedStructFieldTagHash) || taggedStructFieldTagHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid tagged struct field tag hash for ${taggedStructFieldTagHashExportName}`
			);
		}
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(
				/type\s+[A-Za-z_]\w*\s+interface\s*\{([^}]*)\}/g
			)) {
				for (const method of match[1]
					.split(/[;\n]/)
					.map((entry) => entry.trim())
					.filter((entry) => entry !== '')) {
					...
					if (!method.includes('(')) {
						...
						for (const byte of textEncoder.encode(method)) {
							expectedEmbeddedInterfaceMethodNameHash =
								(expectedEmbeddedInterfaceMethodNameHash +
									byte * expectedEmbeddedInterfaceMethodNameHashPosition) >>>
								0;
							expectedEmbeddedInterfaceMethodNameHashPosition += 1;
						}
						expectedEmbeddedInterfaceMethodNameHash =
							(expectedEmbeddedInterfaceMethodNameHash +
								0x0a * expectedEmbeddedInterfaceMethodNameHashPosition) >>>
							0;
						expectedEmbeddedInterfaceMethodNameHashPosition += 1;
						continue;
					}
					...
					for (const byte of textEncoder.encode(methodNameMatch[1])) {
						expectedInterfaceMethodNameHash =
							(expectedInterfaceMethodNameHash +
								byte * expectedInterfaceMethodNameHashPosition) >>>
							0;
						expectedInterfaceMethodNameHashPosition += 1;
					}
					expectedInterfaceMethodNameHash =
						(expectedInterfaceMethodNameHash +
							0x0a * expectedInterfaceMethodNameHashPosition) >>>
						0;
					expectedInterfaceMethodNameHashPosition += 1;
					for (const byte of textEncoder.encode(methodSignatureSource)) {
						expectedInterfaceMethodSignatureHash =
							(expectedInterfaceMethodSignatureHash +
								byte * expectedInterfaceMethodSignatureHashPosition) >>>
							0;
						expectedInterfaceMethodSignatureHashPosition += 1;
					}
					expectedInterfaceMethodSignatureHash =
						(expectedInterfaceMethodSignatureHash +
							0x0a * expectedInterfaceMethodSignatureHashPosition) >>>
						0;
					expectedInterfaceMethodSignatureHashPosition += 1;
				}
			}
		...
		if (typeof interfaceMethodCountExportValue !== 'function') {
			if (expectedInterfaceMethodCount !== 0) {
				throw new Error(
					`frontend lowered artifact probe missing export ${interfaceMethodCountExportName}`
				);
			}
		}
		const interfaceMethodCount =
			typeof interfaceMethodCountExportValue === 'function'
				? Number(interfaceMethodCountExportValue())
				: 0;
		if (!Number.isInteger(interfaceMethodCount) || interfaceMethodCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid interface method count for ${interfaceMethodCountExportName}`
			);
		}
		...
		if (typeof interfaceMethodNameHashExportValue !== 'function') {
			if (expectedInterfaceMethodNameHash !== 0) {
				throw new Error(
					`frontend lowered artifact probe missing export ${interfaceMethodNameHashExportName}`
				);
			}
		}
		const interfaceMethodNameHash =
			typeof interfaceMethodNameHashExportValue === 'function'
				? Number(interfaceMethodNameHashExportValue())
				: 0;
		if (!Number.isInteger(interfaceMethodNameHash) || interfaceMethodNameHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid interface method name hash for ${interfaceMethodNameHashExportName}`
			);
		}
		...
		const interfaceMethodSignatureHashExportValue =
			exportsObject[interfaceMethodSignatureHashExportName];
		if (typeof interfaceMethodSignatureHashExportValue !== 'function') {
			if (expectedInterfaceMethodSignatureHash !== 0) {
				throw new Error(
					`frontend lowered artifact probe missing export ${interfaceMethodSignatureHashExportName}`
				);
			}
		}
		const interfaceMethodSignatureHash =
			typeof interfaceMethodSignatureHashExportValue === 'function'
				? Number(interfaceMethodSignatureHashExportValue())
				: 0;
		if (!Number.isInteger(interfaceMethodSignatureHash) || interfaceMethodSignatureHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid interface method signature hash for ${interfaceMethodSignatureHashExportName}`
			);
		}
		...
		const embeddedInterfaceMethodCountExportValue =
			exportsObject[embeddedInterfaceMethodCountExportName];
		if (typeof embeddedInterfaceMethodCountExportValue !== 'function') {
			if (expectedEmbeddedInterfaceMethodCount !== 0) {
				throw new Error(
					`frontend lowered artifact probe missing export ${embeddedInterfaceMethodCountExportName}`
				);
			}
		}
		const embeddedInterfaceMethodCount =
			typeof embeddedInterfaceMethodCountExportValue === 'function'
				? Number(embeddedInterfaceMethodCountExportValue())
				: 0;
		if (!Number.isInteger(embeddedInterfaceMethodCount) || embeddedInterfaceMethodCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid embedded interface method count for ${embeddedInterfaceMethodCountExportName}`
			);
		}
		...
		const embeddedInterfaceMethodNameHashExportValue =
			exportsObject[embeddedInterfaceMethodNameHashExportName];
		if (typeof embeddedInterfaceMethodNameHashExportValue !== 'function') {
			if (expectedEmbeddedInterfaceMethodNameHash !== 0) {
				throw new Error(
					`frontend lowered artifact probe missing export ${embeddedInterfaceMethodNameHashExportName}`
				);
			}
		}
		const embeddedInterfaceMethodNameHash =
			typeof embeddedInterfaceMethodNameHashExportValue === 'function'
				? Number(embeddedInterfaceMethodNameHashExportValue())
				: 0;
		if (
			!Number.isInteger(embeddedInterfaceMethodNameHash) ||
			embeddedInterfaceMethodNameHash < 0
		) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid embedded interface method name hash for ${embeddedInterfaceMethodNameHashExportName}`
			);
		}
		...
		if (typeof constCountExportValue !== 'function') {
			throw new Error(
				`frontend lowered artifact probe missing export ${constCountExportName}`
			);
		}
		const constCount = Number(constCountExportValue());
		if (!Number.isInteger(constCount) || constCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid const count for ${constCountExportName}`
			);
		}
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const blockMatch of sourceText.matchAll(/^\s*const\s*\(([\s\S]*?)^\s*\)/gm)) {
				for (const line of blockMatch[1]
					.split('\n')
					.map((entry) => entry.trim())
					.filter((entry) => entry !== '')) {
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(
				/^\s*const\s+([A-Za-z_]\w*(?:\s*,\s*[A-Za-z_]\w*)*)/gm
			)) {
				for (const name of match[1]
					...
					for (const byte of textEncoder.encode(name)) {
						expectedConstNameHash =
							(expectedConstNameHash + byte * expectedConstNameHashPosition) >>> 0;
						expectedConstNameHashPosition += 1;
					}
					expectedConstNameHash =
						(expectedConstNameHash + 0x0a * expectedConstNameHashPosition) >>> 0;
					expectedConstNameHashPosition += 1;
				}
			}
			for (const blockMatch of sourceText.matchAll(/^\s*const\s*\(([\s\S]*?)^\s*\)/gm)) {
				for (const line of blockMatch[1]
					...
					for (const name of nameMatch[1]
						.split(',')
						.map((entry) => entry.trim())
						.filter((entry) => entry !== '')) {
						for (const byte of textEncoder.encode(name)) {
							expectedConstNameHash =
								(expectedConstNameHash + byte * expectedConstNameHashPosition) >>>
								0;
							expectedConstNameHashPosition += 1;
						}
						expectedConstNameHash =
		...
		const constNameHash =
			typeof constNameHashExportValue === 'function'
				? Number(constNameHashExportValue())
				: expectedConstNameHash;
		if (!Number.isInteger(constNameHash) || constNameHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid const name hash for ${constNameHashExportName}`
			);
		}
		if (
			typeof constNameHashExportValue === 'function' &&
			constNameHash !== expectedConstNameHash
		) {
			throw new Error(
		...
		if (!Number.isInteger(varCount) || varCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid var count for ${varCountExportName}`
			);
		}
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(
				/^\s*var\s+([A-Za-z_]\w*(?:\s*,\s*[A-Za-z_]\w*)*)/gm
			)) {
				for (const name of match[1]
					.split(',')
					.map((entry) => entry.trim())
					...
					for (const byte of textEncoder.encode(name)) {
						expectedVarNameHash =
							(expectedVarNameHash + byte * expectedVarNameHashPosition) >>> 0;
						expectedVarNameHashPosition += 1;
					}
					expectedVarNameHash =
						(expectedVarNameHash + 0x0a * expectedVarNameHashPosition) >>> 0;
					expectedVarNameHashPosition += 1;
				}
			}
			for (const blockMatch of sourceText.matchAll(/^\s*var\s*\(([\s\S]*?)^\s*\)/gm)) {
		...
		const varNameHash =
			typeof varNameHashExportValue === 'function'
				? Number(varNameHashExportValue())
				: expectedVarNameHash;
		if (!Number.isInteger(varNameHash) || varNameHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid var name hash for ${varNameHashExportName}`
			);
		}
		...
		if (typeof exportedConstCountExportValue !== 'function') {
			throw new Error(
				`frontend lowered artifact probe missing export ${exportedConstCountExportName}`
			);
		}
		const exportedConstCount = Number(exportedConstCountExportValue());
		if (!Number.isInteger(exportedConstCount) || exportedConstCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid exported const count for ${exportedConstCountExportName}`
			);
		}
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(
				...
				for (const name of match[1]
					...
					for (const byte of textEncoder.encode(name)) {
						expectedExportedConstNameHash =
							(expectedExportedConstNameHash +
								byte * expectedExportedConstNameHashPosition) >>>
							0;
						expectedExportedConstNameHashPosition += 1;
					}
					expectedExportedConstNameHash =
						(expectedExportedConstNameHash +
							0x0a * expectedExportedConstNameHashPosition) >>>
						0;
					expectedExportedConstNameHashPosition += 1;
				}
			}
		...
		const exportedConstNameHash =
			typeof exportedConstNameHashExportValue === 'function'
				? Number(exportedConstNameHashExportValue())
				: expectedExportedConstNameHash;
		if (!Number.isInteger(exportedConstNameHash) || exportedConstNameHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid exported const name hash for ${exportedConstNameHashExportName}`
			);
		}
		if (
			typeof exportedConstNameHashExportValue === 'function' &&
			exportedConstNameHash !== expectedExportedConstNameHash
		) {
			throw new Error(
		...
		if (typeof exportedVarCountExportValue !== 'function') {
			throw new Error(
				`frontend lowered artifact probe missing export ${exportedVarCountExportName}`
			);
		}
		const exportedVarCount = Number(exportedVarCountExportValue());
		if (!Number.isInteger(exportedVarCount) || exportedVarCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid exported var count for ${exportedVarCountExportName}`
			);
		}
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(
				...
				for (const name of match[1]
					...
					.filter((entry) => /^[A-Z]\w*$/.test(entry))) {
					for (const byte of textEncoder.encode(name)) {
						expectedExportedVarNameHash =
							(expectedExportedVarNameHash +
								byte * expectedExportedVarNameHashPosition) >>>
							0;
						expectedExportedVarNameHashPosition += 1;
					}
					expectedExportedVarNameHash =
						(expectedExportedVarNameHash +
							0x0a * expectedExportedVarNameHashPosition) >>>
						0;
					expectedExportedVarNameHashPosition += 1;
				}
			}
		...
		const exportedVarNameHash =
			typeof exportedVarNameHashExportValue === 'function'
				? Number(exportedVarNameHashExportValue())
				: expectedExportedVarNameHash;
		if (!Number.isInteger(exportedVarNameHash) || exportedVarNameHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid exported var name hash for ${exportedVarNameHashExportName}`
			);
		}
		if (
			typeof exportedVarNameHashExportValue === 'function' &&
			exportedVarNameHash !== expectedExportedVarNameHash
		) {
			throw new Error(
		...
		if (loweredIRUnit) {
			...
			if (typeof declarationCountExportValue !== 'function') {
				throw new Error(
					`frontend lowered artifact probe missing export ${declarationCountExportName}`
				);
			}
			declarationCount = Number(declarationCountExportValue());
			if (!Number.isInteger(declarationCount) || declarationCount < 0) {
				throw new Error(
					`frontend lowered artifact probe returned an invalid declaration count for ${declarationCountExportName}`
				);
			}
			const expectedDeclarationCount = (loweredIRUnit.declarations ?? []).length;
			if (declarationCount !== expectedDeclarationCount) {
				throw new Error(
					'frontend lowered artifact probe declaration count did not match lowered ir manifest'
				);
			}
			...
			if (typeof declarationNameHashExportValue !== 'function') {
				throw new Error(
					`frontend lowered artifact probe missing export ${declarationNameHashExportName}`
				);
			}
			declarationNameHash = Number(declarationNameHashExportValue());
			if (!Number.isInteger(declarationNameHash) || declarationNameHash < 0) {
				throw new Error(
					`frontend lowered artifact probe returned an invalid declaration name hash for ${declarationNameHashExportName}`
				);
			}
			...
			for (const declaration of loweredIRUnit.declarations ?? []) {
				...
				for (const byte of textEncoder.encode(declarationName)) {
					expectedDeclarationNameHash =
						(expectedDeclarationNameHash + byte * declarationNameHashPosition) >>> 0;
					declarationNameHashPosition += 1;
				}
				expectedDeclarationNameHash =
					(expectedDeclarationNameHash + 0x0a * declarationNameHashPosition) >>> 0;
				declarationNameHashPosition += 1;
			}
			if (declarationNameHash !== expectedDeclarationNameHash) {
				throw new Error(
					'frontend lowered artifact probe declaration name hash did not match lowered ir manifest'
				);
			}
			const declarationSignatureHashExportName = `tinygo_lowered_${symbolID}_declaration_signature_hash`;
			const declarationSignatureHashExportValue =
				exportsObject[declarationSignatureHashExportName];
			if (typeof declarationSignatureHashExportValue !== 'function') {
				throw new Error(
					`frontend lowered artifact probe missing export ${declarationSignatureHashExportName}`
				);
			}
			...
			if (!Number.isInteger(declarationSignatureHash) || declarationSignatureHash < 0) {
				throw new Error(
					`frontend lowered artifact probe returned an invalid declaration signature hash for ${declarationSignatureHashExportName}`
				);
			}
			...
			for (const declaration of loweredIRUnit.declarations ?? []) {
				...
				for (const byte of textEncoder.encode(declarationSignature)) {
					expectedDeclarationSignatureHash =
						(expectedDeclarationSignatureHash +
							byte * declarationSignatureHashPosition) >>>
						0;
					declarationSignatureHashPosition += 1;
				}
				expectedDeclarationSignatureHash =
					(expectedDeclarationSignatureHash + 0x0a * declarationSignatureHashPosition) >>>
					0;
				declarationSignatureHashPosition += 1;
			}
			if (declarationSignatureHash !== expectedDeclarationSignatureHash) {
				throw new Error(
					'frontend lowered artifact probe declaration signature hash did not match lowered ir manifest'
				);
			}
			...
			if (typeof declarationKindHashExportValue !== 'function') {
				throw new Error(
					`frontend lowered artifact probe missing export ${declarationKindHashExportName}`
				);
			}
			declarationKindHash = Number(declarationKindHashExportValue());
			if (!Number.isInteger(declarationKindHash) || declarationKindHash < 0) {
				throw new Error(
					`frontend lowered artifact probe returned an invalid declaration kind hash for ${declarationKindHashExportName}`
				);
			}
			...
			for (const declaration of loweredIRUnit.declarations ?? []) {
				for (const byte of textEncoder.encode(declaration.kind ?? '')) {
					expectedDeclarationKindHash =
						(expectedDeclarationKindHash + byte * declarationKindHashPosition) >>> 0;
					declarationKindHashPosition += 1;
				}
				expectedDeclarationKindHash =
					(expectedDeclarationKindHash + 0x0a * declarationKindHashPosition) >>> 0;
				declarationKindHashPosition += 1;
			}
			if (declarationKindHash !== expectedDeclarationKindHash) {
				throw new Error(
					'frontend lowered artifact probe declaration kind hash did not match lowered ir manifest'
				);
			}
			const declarationExportedCountExportName = `tinygo_lowered_${symbolID}_declaration_exported_count`;
			const declarationExportedCountExportValue =
				exportsObject[declarationExportedCountExportName];
			if (typeof declarationExportedCountExportValue !== 'function') {
				throw new Error(
					`frontend lowered artifact probe missing export ${declarationExportedCountExportName}`
				);
			}
			...
			if (!Number.isInteger(declarationExportedCount) || declarationExportedCount < 0) {
				throw new Error(
					`frontend lowered artifact probe returned an invalid declaration exported count for ${declarationExportedCountExportName}`
				);
			}
			const expectedDeclarationExportedCount = (loweredIRUnit.declarations ?? []).filter(
				(declaration) => declaration.exported
			).length;
			if (declarationExportedCount !== expectedDeclarationExportedCount) {
				throw new Error(
					'frontend lowered artifact probe declaration exported count did not match lowered ir manifest'
				);
			}
			const declarationExportedNameHashExportName = `tinygo_lowered_${symbolID}_declaration_exported_name_hash`;
			const declarationExportedNameHashExportValue =
				exportsObject[declarationExportedNameHashExportName];
			if (typeof declarationExportedNameHashExportValue !== 'function') {
				throw new Error(
					`frontend lowered artifact probe missing export ${declarationExportedNameHashExportName}`
				);
			}
			...
			if (!Number.isInteger(declarationExportedNameHash) || declarationExportedNameHash < 0) {
				throw new Error(
					`frontend lowered artifact probe returned an invalid declaration exported name hash for ${declarationExportedNameHashExportName}`
				);
			}
			let expectedDeclarationExportedNameHash = 0;
			let declarationExportedNameHashPosition = 1;
			for (const declaration of (loweredIRUnit.declarations ?? []).filter(
				...
				for (const byte of textEncoder.encode(declarationName)) {
					expectedDeclarationExportedNameHash =
						(expectedDeclarationExportedNameHash +
							byte * declarationExportedNameHashPosition) >>>
						0;
					declarationExportedNameHashPosition += 1;
				}
				expectedDeclarationExportedNameHash =
					(expectedDeclarationExportedNameHash +
						0x0a * declarationExportedNameHashPosition) >>>
					0;
				declarationExportedNameHashPosition += 1;
			}
			if (declarationExportedNameHash !== expectedDeclarationExportedNameHash) {
				throw new Error(
					'frontend lowered artifact probe declaration exported name hash did not match lowered ir manifest'
				);
			}
			const declarationExportedSignatureHashExportName = `tinygo_lowered_${symbolID}_declaration_exported_signature_hash`;
			const declarationExportedSignatureHashExportValue =
				exportsObject[declarationExportedSignatureHashExportName];
			if (typeof declarationExportedSignatureHashExportValue !== 'function') {
				throw new Error(
					`frontend lowered artifact probe missing export ${declarationExportedSignatureHashExportName}`
				);
			}
			declarationExportedSignatureHash = Number(
				declarationExportedSignatureHashExportValue()
			);
			if (
				!Number.isInteger(declarationExportedSignatureHash) ||
				declarationExportedSignatureHash < 0
			) {
				throw new Error(
					`frontend lowered artifact probe returned an invalid declaration exported signature hash for ${declarationExportedSignatureHashExportName}`
				);
			}
			...
			for (const declaration of (loweredIRUnit.declarations ?? []).filter(
				...
				const declarationSignature = `${declaration.kind ?? ''}:${declaration.name ?? ''}:${declaration.exported ? '1' : '0'}:${declaration.method ? '1' : '0'}`;
				for (const byte of textEncoder.encode(declarationSignature)) {
					expectedDeclarationExportedSignatureHash =
						(expectedDeclarationExportedSignatureHash +
							byte * declarationExportedSignatureHashPosition) >>>
						0;
					declarationExportedSignatureHashPosition += 1;
				}
				expectedDeclarationExportedSignatureHash =
					(expectedDeclarationExportedSignatureHash +
						0x0a * declarationExportedSignatureHashPosition) >>>
					0;
				declarationExportedSignatureHashPosition += 1;
			}
			if (declarationExportedSignatureHash !== expectedDeclarationExportedSignatureHash) {
				throw new Error(
					'frontend lowered artifact probe declaration exported signature hash did not match lowered ir manifest'
				);
			}
			const declarationExportedKindHashExportName = `tinygo_lowered_${symbolID}_declaration_exported_kind_hash`;
			const declarationExportedKindHashExportValue =
				exportsObject[declarationExportedKindHashExportName];
			if (typeof declarationExportedKindHashExportValue !== 'function') {
				throw new Error(
					`frontend lowered artifact probe missing export ${declarationExportedKindHashExportName}`
				);
			}
			...
			if (!Number.isInteger(declarationExportedKindHash) || declarationExportedKindHash < 0) {
				throw new Error(
					`frontend lowered artifact probe returned an invalid declaration exported kind hash for ${declarationExportedKindHashExportName}`
				);
			}
			...
			for (const declaration of (loweredIRUnit.declarations ?? []).filter(
				...
				for (const byte of textEncoder.encode(declaration.kind ?? '')) {
					expectedDeclarationExportedKindHash =
						(expectedDeclarationExportedKindHash +
							byte * declarationExportedKindHashPosition) >>>
						0;
					declarationExportedKindHashPosition += 1;
				}
				expectedDeclarationExportedKindHash =
					(expectedDeclarationExportedKindHash +
						0x0a * declarationExportedKindHashPosition) >>>
					0;
				declarationExportedKindHashPosition += 1;
			}
			if (declarationExportedKindHash !== expectedDeclarationExportedKindHash) {
				throw new Error(
					'frontend lowered artifact probe declaration exported kind hash did not match lowered ir manifest'
				);
			}
			const declarationMethodCountExportName = `tinygo_lowered_${symbolID}_declaration_method_count`;
			const declarationMethodCountExportValue =
				exportsObject[declarationMethodCountExportName];
			if (typeof declarationMethodCountExportValue !== 'function') {
				throw new Error(
					`frontend lowered artifact probe missing export ${declarationMethodCountExportName}`
				);
			}
			...
			if (!Number.isInteger(declarationMethodCount) || declarationMethodCount < 0) {
				throw new Error(
					`frontend lowered artifact probe returned an invalid declaration method count for ${declarationMethodCountExportName}`
				);
			}
			const expectedDeclarationMethodCount = (loweredIRUnit.declarations ?? []).filter(
				(declaration) => declaration.method
			).length;
			if (declarationMethodCount !== expectedDeclarationMethodCount) {
				throw new Error(
					'frontend lowered artifact probe declaration method count did not match lowered ir manifest'
				);
			}
			const declarationMethodNameHashExportName = `tinygo_lowered_${symbolID}_declaration_method_name_hash`;
			const declarationMethodNameHashExportValue =
				exportsObject[declarationMethodNameHashExportName];
			if (typeof declarationMethodNameHashExportValue !== 'function') {
				throw new Error(
					`frontend lowered artifact probe missing export ${declarationMethodNameHashExportName}`
				);
			}
			...
			if (!Number.isInteger(declarationMethodNameHash) || declarationMethodNameHash < 0) {
				throw new Error(
					`frontend lowered artifact probe returned an invalid declaration method name hash for ${declarationMethodNameHashExportName}`
				);
			}
			...
			for (const declaration of (loweredIRUnit.declarations ?? []).filter(
				...
				for (const byte of textEncoder.encode(declarationName)) {
					expectedDeclarationMethodNameHash =
						(expectedDeclarationMethodNameHash +
							byte * declarationMethodNameHashPosition) >>>
						0;
					declarationMethodNameHashPosition += 1;
				}
				expectedDeclarationMethodNameHash =
					(expectedDeclarationMethodNameHash +
						0x0a * declarationMethodNameHashPosition) >>>
					0;
				declarationMethodNameHashPosition += 1;
			}
			if (declarationMethodNameHash !== expectedDeclarationMethodNameHash) {
				throw new Error(
					'frontend lowered artifact probe declaration method name hash did not match lowered ir manifest'
				);
			}
			const declarationMethodSignatureHashExportName = `tinygo_lowered_${symbolID}_declaration_method_signature_hash`;
			const declarationMethodSignatureHashExportValue =
				exportsObject[declarationMethodSignatureHashExportName];
			if (typeof declarationMethodSignatureHashExportValue !== 'function') {
				throw new Error(
					`frontend lowered artifact probe missing export ${declarationMethodSignatureHashExportName}`
				);
			}
			declarationMethodSignatureHash = Number(declarationMethodSignatureHashExportValue());
			if (
				!Number.isInteger(declarationMethodSignatureHash) ||
				declarationMethodSignatureHash < 0
			) {
				throw new Error(
					`frontend lowered artifact probe returned an invalid declaration method signature hash for ${declarationMethodSignatureHashExportName}`
				);
			}
			...
			for (const declaration of (loweredIRUnit.declarations ?? []).filter(
				...
				const declarationSignature = `${declaration.kind ?? ''}:${declaration.name ?? ''}:${declaration.exported ? '1' : '0'}:${declaration.method ? '1' : '0'}`;
				for (const byte of textEncoder.encode(declarationSignature)) {
					expectedDeclarationMethodSignatureHash =
						(expectedDeclarationMethodSignatureHash +
							byte * declarationMethodSignatureHashPosition) >>>
						0;
					declarationMethodSignatureHashPosition += 1;
				}
				expectedDeclarationMethodSignatureHash =
					(expectedDeclarationMethodSignatureHash +
						0x0a * declarationMethodSignatureHashPosition) >>>
					0;
				declarationMethodSignatureHashPosition += 1;
			}
			if (declarationMethodSignatureHash !== expectedDeclarationMethodSignatureHash) {
				throw new Error(
					'frontend lowered artifact probe declaration method signature hash did not match lowered ir manifest'
				);
			}
			const declarationMethodKindHashExportName = `tinygo_lowered_${symbolID}_declaration_method_kind_hash`;
			const declarationMethodKindHashExportValue =
				exportsObject[declarationMethodKindHashExportName];
			if (typeof declarationMethodKindHashExportValue !== 'function') {
				throw new Error(
					`frontend lowered artifact probe missing export ${declarationMethodKindHashExportName}`
				);
			}
			...
			if (!Number.isInteger(declarationMethodKindHash) || declarationMethodKindHash < 0) {
				throw new Error(
					`frontend lowered artifact probe returned an invalid declaration method kind hash for ${declarationMethodKindHashExportName}`
				);
			}
			...
			for (const declaration of (loweredIRUnit.declarations ?? []).filter(
				...
				for (const byte of textEncoder.encode(declaration.kind ?? '')) {
					expectedDeclarationMethodKindHash =
						(expectedDeclarationMethodKindHash +
							byte * declarationMethodKindHashPosition) >>>
						0;
					declarationMethodKindHashPosition += 1;
				}
				expectedDeclarationMethodKindHash =
					(expectedDeclarationMethodKindHash +
						0x0a * declarationMethodKindHashPosition) >>>
					0;
				declarationMethodKindHashPosition += 1;
			}
			if (declarationMethodKindHash !== expectedDeclarationMethodKindHash) {
				throw new Error(
					'frontend lowered artifact probe declaration method kind hash did not match lowered ir manifest'
				);
			}
		}
		...
		if (typeof mainCountExportValue !== 'function') {
			throw new Error(
				`frontend lowered artifact probe missing export ${mainCountExportName}`
			);
		}
		const mainCount = Number(mainCountExportValue());
		if (!Number.isInteger(mainCount) || mainCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid main count for ${mainCountExportName}`
			);
		}
		...
		if (typeof initCountExportValue !== 'function') {
			throw new Error(
				`frontend lowered artifact probe missing export ${initCountExportName}`
			);
		}
		const initCount = Number(initCountExportValue());
		if (!Number.isInteger(initCount) || initCount < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid init count for ${initCountExportName}`
			);
		}
		...
		for (const sourceFile of unit.sourceFiles) {
			...
			for (const match of sourceText.matchAll(
				...
				for (const byte of textEncoder.encode(match[1])) {
					expectedFunctionNameHash =
						(expectedFunctionNameHash + byte * functionNameHashPosition) >>> 0;
					functionNameHashPosition += 1;
				}
				expectedFunctionNameHash =
					(expectedFunctionNameHash + 0x0a * functionNameHashPosition) >>> 0;
				functionNameHashPosition += 1;
			}
		}
		const functionNameHash =
			typeof functionNameHashExportValue === 'function'
				? Number(functionNameHashExportValue())
				: expectedFunctionNameHash;
		if (!Number.isInteger(functionNameHash) || functionNameHash < 0) {
			throw new Error(
				`frontend lowered artifact probe returned an invalid function name hash for ${functionNameHashExportName}`
			);
		}
		if (
			typeof functionNameHashExportValue === 'function' &&
			functionNameHash !== expectedFunctionNameHash
		) {
			throw new Error(
...
export const verifyTinyGoLoweredObjectFiles = (
	...
	if (
		JSON.stringify(loweredArtifactManifest.objectFiles) !==
		JSON.stringify(loweredObjectFiles.map((file) => file.path))
	) {
	...
	for (const loweredObjectFile of loweredObjectFiles) {
		const bytes =
			typeof loweredObjectFile.bytes === 'string'
				? textEncoder.encode(loweredObjectFile.bytes)
				: loweredObjectFile.bytes;
		if (
			bytes.byteLength < 4 ||
			bytes[0] !== 0x00 ||
			bytes[1] !== 0x61 ||
...
export const verifyTinyGoLoweredBitcodeFiles = (
	...
	if (
		JSON.stringify(expectedBitcodeOutputs) !==
		JSON.stringify(loweredBitcodeFiles.map((file) => file.path))
	) {
	...
	for (const loweredBitcodeFile of loweredBitcodeFiles) {
		const bytes =
			typeof loweredBitcodeFile.bytes === 'string'
				? textEncoder.encode(loweredBitcodeFile.bytes)
				: loweredBitcodeFile.bytes;
		if (
			bytes.byteLength < 4 ||
			bytes[0] !== 0x42 ||
			bytes[1] !== 0x43 ||
...
export const verifyTinyGoFinalArtifactFile = (
	...
	const textEncoder = new TextEncoder();
	const bytes =
		typeof finalArtifactFile.bytes === 'string'
			? textEncoder.encode(finalArtifactFile.bytes)
			: finalArtifactFile.bytes;
	if (
		bytes.byteLength < 4 ||
		bytes[0] !== 0x00 ||
		bytes[1] !== 0x61 ||
