...
export const buildTinyGoUpstreamFrontendProbeWasm = async () => {
	...
	const bridgeWorkDir = await mkdtemp(
		path.join(tmpdir(), 'wasm-tinygo-upstream-frontend-probe-')
	);
	...
	const bridgeProbe = spawnSync(
		process.execPath,
		[new URL('./probe-tinygo-driver-bridge.mjs', import.meta.url).pathname],
		{
			cwd: rootDir,
			env: {
				...process.env,
	...
	const gorootPackageDir = (bridgeManifest.packageGraph ?? [])
		...
		.find(
			(packageDir) =>
				typeof packageDir === 'string' && packageDir.includes(`${path.sep}src${path.sep}`)
		);
	if (!gorootPackageDir) {
		throw new Error(
			'TinyGo upstream frontend asset bridge probe did not report a cached goroot package directory'
		);
	}
	const cachedGoroot = gorootPackageDir.slice(
		0,
		gorootPackageDir.lastIndexOf(`${path.sep}src${path.sep}`)
	);
	...
	await cp(path.join(source.patchedRoot, 'src'), path.join(probeAssetRoot, 'src'), {
		recursive: true
	});
	await replaceDirectoryFromCopy(cachedGoroot, gorootPath, {
		recursive: true,
		dereference: true
	});
...
const run = async () => {
	...
	console.log(
		`Built TinyGo upstream WASI frontend probe at ${path.relative(rootDir, result.outputPath)}`
	);
};
