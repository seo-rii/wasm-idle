...
const runGit = ({ args, cwd, label }) => {
	...
	if (result.error) {
		throw new Error(
			`TinyGo source fetch failed: git is not available (${result.error.message})`
		);
	}
	if (result.status !== 0) {
...
export const resolveTinyGoSourcePaths = () => {
	...
	const sourceUrl =
		process.env.WASM_TINYGO_SOURCE_URL ?? 'https://github.com/tinygo-org/tinygo.git';
	...
	const cacheDir =
		process.env.WASM_TINYGO_SOURCE_CACHE_DIR ?? path.join(rootDir, '.cache', 'tinygo-src');
	return {
		cacheDir,
		sourceRef,
		sourceRootOverride,
...
export const ensureTinyGoSourceReady = async () => {
	...
	if (!lockAcquired) {
		throw new Error(
			`TinyGo source fetch failed: timed out waiting for cache lock at ${sourceLockDir}`
		);
	}
	try {
