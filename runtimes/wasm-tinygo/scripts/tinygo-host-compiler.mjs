...
const resolveTargetInfo = ({ stdout }) => {
	...
	for (const line of stdout.split(/\r?\n/)) {
		...
		if (trimmed.startsWith('build tags:')) {
			targetInfo.buildTags = trimmed
				.slice('build tags:'.length)
				.trim()
				.split(/\s+/)
				.filter(Boolean);
			continue;
	...
	if (result.status !== 0) {
		throw new Error(
			[result.stdout, result.stderr].join('').trim() ||
				`tinygo command failed: ${argv.join(' ')}`
		);
	}
	return result;
};
...
const writeWorkspaceFiles = async ({ files, workDir }) => {
	...
	for (const [relativePath, contents] of Object.entries(files)) {
		...
		if (typeof contents !== 'string') {
			throw new Error(
				`TinyGo host compile requires string file contents for ${relativePath}`
			);
		}
...
export const compileTinyGoHostWorkspace = async ({
	...
	const resolvedWorkDir =
		workDir || (await mkdtemp(path.join(tmpdir(), 'wasm-tinygo-host-compile-')));
	...
	const goCachePath =
		process.env.GOCACHE ?? path.join(paths.rootDir, '.cache', 'tinygo-go-build');
...
export const buildTinyGoHostProbe = async ({
	...
	const paths = await ensureTinyGoToolchainReady();
	const resolvedWorkDir =
		workDir || (await mkdtemp(path.join(tmpdir(), 'wasm-tinygo-host-probe-')));
	const target = request?.target ?? 'wasip1';
	const mainPath = request?.entry ?? path.join(resolvedWorkDir, 'main.go');
	const resolvedOutputPath =
		outputPath || request?.output || path.join(resolvedWorkDir, 'main.wasm');
	const goCachePath =
		process.env.GOCACHE ?? path.join(paths.rootDir, '.cache', 'tinygo-go-build');
...
`
	...
	if (!skipRuntime && target === 'wasip1') {
		...
		if (expectedRuntimeLogs && JSON.stringify(logs) !== JSON.stringify(expectedRuntimeLogs)) {
			throw new Error(
				`TinyGo host probe runtime logs did not match expectation: ${JSON.stringify(logs)}`
			);
		}
