...
const buildDefaultArchiveFileName = ({ archiveType, arch, platform, version }) => {
	...
	throw new Error(
		`unsupported TinyGo release archive for platform=${platform} archiveType=${archiveType} arch=${arch}`
	);
};
	...
	if (archiveType === 'tar.gz' || archiveType === 'zip') {
		return {
			binPath: path.join(
				...
				'tinygo',
				'bin',
				platform === 'win32' ? 'tinygo.exe' : 'tinygo'
			),
...
export const resolveTinyGoToolchainPaths = () => {
	...
	if (!arch) {
		throw new Error(
			`unsupported TinyGo release architecture for platform=${platform} archiveType=${archiveType} process.arch=${process.arch}`
		);
	}
	const cacheDir =
		process.env.WASM_TINYGO_TINYGO_CACHE_DIR ??
		path.join(rootDir, '.cache', 'tinygo-toolchain');
	...
	const releaseUrl =
		releaseUrlOverride ??
		`https://github.com/tinygo-org/tinygo/releases/download/v${version}/${archiveFileName}`;
