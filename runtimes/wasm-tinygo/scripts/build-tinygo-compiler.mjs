...
const classifyTinyGoCompilerBlockers = (details) => {
	...
	push(
		'tinygo-cgo',
		/github\.com\/tinygo-org\/tinygo\/cgo|cgo\/cgo\.go:.*clangCursor|cgo\/libclang\.go/i
	);
	return blockers;
};
...
const runGo = ({ argv, cwd, env }) => {
	...
	if (result.error) {
		throw new Error(
			`TinyGo wasm compiler build failed: go is not available (${result.error.message})`
		);
	}
	if (result.status !== 0) {
...
const resolveCompilerPaths = async () => {
	...
	const defaultMainPath =
		compilerGoos === 'wasip1' || compilerGoarch === 'wasm' ? 'cmd/tinygo-wasi' : '.';
...
export const buildTinyGoCompilerWasm = async () => {
	...
	if (initialBuild.status !== 0) {
		...
		const resolvedPatchCommandPath = path.join(
			source.rootPath,
			patch.commandPath.replace(/^\.\//, '')
		);
		...
		const canRetryPatchedMainPath =
			...
			(resolvedPatchProbeCommandPath === null ||
				resolvedMainPath !== resolvedPatchProbeCommandPath);
		if (canRetryPatchedMainPath) {
			...
			if (patchedDirectBuild.status === 0) {
				...
				patchedDirectFailureReason = [patchedDirectBuild.stdout, patchedDirectBuild.stderr]
					.join('')
					.trim();
			}
		}
		if (buildMode === 'patched-upstream-direct') {
			...
			if (patchedBuild.status === 0) {
				...
				patchedEntryFailureReason = [patchedBuild.stdout, patchedBuild.stderr]
					.join('')
					.trim();
				...
				runGo({
					argv: [
						...
						'-o',
						outputPath,
						patch.probeCommandPath ?? patch.commandPath
					],
					cwd: source.rootPath,
					env
				});
