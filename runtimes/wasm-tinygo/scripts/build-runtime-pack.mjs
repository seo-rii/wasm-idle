...
const PUBLIC_DIR = process.env.WASM_TINYGO_RUNTIME_PACK_ROOT ?? path.join(ROOT_DIR, 'public');
const OUTPUT_DIR =
	process.env.WASM_TINYGO_RUNTIME_PACK_OUTPUT ?? path.join(PUBLIC_DIR, 'runtime-pack');
	...
	const root =
		typeof raw.root === 'string' && raw.root.length > 0
			? path.isAbsolute(raw.root)
				? raw.root
				: path.resolve(manifestDir, raw.root)
			: null;
...
const normalizeManifestEntry = (entry, index, manifestDir) => {
	...
	const resolvedPath = path.isAbsolute(filePath) ? filePath : path.resolve(manifestDir, filePath);
	return {
		runtimePath,
