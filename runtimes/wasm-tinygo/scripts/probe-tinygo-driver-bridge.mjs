...
const workDir =
	process.env.WASM_TINYGO_DRIVER_BRIDGE_WORK_DIR ??
	(await mkdtemp(path.join(tmpdir(), 'wasm-tinygo-driver-bridge-')));
const entryPath = process.env.WASM_TINYGO_DRIVER_BRIDGE_ENTRY_PATH ?? path.join(workDir, 'main.go');
const outputPath =
	process.env.WASM_TINYGO_DRIVER_BRIDGE_OUTPUT_PATH ?? path.join(workDir, 'main.wasm');
const requestPath =
...
const resultPath =
	process.env.WASM_TINYGO_DRIVER_BRIDGE_RESULT_PATH ??
	path.join(path.dirname(requestPath), 'tinygo-result.json');
...
try {
	...
	await writeFile(
		path.join(path.dirname(entryPath), 'go.mod'),
		`module example.com/wasm-tinygo/hostprobe
...
const hostProbe = spawnSync(
	process.execPath,
	[fileURLToPath(new URL('./probe-tinygo-host.mjs', import.meta.url))],
	{
		cwd: repoRoot,
		env: {
			...process.env,
...
if (packageList.stdout.trim() !== '') {
	...
	for (let index = 0; index < packageList.stdout.length; index += 1) {
		...
		if (character === '}') {
			...
			if (depth === 0 && currentObjectStart >= 0) {
				parsedPackages.push(
					JSON.parse(packageList.stdout.slice(currentObjectStart, index + 1))
				);
	...
	const resolvedEntryPackage =
		packageGraph.find(
			(packageInfo) => !packageInfo.depOnly && (packageInfo.importPath ?? '') !== ''
...
const frontendInputFile = (driverResult.files ?? []).find(
	(file) => file.path === '/working/tinygo-frontend-input.json'
);
if (!frontendInputFile) {
	throw new Error(
		'native driver did not produce /working/tinygo-frontend-input.json for bridge verification'
	);
}
...
const frontendProgramPackage = (frontendInput.packageGraph ?? []).find(
	(packageInfo) => !(packageInfo.depOnly ?? false) && (packageInfo.importPath ?? '') !== ''
);
...
const canonicalPackageGraphSource =
	(packageGraph ?? []).length === 0
		...
		: packageGraph.map((packageInfo) => {
				let frontendPackageInfo = frontendPackageGraphByImportPath.get(
				...
				return {
					...
					importPath: packageInfo.importPath ?? '',
					imports: [...(packageInfo.imports ?? [])].filter(
...
const canonicalPackageGraph = canonicalPackageGraphSource
	.map((packageInfo, index) => {
		...
		if (
			...
			orderIndex = frontendPackageOrderByImportPath.get(
				frontendProgramPackage.importPath ?? ''
			);
		}
		return {
			depOnly: Boolean(packageInfo.depOnly),
	...
	.filter((packageInfo) => {
		const packageFiles = (packageInfo.files?.goFiles ?? []).map((goFile) =>
			path.join(packageInfo.dir ?? '', goFile)
		);
		return (
			packageFiles.length !== 0 &&
			packageFiles.every((filePath) => allCompileFileSet.has(filePath))
		);
	})
	.sort((left, right) => {
		if (
	...
	.map(
		({ orderIndex: _orderIndex, originalIndex: _originalIndex, ...packageInfo }) => packageInfo
	);
...
await writeFile(
	frontendInputPath,
	`${JSON.stringify(canonicalFrontendInput, null, 2)}
`
);
...
await writeFile(
	frontendAnalysisInputPath,
	`${JSON.stringify(frontendAnalysisInput, null, 2)}
`
);
...
if (includeFrontendAnalysis) {
	...
	if (!frontendAnalysisResult.ok || !frontendAnalysisResult.analysis) {
		throw new Error(
			'frontend analysis bridge verification did not produce a normalized analysis payload'
		);
	}
}
...
if (!frontendRealAdapterResult.ok || !frontendRealAdapterResult.adapter) {
	throw new Error(
		'frontend real adapter bridge verification did not produce a normalized adapter payload'
	);
}
...
const analysisPackageGraph = (
	...
	.filter(
		(packageInfo) =>
			(packageInfo.importPath ?? '') !== '' || (packageInfo.goFiles ?? []).length !== 0
	);
...
const compileUnitManifestFile = (frontendResult.generatedFiles ?? []).find(
	(file) => file.path === '/working/tinygo-compile-unit.json'
);
if (!compileUnitManifestFile) {
	throw new Error(
		'frontend bridge verification did not produce /working/tinygo-compile-unit.json'
	);
}
...
if (!entryPackage || (entryPackage.importPath ?? '') === '') {
	const programCompileUnit = (compileUnitManifest.compileUnits ?? []).find(
	...
	if (programCompileUnit) {
		...
		entryPackage = {
			...
			imports: [...(programCompileUnit.imports ?? [])].filter(
				(importPath) => importPath !== ''
			),
...
if (packageGraph && packageGraph.length > 0 && canonicalAnalysisProgramPackage) {
	packageGraph = packageGraph.map((packageInfo) => {
		if (
			!packageInfo.depOnly &&
			((packageInfo.importPath ?? '') === '' ||
				packageInfo.importPath === 'command-line-arguments')
		) {
			return {
...
if (!packageGraph || packageGraph.length === 0) {
	if (analysisPackageGraph.length > 0) {
		...
		packageGraph = (compileUnitManifest.compileUnits ?? [])
			.map((compileUnit) => {
				...
				return {
					...
					imports: [...(compileUnit.imports ?? [])].filter(
						(importPath) => importPath !== ''
					),
					modulePath:
	...
	const expectedFrontendAnalysisCompileUnits =
		canonicalPackageGraph.length === 0
			...
			: (frontendAnalysisResult.analysis?.compileUnits ?? []).map((compileUnit) => {
					...
					const packageInfo =
						canonicalPackageGraph.find(
							(candidate) => (candidate.importPath ?? '') === compileUnitImportPath
						) ??
						((compileUnit.kind ?? '') === 'program' &&
						compileUnitImportPath === 'command-line-arguments'
							? canonicalPackageGraph.find(
									(candidate) =>
										!candidate.depOnly && (candidate.importPath ?? '') !== ''
								)
							: undefined);
					return {
						...compileUnit,
						imports: packageInfo
							? [...(packageInfo.imports ?? [])].filter(
									(importPath) => importPath !== ''
								)
							: [...(compileUnit.imports ?? [])]
					};
		...
		canonicalPackageGraph.length === 0
			...
			: (frontendAnalysisResult.analysis?.packageGraph ?? []).map((packageInfo) => {
					...
					const canonicalPackageInfo =
						canonicalPackageGraph.find(
							(candidate) => (candidate.importPath ?? '') === packageImportPath
						) ??
					...
					return {
						...
						imports: canonicalPackageInfo
							? [...(canonicalPackageInfo.imports ?? [])].filter(
									(importPath) => importPath !== ''
								)
							: [...(packageInfo.imports ?? [])]
					};
...
if (includeUpstreamFrontendProbe) {
	...
	for (const packageInfo of packageGraph ?? []) {
		...
		for (const goFile of packageInfo.goFiles ?? []) {
			...
			const relativeWorkspacePath = path
				.relative(workspaceRoot, hostFilePath)
				.split(path.sep)
				.join('/');
	...
	const packageList = normalizedProbePackageGraph.map((packageInfo) => {
		const isWorkspacePackage =
			(packageInfo.dir ?? '') === '/workspace' ||
			(packageInfo.dir ?? '').startsWith('/workspace/');
		return {
			DepOnly: Boolean(packageInfo.depOnly),
			Dir: packageInfo.dir ?? '',
			ImportPath: packageInfo.importPath ?? '',
