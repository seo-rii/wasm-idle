...
const RUNTIME_FILES = ['BQN.js', 'BQN.wasm'];
const LICENSE_FILE_NAME = 'LICENSE-GPLv3.txt';
...
async function targetLooksUsable(targetDir) {
	return (
		(await fileExists(path.join(targetDir, 'BQN.js'))) &&
		(await targetRuntimeFileExists(targetDir, 'BQN.wasm'))
	);
}
...
async function writeVersionModule(versionModulePath, fingerprint) {
	...
	const moduleSource = `export const WASM_BQN_ASSET_VERSION = ${JSON.stringify(fingerprint)};\n`;
...
async function resolveSourceDir(sourceDir, targetDir) {
	...
	throw new Error(
		`CBQN runtime assets were not found. Set WASM_BQN_SOURCE_DIR or pass a source dir containing ${RUNTIME_FILES.join(
			', '
		)}.`
	);
}
 ...
 */
export async function syncWasmBqnAssets({
	sourceDir,
	...
	if (resolvedSourceDir) {
		const moduleSource = await readFile(path.join(resolvedSourceDir, 'BQN.js'), 'utf8');
		if (
	...
	if (!RUNTIME_FILES.every((file) => copiedFiles.includes(file) || copiedFiles.includes(`${file}.gz`))) {
		throw new Error(`BQN runtime target is missing one of: ${RUNTIME_FILES.join(', ')}`);
	}
