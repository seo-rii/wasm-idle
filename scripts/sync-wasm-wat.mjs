...
const DEFAULT_SOURCE_DIR = path.resolve(REPO_ROOT, 'runtimes', 'wasm-wat', 'dist');
const DEFAULT_TARGET_DIR = path.resolve(REPO_ROOT, 'static', 'wasm-wat');
...
async function writeVersionModule(versionModulePath, fingerprint) {
	...
	const moduleSource = `export const WASM_WAT_ASSET_VERSION = ${JSON.stringify(fingerprint)};\n`;
...
export async function syncWasmWatDist({
	...
	if (!sourceStats?.isDirectory()) {
		throw new Error(
			`wasm-wat dist directory was not found at ${sourceDir}. Build wasm-wat first with "pnpm --dir runtimes/wasm-wat build".`
		);
	}
	...
	if (!entryModuleStats?.isFile()) {
		throw new Error(`wasm-wat dist entry was not found at ${entryModulePath}.`);
	}
...
if (process.argv[1] && path.resolve(process.argv[1]) === THIS_FILE) {
	...
	const { sourceDir, targetDir } = await syncWasmWatDist({
		sourceDir: sourceDirArg || DEFAULT_SOURCE_DIR,
		targetDir: targetDirArg || DEFAULT_TARGET_DIR
	});

	console.log(`Synced wasm-wat from ${sourceDir} to ${targetDir}`);
}
