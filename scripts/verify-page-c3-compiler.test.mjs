...
test('c3 page command rejects a fresh checkout without required producer input', async (t) => {
	const rootDir = await mkdtemp(path.join(os.tmpdir(), 'c3-page-missing-'));
	t.after(() => rm(rootDir, { recursive: true, force: true }));
	const pkg = JSON.parse(await readFile(new URL('../package.json', import.meta.url)));
	const step = pkg.scripts['page:build']
		.split(' && ')
		.find((value) => value === 'pnpm run verify:page-c3 build');
	assert.ok(step);
	...
	assert.match(result.stderr, /sync:wasm-c3.*before page:build/);
...
test('c3 page build and direct publish validate required input and final output', async () => {
	...
	const verification = publish.indexOf('await verifyPageC3Compiler({ rootDir: buildDir })');
