...
const sources = [
    {
        name: 'Main.cs',
        content: fs.readFileSync(path.join(__dirname, 'test.cs'), 'utf-8'),
    },
    {
        name: 'Helper.cs',
        content: fs.readFileSync(path.join(__dirname, 'helper.cs'), 'utf-8'),
    },
]
...
async function main() {
    ...
    const tmpPath = fs.mkdtempSync(path.join(os.tmpdir(), 'dool-csharp-judge-'))
    let cleanupUser = ''
    try {
        for (const source of sources) {
            fs.writeFileSync(path.join(tmpPath, source.name), source.content, {mode: 0o644})
        }

        const buildCommand = csharp.build(tmpPath, randomUUID(), sources.map((source) => source.name), 'Main')
        const runCommand =
            `ulimit -v ${csharp.getMemoryLimit(4) * 1024};` +
            'ulimit -s unlimited;' +
            'ts=$(date +%s%N);' +
            `/usr/bin/time -f "__DOOL_MAXRSS_KB__=%M" ${csharp.getExecuteCommand(tmpPath, randomUUID(), 'Main')};` +
            'ec=$?;' +
