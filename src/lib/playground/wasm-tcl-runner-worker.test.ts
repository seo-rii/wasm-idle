...
const wacl = { onReady: (callback) => callback(interp) };
function requirejs(_modules, resolve) { resolve(wacl); }
requirejs.config = () => {};
...
describe('Wacl Tcl runner worker', () => {
	it('keeps the synced worker and cache-busting fingerprint current', async () => {
		...
		const compressedFiles = new Set(['tcl/wacl-library.data', 'tcl/wacl.wasm']);
		...
		for (const fileName of [
			...
			hash.update('\0');
			const compressed = compressedFiles.has(fileName);
		...
		expect(hash.digest('hex').slice(0, 16)).toBe(WASM_TCL_ASSET_VERSION);
	...
	it('prints a prompt before reading live shared-ring input', async () => {
		...
		const messages = await runHarness(
			{
				baseUrl: 'https://runtime.example/wasm-tcl/',
				code: 'gets stdin line',
	...
	it('fails closed on a malformed shared stdin descriptor', async () => {
		...
		expect(messages).toEqual([{ error: 'Invalid Wacl Tcl streaming stdin channel.' }]);
	});
});
