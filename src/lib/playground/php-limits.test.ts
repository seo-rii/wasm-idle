...
describe('PHP execution message limits', () => {
	beforeEach(() => {
	...
	it('terminates output before exceeding the cumulative UTF-8 byte limit', async () => {
		...
		const running = sandbox.run('<?php echo "ok";', false, true, undefined, [], {
		...
		const retry = sandbox.run('<?php echo "retry";', false);
	...
	it('counts diagnostics even when no callback is registered', async () => {
		...
		await sandbox.load('/assets');
		const retiredWorker = workerInstances[0];
		const running = sandbox.run('<?php invalid', false, true, undefined, [], {
	...
	it('retires the quota owner before signal cleanup can start a replacement', async () => {
			...
			removeEventListener() {
				...
				if (listenerRemovals === 2) {
					replacement = sandbox.load({
						php: { moduleUrl: '/replacement/php.mjs' }
					});
				}
			}
		} as unknown as AbortSignal;
		const running = sandbox.run('<?php echo "large";', false, true, undefined, [], {
			signal,
		...
		expect(replacementWorker.postMessage).toHaveBeenCalledWith({
			load: true,
			moduleUrl: 'http://localhost:3000/replacement/php.mjs',
			log: true
		});
