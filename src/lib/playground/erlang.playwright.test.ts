...
describe('wasm-idle Erlang browser playwright integration', () => {
	it(
		'runs Erlang through the Popcorn/AtomVM wasm runtime and connects stdin on the page path',
		{
			skip: process.env.WASM_IDLE_RUN_REAL_BROWSER_ERLANG !== '1',
			meta: {
				browser: true,
				requiredBrowser: !(process.env.WASM_IDLE_RUN_REAL_BROWSER_ERLANG !== '1')
			},
			timeout: 960_000
		},
		async () => {
			...
			await runWithBrowserProbeSessionLock(async () => {
				...
				if (!reuseProvidedBrowserUrl && serverMode === 'preview') {
					await runBrowserPreparationScripts(
						...
						{
							timeoutMs: Number(
								process.env.WASM_IDLE_ERLANG_PREP_TIMEOUT_MS || '900000'
							)
						}
					);
				}
				...
				try {
					const summary = await runStdinBrowserProbe({
						...
						language: 'ERLANG',
						runTimeoutMs: Number(
							process.env.WASM_IDLE_ERLANG_RUN_TIMEOUT_MS || '240000'
						),
