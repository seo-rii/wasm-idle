		...
		async () => {
			...
			await runWithBrowserProbeSessionLock(async () => {
				...
				const runtimeManifest = JSON.parse(
					await readFile(
						new URL(
							'../../../static/wasm-go/runtime/runtime-manifest.v1.json',
							import.meta.url
						),
						'utf8'
					)
					...
					for (const target of expectedGoTargets) {
						...
						expect(
							summary.consoleTail.some((entry: string) =>
								entry.includes(
									`[wasm-idle:go-worker] compile start prepare=true target=${target}`
								)
							)
						).toBe(true);
						expect(
						...
						expect(
							summary.consoleTail.some((entry: string) =>
								entry.includes(
									`[wasm-idle:go-worker] runtime start target=${target}`
								)
							)
						).toBe(true);
						expect(
