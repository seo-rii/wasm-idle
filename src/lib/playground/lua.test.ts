...
describe('Lua sandbox', () => {
	...
	it('terminates Lua output before exceeding the cumulative UTF-8 byte limit', async () => {
		const sandbox = new Lua();
		const output = vi.fn();
	...
	it('terminates Lua diagnostics before exceeding the message limit', async () => {
		const sandbox = new Lua();
	...
	it('keeps clear reusable but disposes an idle Lua runtime exactly once', async () => {
		const sandbox = new Lua();
		const output = vi.fn();
		const diagnostic = vi.fn();
		sandbox.output = output;
		...
		await sandbox.clear();
		expect(worker.terminate).not.toHaveBeenCalled();
		await expect(sandbox.run('print("reusable")', false)).resolves.toBe(true);
		sandbox.write('queued input\n');
		sandbox.eof();
		expect(sandbox.pendingInput).toEqual(['queued input\n']);
		expect(sandbox.pendingEof).toBe(true);
		...
		await expect(sandbox.run('print("unavailable")', false)).rejects.toMatchObject({
			name: 'RuntimeConfigurationError',
			code: 'runtime-configuration',
			phase: 'dispose',
			runtimeId: 'LUA'
	...
	it('settles an active Lua run, clears stdin, and ignores retained messages after disposal', async () => {
		const sandbox = new Lua();
		const output = vi.fn();
		const diagnostic = vi.fn();
		sandbox.output = output;
