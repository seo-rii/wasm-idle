...
describe('Haskell sandbox', () => {
	...
	it('terminates Haskell output before exceeding the cumulative UTF-8 byte limit', async () => {
		const sandbox = new Haskell();
		const output = vi.fn();
	...
	it('terminates Haskell diagnostics before exceeding the message limit', async () => {
		const sandbox = new Haskell();
	...
	it('keeps clear reusable but disposes an idle Haskell runtime exactly once', async () => {
		const sandbox = new Haskell();
		const output = vi.fn();
		const diagnostic = vi.fn();
		sandbox.output = output;
		...
		expect(sandbox.worker).toBeUndefined();
		expect(sandbox.runtimeKey).toBe('');
	...
	it('settles an active Haskell run, clears stdin, and ignores retained messages after disposal', async () => {
		const sandbox = new Haskell();
		const output = vi.fn();
		const diagnostic = vi.fn();
		sandbox.output = output;
