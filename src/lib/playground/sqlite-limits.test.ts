...
describe('SQLite execution message limits', () => {
	beforeEach(() => {
	...
	it('retires the quota owner before signal cleanup can start a replacement', async () => {
			...
			removeEventListener() {
				...
				if (listenerRemovals === 2) {
					replacement = sandbox.load({
						sqlite: {
							moduleUrl: '/replacement/sqlite.js',
							wasmUrl: '/replacement/sql-wasm.wasm'
						}
					});
				}
			}
		...
		expect(replacementWorker.postMessage).toHaveBeenCalledWith({
			load: true,
			moduleUrl: 'http://localhost:3000/replacement/sqlite.js',
			wasmUrl: 'http://localhost:3000/replacement/sql-wasm.wasm',
			log: true
		});
