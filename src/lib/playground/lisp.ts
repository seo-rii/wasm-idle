...
const abortReason = (signal: AbortSignal, phase: LispOperation['phase']) =>
	signal.reason !== undefined
		? signal.reason
		: new DOMException(
				phase === 'startup' ? 'Lisp runtime startup aborted' : 'Lisp execution aborted',
				'AbortError'
			);

class Lisp implements Sandbox {
	output: any = null;
	...
	pendingEof = false;
	private activeOperation: LispOperation | null = null;
	private disposed = false;
	private disposePromise: Promise<void> | null = null;
	private readonly disposeCancellation = new CancelledError('Lisp sandbox disposed', {
		phase: 'dispose',
		runtimeId: 'LISP',
		recoverable: false
	});
	...
	private requireOperationIdle() {
		...
		throw new BusyError('Lisp runtime already has an active operation', {
			runtimeId: 'LISP',
			phase: this.activeOperation.phase
		});
	}

	private beginOperation(phase: LispOperation['phase']) {
		if (this.disposed) {
			throw new RuntimeConfigurationError('Lisp sandbox is disposed', {
				phase: 'dispose',
				runtimeId: 'LISP'
			});
		}
		this.requireOperationIdle();
		const operation: LispOperation = {
			token: Symbol(phase),
			phase,
			cancelled: false,
			cleanedUp: false,
	...
	private isOperationActive(operation: LispOperation) {
		return this.activeOperation?.token === operation.token && !operation.cancelled;
	}

	private releaseOperation(operation: LispOperation) {
		if (this.activeOperation?.token !== operation.token) return false;
		this.activeOperation = null;
		return true;
	}

	private cleanupOperation(operation: LispOperation) {
	...
	private releaseBeforeSession(operation: LispOperation, reason: unknown) {
	...
	private bindPreSessionAbort(operation: LispOperation, signal: AbortSignal | undefined) {
	...
	private bindAbortSignal(operation: LispOperation, signal: AbortSignal | undefined) {
	...
	private bindOperationTimeout(operation: LispOperation, timeoutMs: number) {
		if (!this.isOperationActive(operation)) return;
		...
		try {
			timeout = setTimeout(() => {
				...
				this.cancelOperation(
					operation,
					new TimeoutError(`Lisp ${label} timed out after ${timeoutMs} ms`, {
	...
	private finishExplicitStdin(operation: LispOperation) {
	...
	load(
		...
		let activeOperation: LispOperation;
		try {
		...
		try {
			...
			if (!this.isOperationActive(activeOperation) || signal?.aborted) {
				return Promise.reject(
					this.releaseBeforeSession(activeOperation, 'Lisp runtime startup cancelled')
				);
			}
			...
			handler = (event: Event & { data: any }) => {
				...
				try {
					...
					if (buffer && !hasExplicitStdin) {
						...
						if (!prepare) {
							reportWorkerInputReady(_prog, 'Lisp runtime ready for input');
							if (!ownsRun()) return;
						}
						this.flushPendingInput();
						if (!ownsRun()) return;
						...
						if (actual > limits.maxOutputBytes) {
							failRun(
								new OutputLimitError(
									`Lisp output exceeded ${limits.maxOutputBytes} bytes`,
									{
										actual,
						...
						if (actual > limits.maxDiagnostics) {
							failRun(
								new DiagnosticLimitError(
									`Lisp diagnostics exceeded ${limits.maxDiagnostics} messages`,
									{
										actual,
	...
	private cancelOperation(operation: LispOperation, reason: unknown) {
		if (!this.isOperationActive(operation)) return;
		operation.cancelled = true;
		operation.cancellationReason = reason;
		this.activeOperation = null;
