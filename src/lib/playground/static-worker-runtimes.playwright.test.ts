...
describe('wasm-idle static worker language browser integrations', () => {
	it(
		'runs real SWI-Prolog wasm and connects stdin on the page path',
		{
			skip: process.env.WASM_IDLE_RUN_REAL_BROWSER_PROLOG !== '1',
			meta: {
				browser: true,
				requiredBrowser: !(process.env.WASM_IDLE_RUN_REAL_BROWSER_PROLOG !== '1')
			},
			timeout: 960_000
		},
		async () => {
			...
			await withPreviewServer(
				...
				async (browserUrl) => {
					const summary = await runStdinBrowserProbe({
						...
						language: 'PROLOG',
						runTimeoutMs: Number(
							process.env.WASM_IDLE_PROLOG_RUN_TIMEOUT_MS || '240000'
						),
	...
	it(
		'runs real Gleam wasm compiler output and connects stdin on the page path',
		{
			skip: process.env.WASM_IDLE_RUN_REAL_BROWSER_GLEAM !== '1',
			meta: {
				browser: true,
				requiredBrowser: !(process.env.WASM_IDLE_RUN_REAL_BROWSER_GLEAM !== '1')
			},
			timeout: 960_000
		},
		async () => {
			...
			await withPreviewServer(
				...
				async (browserUrl) => {
					const summary = await runStdinBrowserProbe({
						...
						language: 'GLEAM',
						runTimeoutMs: Number(
							process.env.WASM_IDLE_GLEAM_RUN_TIMEOUT_MS || '240000'
						),
	...
	it(
		'runs real WebPerl wasm and connects stdin on the page path',
		{
			skip: process.env.WASM_IDLE_RUN_REAL_BROWSER_PERL !== '1',
			meta: {
				browser: true,
				requiredBrowser: !(process.env.WASM_IDLE_RUN_REAL_BROWSER_PERL !== '1')
			},
			timeout: 960_000
		},
		async () => {
	...
	it(
		'runs real Wacl Tcl wasm and connects stdin on the page path',
		{
			skip: process.env.WASM_IDLE_RUN_REAL_BROWSER_TCL !== '1',
			meta: {
				browser: true,
				requiredBrowser: !(process.env.WASM_IDLE_RUN_REAL_BROWSER_TCL !== '1')
			},
			timeout: 960_000
		},
		async () => {
	...
	it(
		'runs real GoAWK wasm and connects stdin on the page path',
		{
			skip: process.env.WASM_IDLE_RUN_REAL_BROWSER_AWK !== '1',
			meta: {
				browser: true,
				requiredBrowser: !(process.env.WASM_IDLE_RUN_REAL_BROWSER_AWK !== '1')
			},
			timeout: 960_000
		},
		async () => {
	...
	it(
		'runs real pas2js Pascal assets and connects stdin on the page path',
		{
			skip: process.env.WASM_IDLE_RUN_REAL_BROWSER_PASCAL !== '1',
			meta: {
				browser: true,
				requiredBrowser: !(process.env.WASM_IDLE_RUN_REAL_BROWSER_PASCAL !== '1')
			},
			timeout: 960_000
		},
		async () => {
			...
			await withPreviewServer(
				...
				async (browserUrl) => {
					const summary = await runStdinBrowserProbe({
						...
						language: 'PASCAL',
						runTimeoutMs: Number(
							process.env.WASM_IDLE_PASCAL_RUN_TIMEOUT_MS || '240000'
						),
	...
	it(
		'runs the real self-hosted ClojureScript compiler and connects stdin',
		{
			skip: process.env.WASM_IDLE_RUN_REAL_BROWSER_CLOJURESCRIPT !== '1',
			meta: {
				browser: true,
				requiredBrowser: !(process.env.WASM_IDLE_RUN_REAL_BROWSER_CLOJURESCRIPT !== '1')
			},
			timeout: 960_000
		},
		async () => {
			...
			await withPreviewServer(
				...
				async (browserUrl) => {
					...
					expect(
						runtimePaths.some((path) =>
							path.endsWith('/wasm-clojurescript/compiler.js.gz')
						)
					).toBe(false);
					expect(
						runtimePaths.some((path) =>
							path.endsWith('/wasm-clojurescript/compiler.js')
						)
					).toBe(false);
				}
			);
	...
	it(
		'runs real WAForth wasm and connects stdin on the page path',
		{
			skip: process.env.WASM_IDLE_RUN_REAL_BROWSER_FORTH !== '1',
			meta: {
				browser: true,
				requiredBrowser: !(process.env.WASM_IDLE_RUN_REAL_BROWSER_FORTH !== '1')
			},
			timeout: 960_000
		},
		async () => {
			...
			await withPreviewServer(
				...
				async (browserUrl) => {
					const summary = await runStdinBrowserProbe({
						...
						language: 'FORTH',
						runTimeoutMs: Number(
							process.env.WASM_IDLE_FORTH_RUN_TIMEOUT_MS || '240000'
						),
	...
	it(
		'runs real J playground wasm and connects stdin on the page path',
		{
			skip: process.env.WASM_IDLE_RUN_REAL_BROWSER_J !== '1',
			meta: {
				browser: true,
				requiredBrowser: !(process.env.WASM_IDLE_RUN_REAL_BROWSER_J !== '1')
			},
			timeout: 960_000
		},
		async () => {
	...
	it(
		'runs real CBQN wasm and connects stdin on the page path',
		{
			skip: process.env.WASM_IDLE_RUN_REAL_BROWSER_BQN !== '1',
			meta: {
				browser: true,
				requiredBrowser: !(process.env.WASM_IDLE_RUN_REAL_BROWSER_BQN !== '1')
			},
			timeout: 960_000
		},
		async () => {
	...
	it(
		'runs real Janet VM wasm and connects stdin on the page path',
		{
			skip: process.env.WASM_IDLE_RUN_REAL_BROWSER_JANET !== '1',
			meta: {
				browser: true,
				requiredBrowser: !(process.env.WASM_IDLE_RUN_REAL_BROWSER_JANET !== '1')
			},
			timeout: 960_000
		},
		async () => {
			...
			await withPreviewServer(
				...
				async (browserUrl) => {
					const summary = await runStdinBrowserProbe({
						...
						language: 'JANET',
						runTimeoutMs: Number(
							process.env.WASM_IDLE_JANET_RUN_TIMEOUT_MS || '240000'
						),
	...
	it(
		'runs real Julia wasm and connects stdin on the page path',
		{
			skip: process.env.WASM_IDLE_RUN_REAL_BROWSER_JULIA !== '1',
			meta: {
				browser: true,
				requiredBrowser: !(process.env.WASM_IDLE_RUN_REAL_BROWSER_JULIA !== '1')
			},
			timeout: 960_000
		},
		async () => {
			...
			await withPreviewServer(
				...
				async (browserUrl) => {
					const summary = await runStdinBrowserProbe({
						...
						language: 'JULIA',
						runTimeoutMs: Number(
							process.env.WASM_IDLE_JULIA_RUN_TIMEOUT_MS || '240000'
						),
	...
	it(
		'runs real Nim wasm compiler output and connects stdin on the page path',
		{
			skip: process.env.WASM_IDLE_RUN_REAL_BROWSER_NIM !== '1',
			meta: {
				browser: true,
				requiredBrowser: !(process.env.WASM_IDLE_RUN_REAL_BROWSER_NIM !== '1')
			},
			timeout: 960_000
		},
		async () => {
			...
			await withPreviewServer(
				...
				async (browserUrl) => {
					...
					const runnerRequestIndex = nimRuntimePaths.indexOf(
						'/wasm-nim/runner-worker.js'
					);
