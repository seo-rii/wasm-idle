...
class D implements Sandbox {
	...
	private readonly disposeCancellation = new CancelledError('D sandbox disposed', {
		phase: 'dispose',
		runtimeId: 'D',
		recoverable: false
	});
	...
	private beginOperation(phase: DOperation['phase']) {
		if (this.disposed) {
			throw new RuntimeConfigurationError('D sandbox is disposed', {
				phase: 'dispose',
				runtimeId: 'D'
			});
		}
	...
	private bindOperationTimeout(operation: DOperation, timeoutMs: number) {
		if (!this.isOperationActive(operation)) return;
		...
		try {
			timeout = setTimeout(() => {
				...
				this.cancelOperation(
					operation,
					new TimeoutError(`D ${label} timed out after ${timeoutMs} ms`, {
						...
						if (actual > limits.maxOutputBytes) {
							failRun(
								new OutputLimitError(
									`D output exceeded ${limits.maxOutputBytes} bytes`,
									{
										actual,
						...
						if (actual > limits.maxDiagnostics) {
							failRun(
								new DiagnosticLimitError(
									`D diagnostics exceeded ${limits.maxDiagnostics} messages`,
									{
										actual,
	...
	dispose() {
		...
		this.moduleUrl = '';
		this.manifestUrl = '';
		this.outerIntegrity = undefined;
		this.activeDAssetsKey = '';
		this.output = null;
