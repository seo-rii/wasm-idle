...
describe('wasm-idle rust browser playwright integration', () => {
	...
	it(
		...
		{
			skip: process.env.WASM_IDLE_RUN_REAL_BROWSER_RUST !== '1',
			meta: {
				browser: true,
				requiredBrowser: !(process.env.WASM_IDLE_RUN_REAL_BROWSER_RUST !== '1')
			},
			timeout: 780_000
		},
		async () => {
			...
			await runWithBrowserProbeSessionLock(async () => {
				...
				for (const targetTriple of expectedRustTargets) {
					...
					const targetRuns = Number(
						configuredRepeats ?? defaultBrowserRepeats[targetTriple]
					);
						...
						try {
							...
							if (targetTriple === 'wasm32-wasip2') {
								expect(summary.transcript).toContain(
									'preview2_component=preview2-cli'
								);
							}
