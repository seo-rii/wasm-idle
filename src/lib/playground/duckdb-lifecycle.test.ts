...
describe('DuckDB worker lifecycle', () => {
	beforeEach(() => {
	...
	it('ignores a retained handler after terminate and reload', async () => {
		...
		const terminationReason = new Error('replace DuckDB execution');
	...
	it.each(['output', 'diagnostic', 'progress'] as const)(
		...
		async (callbackKind) => {
			autoResolveRun = false;
			const sandbox = new DuckDB();
			const callbackError = new Error(`DuckDB ${callbackKind} callback failed`);
	...
	it('settles a throwing startup progress callback and permits retry', async () => {
		const sandbox = new DuckDB();
		const callbackError = new Error('DuckDB startup progress failed');
	...
	it('releases ownership after synchronous load and run dispatch failures', async () => {
		const sandbox = new DuckDB();
		const startupError = new Error('DuckDB load dispatch failed');
		...
		const executionError = new Error('DuckDB run dispatch failed');
	...
	it('treats false results and empty errors as terminal payloads', async () => {
		...
		const falseResult = sandbox.run('select false', false);
		worker.resolveRun(false);
		await expect(falseResult).resolves.toBe(false);

		const emptyError = sandbox.run('invalid query', false);
		worker.emit({ error: '' });
