...
class Php implements Sandbox {
				...
				this.worker.onerror = (event: ErrorEvent) => {
					const location =
						event.filename && event.lineno
							...
							: '';
					reject(
						`PHP worker script error: ${event.message || 'unknown error'}${location}`
					);
				};
				this.worker.onmessageerror = () => {
					reject('PHP worker message deserialization failed');
				};
	...
	run(
		...
		return new Promise<boolean | string>((resolve, reject) => {
			...
			const { programArgs } = resolveSandboxExecutionArgs('PHP', args, options);
			const _uid = ++this.uid;
