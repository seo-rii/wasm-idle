...
describe('R sandbox', () => {
	...
	it('keeps clear reusable but disposes an idle R runtime exactly once', async () => {
		const sandbox = new R();
		const output = vi.fn();
		const diagnostic = vi.fn();
		sandbox.output = output;
		...
		await sandbox.clear();
		expect(worker.terminate).not.toHaveBeenCalled();
		await expect(sandbox.run('cat("reusable\\n")', false)).resolves.toBe(true);
		sandbox.write('queued input\n');
		sandbox.eof();
		expect(sandbox.pendingInput).toEqual(['queued input\n']);
		expect(sandbox.pendingEof).toBe(true);
		...
		worker.terminate.mockImplementationOnce(() => {
			...
			reentrantLoad = sandbox.load({ r: { baseUrl: '/webr/reentrant/' } });
		...
		await expect(sandbox.load({ r: { baseUrl: '/webr/replacement/' } })).rejects.toMatchObject({
			name: 'RuntimeConfigurationError',
			code: 'runtime-configuration',
			phase: 'dispose',
			runtimeId: 'R'
		});
		await expect(sandbox.run('cat("unavailable\\n")', false)).rejects.toMatchObject({
			name: 'RuntimeConfigurationError',
			code: 'runtime-configuration',
			phase: 'dispose',
			runtimeId: 'R'
	...
	it('settles active R startup with one stable disposal cancellation', async () => {
		suppressAutoLoadAck = true;
		const sandbox = new R();
		const loading = sandbox.load({ r: { baseUrl: '/webr/test/' } });
	...
	it('settles an active R run, clears stdin, and ignores retained messages after disposal', async () => {
		const sandbox = new R();
		const output = vi.fn();
		const diagnostic = vi.fn();
		sandbox.output = output;
		...
		const worker = workerInstances[0];
		worker.postMessage.mockImplementationOnce(() => undefined);
		const running = sandbox.run('cat("active\\n")', false);
		const staleHandler = worker.onmessage;
