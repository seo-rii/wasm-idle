import type { JRuntimePreflightProfile } from '$lib/playground/assets';
import {
	AssetIntegrityError,
	AssetTooLargeError,
	CancelledError,
...
export const J_PREFLIGHT_PROTOCOL = 'wasm-idle-j-preflight' as const;
export const J_PREFLIGHT_PROTOCOL_VERSION = 1 as const;
export const J_PREFLIGHT_RUNTIME_ID = 'J' as const;

const MAX_MANIFEST_BYTES = 64 * 1024;
const VERIFIED_WASM_STORAGE_PATH = 'jamalgam.wasm.gz.bin';

export interface JRuntimePreflightRequest {
...
export interface JRuntimePreflightPayload {
	readonly protocol: typeof J_PREFLIGHT_PROTOCOL;
	readonly protocolVersion: typeof J_PREFLIGHT_PROTOCOL_VERSION;
...
export async function preflightJRuntimeAssets(
	request: JRuntimePreflightRequest
): Promise<JRuntimePreflightPayload> {
	if (!request || typeof request !== 'object') {
		throw new RuntimeConfigurationError('J runtime preflight request is required', {
			phase: 'asset',
			runtimeId: J_PREFLIGHT_RUNTIME_ID
		});
	}
	let baseUrl: URL;
	let manifestUrl: URL;
	try {
		...
		throw new RuntimeConfigurationError('J runtime asset URLs are invalid', {
			cause: error,
			phase: 'asset',
			runtimeId: J_PREFLIGHT_RUNTIME_ID
		});
	...
	if (
		...
		throw new RuntimeConfigurationError(
			'J runtime base must be a credential-free HTTP(S) directory URL without a query or fragment',
			{ phase: 'asset', runtimeId: J_PREFLIGHT_RUNTIME_ID }
		);
	}
	if (
		...
		throw new RuntimeConfigurationError(
			'J runtime manifest must be an HTTP(S) asset beneath the configured runtime base',
			{ phase: 'asset', runtimeId: J_PREFLIGHT_RUNTIME_ID }
		);
	}
	...
	if (
		...
		throw new RuntimeConfigurationError(
			'J runtime manifest path must be a normalized file beneath the runtime base',
			{ phase: 'asset', runtimeId: J_PREFLIGHT_RUNTIME_ID }
		);
	}
	if (manifestPath === 'jamalgam.js' || manifestPath === VERIFIED_WASM_STORAGE_PATH) {
		throw new RuntimeConfigurationError(
			'J runtime manifest and executable asset paths must be distinct',
			{ phase: 'asset', runtimeId: J_PREFLIGHT_RUNTIME_ID }
		);
	}
	...
	if (
		...
		!/^jsoftware-j-playground-[A-Za-z0-9._-]+$/u.test(profile.profileId) ||
		...
		throw new RuntimeConfigurationError('J runtime preflight profile is invalid', {
			phase: 'asset',
			runtimeId: J_PREFLIGHT_RUNTIME_ID
		});
	}
	for (const [label, receipt] of [
		...
		if (
			...
			throw new RuntimeConfigurationError(`J runtime ${label} preflight receipt is invalid`, {
				phase: 'asset',
	...
	if (
		...
		throw new RuntimeConfigurationError('J runtime Wasm preflight receipt is invalid', {
			phase: 'asset',
			profileId: profile.profileId,
			runtimeId: J_PREFLIGHT_RUNTIME_ID
		});
	...
	for (const [label, bytes, limit] of oversized) {
		if (bytes > limit) {
			throw new AssetTooLargeError(`J runtime ${label} exceeds the ${limit} byte limit`, {
				actual: bytes,
				limit,
				phase: 'asset',
				profileId: profile.profileId,
	...
	if (manifestUrl.search && manifestUrl.search !== expectedManifestQuery) {
		throw new RuntimeConfigurationError(
			'J runtime manifest query must be the pinned manifest fingerprint cache-buster',
			{
				phase: 'asset',
				profileId: profile.profileId,
				runtimeId: J_PREFLIGHT_RUNTIME_ID
	...
	const moduleRequestUrl = new URL('jamalgam.js', baseUrl);
	...
	const registry: RuntimeRegistryManifest = {
		schemaVersion: 2,
		manifestId: 'wasm-idle/j-preflight',
		...
		runtimes: [
			{
				...
				contracts: {
					...
					runtimeAssetKey: 'j',
					documentationId: 'J',
					syncTarget: 'sync:wasm-j',
					browserTestId: 'browser:j'
				}
			}
		]
	};
	...
	const timeout = setTimeout(() => {
		timedOut = true;
		controller.abort(new DOMException('J runtime preflight timed out', 'TimeoutError'));
	...
	try {
		...
		if (!manifestAsset || !moduleAsset || !wasmAsset) {
			throw new RuntimeConfigurationError(
				'J runtime preflight returned an incomplete asset set',
				{
					phase: 'asset',
					profileId: profile.profileId,
					runtimeId: J_PREFLIGHT_RUNTIME_ID
		...
		if (wasmAsset.bytes[0] !== 0x1f || wasmAsset.bytes[1] !== 0x8b) {
			throw new AssetIntegrityError('J runtime Wasm storage asset is not gzip data', {
				profileId: profile.profileId,
				runtimeId: J_PREFLIGHT_RUNTIME_ID
			});
		}
		...
		try {
			for (;;) {
				...
				if (offset + value.byteLength > wasmBytes.byteLength) {
					const error = new AssetIntegrityError(
						'J runtime Wasm gzip output exceeds its logical receipt',
						{ profileId: profile.profileId, runtimeId: J_PREFLIGHT_RUNTIME_ID }
					);
					try {
			...
			throw new AssetIntegrityError('J runtime Wasm gzip decompression failed', {
				cause: error,
				profileId: profile.profileId,
				runtimeId: J_PREFLIGHT_RUNTIME_ID
			});
		...
		if (offset !== wasmBytes.byteLength) {
			throw new AssetIntegrityError(
				'J runtime Wasm gzip output is shorter than its logical receipt',
				{
					profileId: profile.profileId,
					runtimeId: J_PREFLIGHT_RUNTIME_ID
				}
		...
		if (timedOut) {
			throw new TimeoutError(
				`J runtime preflight timed out after ${limits.assetTimeoutMs} ms`,
				{
					cause: error,
					phase: 'asset',
					profileId: profile.profileId,
		...
		if (request.signal?.aborted) {
			throw new CancelledError('J runtime preflight cancelled', {
