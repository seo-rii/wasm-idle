...
class Wasm implements Sandbox {
	...
	private readonly disposeCancellation = new CancelledError('WASM sandbox disposed', {
		phase: 'dispose',
		runtimeId: 'WASM',
		recoverable: false
	});
	...
	private beginOperation(phase: WasmOperation['phase']) {
		if (this.disposed) {
			throw new RuntimeConfigurationError('WASM sandbox is disposed', {
				phase: 'dispose',
				runtimeId: 'WASM'
			});
		}
						...
						if (actual > limits.maxOutputBytes) {
							failRun(
								new OutputLimitError(
									`WASM output exceeded ${limits.maxOutputBytes} bytes`,
									{
										actual,
						...
						if (actual > limits.maxDiagnostics) {
							failRun(
								new DiagnosticLimitError(
									`WASM diagnostics exceeded ${limits.maxDiagnostics} messages`,
									{
										actual,
