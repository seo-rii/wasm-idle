...
describe('WAT sandbox', () => {
	...
	it('terminates WAT output before exceeding the cumulative UTF-8 byte limit', async () => {
		const sandbox = new Wat();
		const output = vi.fn();
	...
	it('terminates WAT diagnostics before exceeding the message limit', async () => {
		const sandbox = new Wat();
	...
	it('keeps clear reusable but disposes an idle WAT runtime exactly once', async () => {
		const sandbox = new Wat();
		const output = vi.fn();
		const diagnostic = vi.fn();
		sandbox.output = output;
	...
	it('settles an active WAT run, clears stdin, and ignores retained messages after disposal', async () => {
		suppressAutoRunAck = true;
		const sandbox = new Wat();
		const output = vi.fn();
		const diagnostic = vi.fn();
