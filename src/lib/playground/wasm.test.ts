...
describe('WASM sandbox', () => {
	...
	it('terminates WASM diagnostics before exceeding the message limit', async () => {
		const sandbox = new Wasm();
		...
		const worker = workerInstances[0];
		worker.postMessage.mockImplementationOnce(() => undefined);
		const running = sandbox.run('AGFzbQ==', true, true, undefined, [], {
	...
	it('preserves an exact null pre-abort reason without changing idle WASM state', async () => {
		const sandbox = new Wasm();
		await sandbox.load('/absproxy/5173');
		const worker = workerInstances[0];
		const handler = worker.onmessage;
	...
	it('preserves replacement startup when the outer signal getter terminates WASM', async () => {
		const sandbox = new Wasm();
		await sandbox.load('/absproxy/5173');
		const retiredWorker = workerInstances[0];
		const reason = new Error('replace WASM during startup option snapshot');
		let replacement: Promise<void> | undefined;
		const options = {
			get signal() {
	...
	it('preserves the first cancellation and replacement across later WASM option failure', async () => {
		const sandbox = new Wasm();
		await sandbox.load('/absproxy/5173');
		const retiredWorker = workerInstances[0];
		const reason = new Error('replace WASM during execution option snapshot');
		const laterError = new Error('later WASM workspace getter failed');
		let replacement: Promise<void> | undefined;
		const options = {
			get limits() {
				sandbox.terminate(reason);
		...
		const superseded = sandbox.run('AGFzbQ==', false, true, undefined, [], options);
	...
	it('keeps clear reusable but disposes an idle WASM runtime exactly once', async () => {
		const sandbox = new Wasm();
		const output = vi.fn();
		const diagnostic = vi.fn();
		sandbox.output = output;
	...
	it('settles active WASM startup with one stable disposal cancellation', async () => {
		suppressAutoLoadAck = true;
		const sandbox = new Wasm();
		const loading = sandbox.load('/absproxy/5173');
	...
	it('settles an active WASM run, clears stdin, and ignores retained messages after disposal', async () => {
		const sandbox = new Wasm();
		const output = vi.fn();
		const diagnostic = vi.fn();
		sandbox.output = output;
