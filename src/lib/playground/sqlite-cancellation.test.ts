...
describe('SQLite cancellation and timeout contract', () => {
	beforeEach(() => {
		vi.useRealTimers();
	...
	it('rejects pre-aborted startup without creating or mutating a worker', async () => {
		...
		const reason = new Error('cancel SQLite before startup');
	...
	it('preserves an existing idle worker when replacement startup is pre-aborted', async () => {
		const sandbox = new Sqlite();
		await sandbox.load('/assets');
		const worker = workerInstances[0];
		const handler = worker.onmessage;
		const wasmUrl = sandbox.wasmUrl;
		const moduleUrl = sandbox.moduleUrl;
		const controller = new AbortController();
		const reason = new Error('cancel SQLite replacement startup');
		controller.abort(reason);
		...
		expect(sandbox.wasmUrl).toBe(wasmUrl);
		expect(sandbox.moduleUrl).toBe(moduleUrl);
	});

	it('rejects a pre-aborted run with its exact null reason and preserves the worker', async () => {
		const sandbox = new Sqlite();
		await sandbox.load('/assets');
		const worker = workerInstances[0];
		const handler = worker.onmessage;
		...
		await expect(
			sandbox.run('select 1;', false, true, undefined, [], {
	...
	it('aborts active startup with the exact reason and ignores late readiness', async () => {
		...
		const sandbox = new Sqlite();
		const controller = new AbortController();
		const reason = new Error('cancel active SQLite startup');
	...
	it('aborts an active run and permits a clean retry', async () => {
		...
		const worker = workerInstances[0];
		const controller = new AbortController();
		const reason = new Error('cancel active SQLite run');
	...
	it('preserves abort-time replacement and reason when a later option getter throws', async () => {
		const sandbox = new Sqlite();
		await sandbox.load('/assets');
		const retiredWorker = workerInstances[0];
		const controller = new AbortController();
		const reason = new Error('replace SQLite during option snapshot');
		const laterError = new Error('SQLite option getter failed after abort');
	...
	it('preserves an abort reason when an output callback reloads and then throws', async () => {
		...
		const retiredWorker = workerInstances[0];
		const controller = new AbortController();
		const reason = new Error('abort from SQLite output');
		const laterError = new Error('SQLite output failed after abort');
		let replacement: Promise<void> | undefined;
		sandbox.output = () => {
			controller.abort(reason);
			replacement = sandbox.load('/replacement');
