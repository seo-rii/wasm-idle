...
describe('PHP cancellation and timeout contract', () => {
	beforeEach(() => {
		vi.useRealTimers();
	...
	it('rejects pre-aborted startup without creating or mutating a worker', async () => {
		...
		const reason = new Error('cancel PHP before startup');
	...
	it('rejects a pre-aborted run with its exact null reason and preserves queued input', async () => {
		...
		await expect(
			sandbox.run('<?php echo 1;', false, true, undefined, [], {
	...
	it('aborts active startup with the exact reason and ignores late readiness', async () => {
		...
		const sandbox = new Php();
		const controller = new AbortController();
		const reason = new Error('cancel active PHP startup');
	...
	it('aborts an active run, clears owned stdin, and permits a clean retry', async () => {
		...
		const worker = workerInstances[0];
		const controller = new AbortController();
		const reason = new Error('cancel active PHP run');
		const running = sandbox.run('<?php echo 1;', false, true, undefined, [], {
			signal: controller.signal,
			stdin: 'fixed input\n'
		});
		const staleHandler = worker.onmessage;
	...
	it('preserves an abort reason when an output callback reloads and then throws', async () => {
		...
		const controller = new AbortController();
		const reason = new Error('abort from PHP output');
		const laterError = new Error('PHP output failed after abort');
