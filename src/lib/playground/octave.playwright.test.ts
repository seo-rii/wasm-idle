...
describe('wasm-idle Octave browser playwright integration', () => {
	it(
		'runs the real GNU Octave wasm runtime and connects stdin on the page path',
		{
			skip: process.env.WASM_IDLE_RUN_REAL_BROWSER_OCTAVE !== '1',
			meta: {
				browser: true,
				requiredBrowser: !(process.env.WASM_IDLE_RUN_REAL_BROWSER_OCTAVE !== '1')
			},
			timeout: 960_000
		},
		async () => {
			...
			await runWithBrowserProbeSessionLock(async () => {
				...
				if (!reuseProvidedBrowserUrl && serverMode === 'preview') {
					await runBrowserPreparationScripts(
						...
						{
							timeoutMs: Number(
								process.env.WASM_IDLE_OCTAVE_PREP_TIMEOUT_MS || '900000'
							)
						}
					);
				}
				...
				try {
					const summary = await runStdinBrowserProbe({
						...
						language: 'OCTAVE',
						runTimeoutMs: Number(
							process.env.WASM_IDLE_OCTAVE_RUN_TIMEOUT_MS || '480000'
						),
