...
const staticWorkerUrl = new URL('../../../static/wasm-awk/runner-worker.js', import.meta.url);
...
describe('GoAWK runner worker', () => {
	it('keeps the synced worker and cache-busting fingerprint current', async () => {
		...
		for (const fileName of [...manifest.files, 'runner-worker.js'].sort()) {
			...
			hash.update('\0');
			hash.update(await readRuntimeFile(fileName));
			hash.update('\n');
		}
		expect(hash.digest('hex').slice(0, 16)).toBe(WASM_AWK_ASSET_VERSION);
