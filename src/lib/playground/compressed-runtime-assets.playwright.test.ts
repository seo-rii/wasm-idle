...
const compressedRuntimeAssetSamples = [
	...
	'wasm-dotnet/runtime/csharp/dotnet.native.wasm',
...
const compressedAssetTestTimeoutMs = Number(
	process.env.WASM_IDLE_COMPRESSED_ASSET_TEST_TIMEOUT_MS || '600000'
);
			...
			await runWithBrowserProbeSessionLock(async () => {
				...
				try {
					...
					const expectedAssets = await Promise.all(
						[
							...compressedRuntimeAssetSamples,
							...(await discoverBuildAssetSamples())
						].map((assetPath) => expectedAssetMetadata(assetPath))
					);
					const perAssetTimeoutMs = Number(
						process.env.WASM_IDLE_COMPRESSED_ASSET_TIMEOUT_MS || '120000'
					);
					...
					for (const asset of expectedAssets) {
						responses.push(
							await page.evaluate(
								async ({ asset, timeoutMs }) => {
									const controller = new AbortController();
									const timeout = setTimeout(() => controller.abort(), timeoutMs);
									try {
										const response = await fetch(
											new URL(asset.path, location.href),
											{
										...
										throw new Error(
											`Failed to fetch compressed runtime asset ${asset.path}: ${error instanceof Error ? error.message : String(error)}`
										);
									} finally {
										clearTimeout(timeout);
									}
								},
								{ asset, timeoutMs: perAssetTimeoutMs }
							)
						);
					}
							...
							await page.evaluate(
								async ({ aliasPath, canonicalPath }) => {
									...
									const aliasResponse = await fetch(
										new URL(aliasPath, location.href)
									);
