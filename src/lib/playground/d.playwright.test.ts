...
describe('wasm-idle D browser playwright integration', () => {
	it(
		'runs the real D page path through bundled LDC and Emscripten LLD assets',
		{
			skip: process.env.WASM_IDLE_RUN_REAL_BROWSER_D !== '1',
			meta: {
				browser: true,
				requiredBrowser: !(process.env.WASM_IDLE_RUN_REAL_BROWSER_D !== '1')
			},
			timeout: 720_000
		},
		async () => {
