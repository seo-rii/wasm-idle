...
export const ELIXIR_RUNTIME_ASSET_NAMES = ['bundle.avm', 'AtomVM.mjs', 'AtomVM.wasm'] as const;

export type ElixirRuntimeAssetName = (typeof ELIXIR_RUNTIME_ASSET_NAMES)[number];
...
export function snapshotElixirRuntimeAssetReceipts(
	...
	const expectedAssets = [...ELIXIR_RUNTIME_ASSET_NAMES].sort();
	if (
		receivedAssets.length !== expectedAssets.length ||
		receivedAssets.some((asset, index) => asset !== expectedAssets[index])
	) {
		throw new TypeError('Elixir runtime requires exactly three asset receipts');
	}
	const receipts = Object.fromEntries(
		ELIXIR_RUNTIME_ASSET_NAMES.map((asset) => {
			...
			if (
				...
				throw new TypeError(`Elixir runtime asset receipt is invalid for ${asset}`);
			}
			return [
				asset,
				Object.freeze({
