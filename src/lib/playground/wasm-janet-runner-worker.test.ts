...
async function createHarnessWorker() {
	...
	const importStatement = "const runtimeModule = await import(assetUrl(baseUrl, 'janet.js'));";
	if (!workerSource.includes(importStatement)) {
		throw new Error('Janet worker module import seam was not found.');
	}
	const harnessSource = workerSource.replace(
		importStatement,
		'const runtimeModule = { default: globalThis.__createJanetModule };'
	);
	const harness = `
...
describe('Janet runner worker', () => {
	it('keeps the synced worker and cache-busting fingerprint current', async () => {
		...
		expect(hash.digest('hex').slice(0, 16)).toBe(WASM_JANET_ASSET_VERSION);
	...
	it('streams a prompt before reading shared-ring input', async () => {
		...
		const messages = await runHarness(
			{
				baseUrl: 'https://runtime.example/wasm-janet/',
				code: '(getline)',
	...
	it('fails closed on a malformed shared stdin descriptor', async () => {
		...
		expect(messages).toEqual([{ error: 'Invalid Janet streaming stdin channel.' }]);
	});
});
