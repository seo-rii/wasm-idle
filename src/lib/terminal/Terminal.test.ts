...
describe('Terminal source', () => {
	...
	it('keeps a hidden transcript mirror for browser debugging and Playwright assertions', () => {
		...
		expect(source).toMatch(
			/kotlinCheerpjLoaderUrl:\s+currentRuntimeAssets\?\.kotlin\?\.cheerpjLoaderUrl \|\| '',/
		);
		expect(source).toMatch(
	...
	it('uses rust-specific progress windows instead of jumping straight to the prepare band', () => {
		...
		expect(source).toMatch(
			/language === 'RUST' \|\|\s+language === 'GO' \|\|\s+language === 'CSHARP' \|\|\s+language === 'FSHARP' \|\|\s+language === 'SCALA' \|\|\s+language === 'KOTLIN' \|\|\s+language === 'TINYGO' \|\|\s+language === 'OCAML'\s+\? phaseProgress\(prog, 0, 0\.05\)\s+: phaseProgress\(prog, 0, 0\.85\)/
		);
		expect(source).toMatch(
			/language === 'RUST' \|\|\s+language === 'GO' \|\|\s+language === 'CSHARP' \|\|\s+language === 'FSHARP' \|\|\s+language === 'SCALA' \|\|\s+language === 'KOTLIN' \|\|\s+language === 'TINYGO' \|\|\s+language === 'OCAML'\s+\? phaseProgress\(prog, 0\.05, 0\.99\)\s+: phaseProgress\(prog, 0\.85, 0\.99\)/
		);
	});
});
