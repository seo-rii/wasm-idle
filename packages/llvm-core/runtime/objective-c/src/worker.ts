...
let workerDependencies: ObjectiveCWorkerDependencies | null = null;

const postMessage = (message: unknown) => workerScope?.postMessage(message);
const configureWorkerRuntimeAssets = (config: ObjectiveCWorkerRuntimeAssetConfig | null) =>
	workerDependencies?.configureRuntimeAssets(config);
const handleWorkerAssetMessage = (message: unknown) =>
	workerDependencies?.handleAssetMessage(message) ?? false;
const waitForBufferedStdin = (buffer: Int32Array | null, requestInput: () => void) => {
	if (!workerDependencies) throw new Error('Objective-C worker dependencies are not installed.');
	return workerDependencies.waitForStdin(buffer, requestInput);
};

export interface ObjectiveCWorkerAssetConfig {
	libobjcUrl: string;
	headersUrl: string;
...
function resolveHostedAssetUrl(value: string, label: string) {
...
function resolveObjectiveCAssetConfig(assets: ObjectiveCWorkerAssetConfig) {
	return {
		...
		libgnustepBaseUrl: resolveHostedAssetUrl(
			assets.libgnustepBaseUrl,
			'Objective-C GNUstep Base URL'
		),
		libgnustepBaseObjectUrl: resolveHostedAssetUrl(
			assets.libgnustepBaseObjectUrl,
			'Objective-C GNUstep Base object URL'
		),
		foundationHeadersUrl: resolveHostedAssetUrl(
...
async function loadObjectiveCRuntime(
	clangAssets: ObjectiveCWorkerRuntimeAssetConfig | undefined,
...
const handleObjectiveCWorkerMessage = async (event: { data: any }) => {
...
export function installObjectiveCWorker(
	scope: ObjectiveCWorkerScope,
	dependencies: ObjectiveCWorkerDependencies
) {
	workerScope = scope;
