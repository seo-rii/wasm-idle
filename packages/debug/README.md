...
the UI. The adapter controller and playground host also ignore a superseded request's late failure
instead of surfacing it after the current update succeeds. Updates for different workspace files
remain independent, while failures from the latest request still propagate.
