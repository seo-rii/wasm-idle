...
export interface WatWorkerOptions {
...
const WAT_HOVER: Record<string, string> = {
...
function parseWatDiagnostics(message: string, fileName: string): LspDiagnostic[] {
...
export function createWatWorkerService(): WorkerLanguageService {
	...
	return {
		...
		async initialize(options, context) {
			features = (options as WatWorkerOptions | undefined)?.features;
