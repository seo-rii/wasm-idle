...
const diagnosticSeverity = (category: ts.DiagnosticCategory): 1 | 2 | 3 | 4 => {
	switch (category) {
...
const scriptElementKindValue = (kind: ts.ScriptElementKind) => String(kind);

const completionKind = (kind: ts.ScriptElementKind) => {
	const kinds = ts.ScriptElementKind;
	if (kind === kinds.keyword) return 14;
	if (kind === kinds.moduleElement || kind === kinds.externalModuleName) return 9;
	if (kind === kinds.classElement) return 7;
	if (kind === kinds.interfaceElement) return 8;
	if (kind === kinds.enumElement) return 13;
	if (kind === kinds.enumMemberElement) return 20;
	if (kind === kinds.memberFunctionElement || scriptElementKindValue(kind) === 'method') return 2;
	if (kind === kinds.functionElement || kind === kinds.localFunctionElement) return 3;
	if (kind === kinds.constructorImplementationElement) return 4;
	if (kind === kinds.memberVariableElement || kind === kinds.memberGetAccessorElement) return 5;
	if (kind === kinds.constElement) return 21;
	if (
		kind === kinds.variableElement ||
		kind === kinds.localVariableElement ||
		kind === kinds.parameterElement
	) {
		return 6;
	}
	if (kind === kinds.typeElement || kind === kinds.typeParameterElement) return 25;
	if (kind === kinds.string) return 1;
	return 6;
};

const symbolKind = (kind: ts.ScriptElementKind) => {
	const kinds = ts.ScriptElementKind;
	if (kind === kinds.moduleElement || kind === kinds.externalModuleName) return 2;
	if (kind === kinds.classElement) return 5;
	if (scriptElementKindValue(kind) === 'method' || kind === kinds.memberFunctionElement) return 6;
	if (kind === kinds.memberVariableElement) return 8;
	if (kind === kinds.constructorImplementationElement) return 9;
	if (kind === kinds.enumElement) return 10;
	if (kind === kinds.interfaceElement) return 11;
	if (kind === kinds.functionElement || kind === kinds.localFunctionElement) return 12;
	if (kind === kinds.constElement) return 14;
	return 13;
};
...
export function createTypeScriptWorkerService(
	...
	const documentForFileName = (fileName: string) => {
		const normalized = normalizePath(fileName);
		for (const document of context.documents.values()) {
			if (normalizePath(uriToPath(document.uri)) === normalized) return document;
		}
		return undefined;
	};

	const sourceTextForFileName = (fileName: string) =>
		documentForFileName(fileName)?.text ||
	...
	const host: ts.LanguageServiceHost = {
		...
		getScriptFileNames: () => [
			...new Set([
				...Array.from(context.documents.values(), (document) =>
					normalizePath(uriToPath(document.uri))
				),
				...Object.keys(extraLibs).map(normalizePath)
			])
		],
		getScriptVersion: (fileName) =>
			String(documentForFileName(fileName)?.version ?? (fileName in extraLibs ? 1 : 0)),
	...
	const locationForSpan = (fileName: string, textSpan: ts.TextSpan) => {
		const source = sourceTextForFileName(fileName);
		return {
	...
	return {
		...
		async initialize(options, nextContext) {
			const config = (options || {}) as TypeScriptWorkerOptions;
		...
		definition(document, position) {
			...
			return (
				languageService
					...
					?.map((definition) =>
						locationForSpan(definition.fileName, definition.textSpan)
					) || null
			);
		},
				...
				item.childItems?.map((child) => ({
					...
					range: {
						start: positionAt(document.text, child.spans[0]?.start || 0),
							...
							document.text,
							(child.spans[0]?.start || 0) + (child.spans[0]?.length || 0)
						)
					},
					selectionRange: {
...
export function replaceWholeDocument(document: LspDocument, newText: string) {
