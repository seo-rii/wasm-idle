import { resolvePerlLanguageServerAssetConfig } from '../runtime.js';
...
function snapshotRunnerReceipt(value: RuntimeAssetIntegrityEntry): RuntimeAssetIntegrityEntry {
	if (
		...
		throw new TypeError('Perl language server runner receipt is invalid');
	}
...
function abortReason(signal: AbortSignal): unknown {
	return (
		signal.reason ?? new DOMException('Perl language server startup cancelled', 'AbortError')
	);
}
...
function ownedTransferBuffer(bytes: Uint8Array, label: string): ArrayBuffer {
	if (
		...
		throw new TypeError(`Perl language server ${label} bytes are not exclusively owned`);
	}
	return bytes.buffer;
}
...
export async function getPerlLanguageServer(
	...
	const resolved = resolvePerlLanguageServerAssetConfig(options, currentUrl);
	const workerReceipt = snapshotRunnerReceipt(resolved.workerReceipt);
	const configuredMaxAssetBytes = hostOptions?.maxAssetBytes ?? PERL_MAX_ASSET_BYTES;
	if (!Number.isSafeInteger(configuredMaxAssetBytes) || configuredMaxAssetBytes <= 0) {
		throw new TypeError('Perl language server maxAssetBytes must be a positive safe integer');
	}
	const maxAssetBytes = Math.min(configuredMaxAssetBytes, PERL_MAX_ASSET_BYTES);
	if ((workerReceipt.bytes as number) > maxAssetBytes) {
		throw new TypeError('Perl language server runner exceeds maxAssetBytes');
	}
	...
	try {
		...
		transfer = [
			...
			ownedTransferBuffer(runtimePreflight.dataBytes, 'data'),
