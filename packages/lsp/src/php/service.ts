...
const PHP_HOVER: Record<string, string> = {
...
const skipQuotedString = (text: string, start: number, quote: string): number => {
	let index = start + 1;
...
function scanPhpSyntax(text: string): LspDiagnostic[] {
	const diagnostics: LspDiagnostic[] = [];
	const stack: Array<{ character: string; offset: number }> = [];
	let index = 0;

	const pushDiagnostic = (offset: number, message: string) => {
	...
	for (const item of stack.reverse()) {
		pushDiagnostic(
			item.offset,
			`Unclosed '${item.character}', expected '${OPEN_TO_CLOSE[item.character]}'`
		);
	}
	return diagnostics;
}

export function createPhpWorkerService(): WorkerLanguageService {
	let version = '8.4';
	...
	return {
		...
		initialize(options, context) {
			const config = (options || {}) as PhpWorkerOptions;
