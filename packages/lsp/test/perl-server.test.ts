...
import { PERL_MAX_ASSET_BYTES } from '@wasm-idle/core';
import {
...
import {
	...
	perlTestAssetBytes,
	type PerlTestAssetName
} from './perl-fixture.js';
...
function installAssetFetch(overrides: Partial<Record<PerlTestAssetName, Uint8Array>> = {}) {
	const fetchMock = vi.fn(async (input: string | URL | Request) => {
		const requestUrl = new URL(
			typeof input === 'string' || input instanceof URL ? input : input.url
		);
		const response = createPerlTestAssetResponse(requestUrl, overrides);
...
describe('getPerlLanguageServer host preflight', () => {
	it('verifies all runtime and runner assets before creating the outer worker', async () => {
		...
		const handle = await getPerlLanguageServer({ rootUrl, currentUrl, createWorker });

		expect(createWorker).toHaveBeenCalledOnce();
		expect(new Set(fetchMock.mock.calls.map(([input]) => String(input)))).toEqual(
			new Set([
				`https://app.example.com/wasm-idle/wasm-perl/runtime-manifest.v2.json?v=${BUNDLED_PERL_RUNTIME_PROFILE.manifestFingerprint}`,
				`https://app.example.com/wasm-idle/wasm-perl/emperl.js.gz.bin?v=${BUNDLED_PERL_RUNTIME_PROFILE.javascriptReceipt.sha256}`,
				`https://app.example.com/wasm-idle/wasm-perl/emperl.wasm.gz.bin?v=${BUNDLED_PERL_RUNTIME_PROFILE.wasmReceipt.sha256}`,
				`https://app.example.com/wasm-idle/wasm-perl/emperl.data.gz.bin?v=${BUNDLED_PERL_RUNTIME_PROFILE.dataReceipt.sha256}`,
		...
		expect(init.options.runnerWorkerBytes).toEqual(perlTestAssetBytes['runner-worker.js']);
	...
	it('does not create the outer worker while the runner preflight is pending', async () => {
		...
		const worker = new ReadyWorker();
		const createWorker = vi.fn(() => worker as unknown as Worker);
		const startup = getPerlLanguageServer({ rootUrl, currentUrl, createWorker });
	...
	it.each([
		...
		['emperl.js.gz.bin', 'Runtime asset emperl.js.gz.bin compressed SHA-256 mismatch']
	] as const)('rejects corrupt %s before creating the outer worker', async (asset, message) => {
		const corruptBytes = Uint8Array.from(perlTestAssetBytes[asset]);
		corruptBytes[0] ^= 0xff;
		installAssetFetch({ [asset]: corruptBytes });
		const createWorker = vi.fn();

		await expect(getPerlLanguageServer({ rootUrl, currentUrl, createWorker })).rejects.toThrow(
			message
		);
		expect(createWorker).not.toHaveBeenCalled();
	});
	...
	it('preserves an already-aborted caller reason without fetching or creating a worker', async () => {
		...
		const reason = new DOMException('Perl startup cancelled', 'AbortError');
		controller.abort(reason);

		await expect(
			getPerlLanguageServer({ rootUrl, currentUrl, createWorker, signal: controller.signal })
		).rejects.toBe(reason);
	...
	it('aborts every pending asset request when the caller cancels mid-preflight', async () => {
		const controller = new AbortController();
		const reason = new DOMException('cancel pending Perl assets', 'AbortError');
		...
		vi.stubGlobal('fetch', fetchMock);
		const createWorker = vi.fn();
		const startup = getPerlLanguageServer({
			rootUrl,
			currentUrl,
			createWorker,
	...
	it('aborts sibling runtime requests when runner preflight fails', async () => {
		...
		await expect(
			getPerlLanguageServer({ rootUrl, currentUrl, createWorker, onStatus })
	...
	it('does not cache a failed preflight and succeeds on a later call', async () => {
		...
		const fetchMock = vi.fn(async (input: string | URL | Request) => {
			...
			const overrides: Partial<Record<PerlTestAssetName, Uint8Array>> = {};
			if (corruptRunner && requestUrl.pathname.endsWith('/runner-worker.js')) {
				const bytes = Uint8Array.from(perlTestAssetBytes['runner-worker.js']);
				bytes[0] ^= 0xff;
	...
	it('times out preflight without constructing the outer worker', async () => {
		...
		await expect(
			getPerlLanguageServer({ rootUrl, currentUrl, createWorker, assetTimeoutMs: 5 })
