...
async function loadGoCompiler(compilerUrl) {
    const module = await import(/* @vite-ignore */ compilerUrl);
    const factory = typeof module.createGoCompiler === 'function'
...
export function createGoWorkerService() {
    let compiler = null;
    let target = 'wasip1/wasm';
    let lastKey = '';
    let lastDiagnostics = [];
    return {
        ...
        async diagnostics(document, context) {
            ...
            const result = await compiler.compile({
                ...
                log: false,
                onProgress(progress) {
                    context.reportProgress(progress.stage || 'go-diagnostics', progress.completed, progress.total);
                }
            });
            lastKey = key;
