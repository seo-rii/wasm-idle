...
const normalizeFileName = (value) => value.replaceAll('\\', '/').replace(/^\/workspace\//u, '').replace(/^\/+/u, '');
...
const wordAt = (text, position) => {
    ...
    return ((line.slice(0, character).match(/[A-Za-z0-9_$]+$/u)?.[0] || '') +
        (line.slice(character).match(/^[A-Za-z0-9_$]+/u)?.[0] || ''));
};
function parseAssemblyScriptDiagnostics(message) {
    const diagnostics = [];
...
export function createAssemblyScriptWorkerService(loadCompiler = importAssemblyScriptCompiler) {
    let compiler;
    let context;
    let extraFiles = {};
    const collectFiles = () => {
    ...
    return {
        ...
        async initialize(options, nextContext) {
            context = nextContext;
            extraFiles = Object.fromEntries(Object.entries(options?.extraFiles || {}).map(([fileName, source]) => [normalizeFileName(fileName), source]));
