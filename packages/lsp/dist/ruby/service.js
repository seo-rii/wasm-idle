...
async function loadRubyWasmChecker(options) {
    if (!options.wasmUrl) {
    ...
    return {
        async check(code, fileName) {
            ...
            const wasi = new WASI(['ruby.wasm'], [], [new OpenFile(new File([])), new OpenFile(new File([])), new OpenFile(new File([]))], { debug: false });
            ...
            try {
                vm.eval(`RubyVM::InstructionSequence.compile(${JSON.stringify(code)}, ${JSON.stringify(fileName)}, ${JSON.stringify(fileName)}, 1)`);
                return [];
            }
            catch (error) {
...
export function createRubyWorkerService(loadChecker = loadRubyWasmChecker) {
    let checker = null;
    let lastKey = '';
    let lastDiagnostics = [];
    return {
