...
export function startWorkerLanguageServer(service, scope = globalThis) {
    ...
    const scheduleDiagnostics = (document) => {
        ...
        if (debug) {
            console.debug(`[wasm-idle:lsp-worker:${service.name}] schedule diagnostics uri=${document.uri} version=${document.version} bytes=${document.text.length}`);
        }
        setTimeout(async () => {
            try {
                if (diagnosticVersions.get(document.uri) !== version ||
                    ...
                    if (debug) {
                        console.debug(`[wasm-idle:lsp-worker:${service.name}] diagnostics skipped stale uri=${document.uri} version=${document.version}`);
                    }
                    return;
                }
                if (debug) {
                    console.debug(`[wasm-idle:lsp-worker:${service.name}] diagnostics start uri=${document.uri} version=${document.version}`);
                }
                const diagnostics = await service.diagnostics?.(document, context);
                if (debug) {
                    console.debug(`[wasm-idle:lsp-worker:${service.name}] diagnostics done uri=${document.uri} version=${document.version} count=${diagnostics?.length || 0}`);
                }
                if (diagnosticVersions.get(document.uri) === version &&
                    documents.get(document.uri)?.version === document.version) {
                ...
                if (debug) {
                    console.error(`[wasm-idle:lsp-worker:${service.name}] diagnostics failed`, error);
                }
                if (diagnosticVersions.get(document.uri) !== version)
                    return;
    ...
    const handleNotification = async (message) => {
        ...
        switch (message.method) {
            ...
            case 'textDocument/didOpen': {
                ...
                if (debug) {
                    console.debug(`[wasm-idle:lsp-worker:${service.name}] didOpen uri=${document.uri} version=${document.version} bytes=${document.text.length}`);
                }
            ...
            case 'textDocument/didChange': {
                ...
                if (debug) {
                    console.debug(`[wasm-idle:lsp-worker:${service.name}] didChange uri=${document.uri} version=${document.version} bytes=${document.text.length}`);
                }
                documents.set(document.uri, document);
                scheduleDiagnostics(document);
                return;
    ...
    scope.addEventListener('message', (event) => {
        ...
        if (message.type === 'init') {
            if (initialized)
                return;
            initialized = true;
            const initOptions = message.options;
            debug =
