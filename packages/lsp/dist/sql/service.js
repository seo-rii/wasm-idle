...
const severityFor = (severity) => severity === 'warning' ? 2 : severity === 'info' ? 3 : 1;
const positionForSqlError = (text, diagnostic) => {
...
const diagnosticFor = (text, diagnostic) => {
...
async function loadSqlJsEngine(options) {
...
export function createSqlWorkerService(loadEngine = loadSqlJsEngine) {
    let engine = null;
    let dialect = 'sqlite';
    let lastKey = '';
    let lastDiagnostics = [];
    return {
        ...
        async initialize(options, context) {
            ...
            context.reportProgress(dialect === 'duckdb' ? 'load-duckdb-language-service' : 'load-sqlite-language-service');
        ...
        async diagnostics(document) {
            ...
            lastKey = key;
            const diagnostics = await engine.validate(document.text, document.uri.split('/').pop() || 'main.sql');
