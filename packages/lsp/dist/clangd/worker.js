...
import { CLANGD_CPP_FILE_PATH, CLANGD_WORKSPACE_PATH, createClangdCompileFlags, normalizeClangdBaseUrl } from './config.js';
...
const describeMessage = (data) => {
    ...
    if (record.id !== undefined)
        return `response:${String(record.id)}`;
    return 'unknown';
};
const isJsonRpcMessage = (data) => {
    const record = data;
    return !!record && typeof record === 'object' && record.jsonrpc === '2.0';
};
const debugLog = (...args) => {
    if (debugEnabled)
        console.debug('[wasm-idle:clangd-worker]', ...args);
};
const stdin = () => {
...
const syncWorkspaceFile = (filePath) => {
    if (!clangdRuntime)
        return;
    ...
    const directoryPath = lastSlash > 0 ? normalizedFilePath.slice(0, lastSlash) : CLANGD_WORKSPACE_PATH;
    clangdRuntime.FS.mkdirTree(directoryPath);
    clangdRuntime.FS.writeFile(normalizedFilePath, '');
};
async function fetchAssetBytes(baseUrl, asset) {
...
self.addEventListener('message', async (event) => {
    ...
    try {
        ...
        const jsDataUrl = URL.createObjectURL(new Blob([jsSource], { type: 'text/javascript;charset=utf-8' }));
        ...
        clangdRuntime = await Clangd({
            ...
            locateFile: (path, prefix) => path.endsWith('.wasm') ? wasmDataUrl : `${prefix}${path}`,
            stdinReady,
            stdin,
            stdout,
            stderr,
        ...
        clangdRuntime.FS.writeFile(`${CLANGD_WORKSPACE_PATH}/.clangd`, JSON.stringify({
            CompileFlags: {
