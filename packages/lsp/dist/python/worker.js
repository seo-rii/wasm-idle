...
function emitTransportMessage(payload) {
    try {
...
function installPythonServerPackage(currentPyodide) {
    currentPyodide.FS.mkdirTree('/wasm_idle_lsp/wasm_idle_python_lsp');
    currentPyodide.FS.writeFile('/wasm_idle_lsp/wasm_idle_python_lsp/__init__.py', packageInitSource);
    currentPyodide.FS.writeFile('/wasm_idle_lsp/wasm_idle_python_lsp/server.py', packageServerSource);
}
async function bootstrapPythonBridge(pyodideBaseUrl) {
    if (bridge)
        return;
    if (initPromise)
        return initPromise;
