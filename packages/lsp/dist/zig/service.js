...
const normalizeWorkspacePath = (value, fallback = 'main.zig') => {
...
const addFile = (root, filePath, data) => {
...
function parseZigDiagnostics(output) {
    const diagnostics = [];
...
async function fetchBytes(url, stage, reportProgress) {
...
async function unzipStdDirectory(source) {
...
async function loadDefaultZigCompilerHost(options, context) {
    ...
    return {
        async compile(request) {
            ...
            const instance = instantiateResult(await WebAssembly.instantiate(compilerModule, {
                wasi_snapshot_preview1: zigWasi.wasiImport
            }));
            const exitCode = Number(zigWasi.start(instance) || 0);
...
export function createZigWorkerService(loadCompilerHost = loadDefaultZigCompilerHost) {
    let compiler = null;
    let targetTriple = 'wasm64-wasi';
    let compileArgs = [];
    let lastKey = '';
    let lastDiagnostics = [];
    ...
    return {
        ...
        async diagnostics(document, context) {
            ...
            const result = await compiler.compile({
                ...
                log: false,
                onProgress(progress) {
                    context.reportProgress(progress.stage || 'zig-diagnostics', progress.completed, progress.total);
                }
            });
