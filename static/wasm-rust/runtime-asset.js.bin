...
const runtimeAssetReceipts = new Map();
const cacheIdentityIds = new WeakMap();
...
function resolveVersionedRuntimeAssetUrl(baseUrl, assetPath) {
...
export function registerRuntimeAssetReceipts(runtimeBaseUrl, receipts) {
    ...
    for (const [assetPath, sourceReceipt] of Object.entries(receipts)) {
        ...
        const versionedAssetUrl = new URL(resolveVersionedRuntimeAssetUrl(runtimeBaseUrl, assetPath));
...
export function hasRegisteredRuntimeAssetReceipt(assetUrl) {
    return runtimeAssetReceipts.has(new URL(assetUrl.toString()).href);
}
export function runtimeAssetReceiptIdentity(receipt) {
    return receipt
        ? [
            receipt.bytes,
            receipt.sha256,
...
export function createRuntimeAssetCacheKey(assetUrl, fetchImpl, options = {}) {
...
function runtimeAssetAbortReason(signal) {
...
async function readBoundedStream(stream, assetLabel, maxAssetBytes, sizeKind, total, onProgress, signal) {
    ...
    try {
        while (true) {
            ...
            if (!Number.isSafeInteger(nextLength) || nextLength > maxAssetBytes) {
                const error = new Error(`wasm-rust runtime asset ${assetLabel} ${sizeKind} size exceeds the ${maxAssetBytes} byte limit`);
                cancelReader(error);
                throw error;
            }
            if (nextLength > bytes.byteLength) {
                const nextCapacity = Math.min(maxAssetBytes, Math.max(nextLength, Math.max(bytes.byteLength * 2, 1)));
...
async function readResponseBytes(response, assetLabel, maxAssetBytes, onProgress, signal) {
    throwIfRuntimeAssetAborted(signal);
    ...
    if (contentLength !== null) {
        ...
        if (!/^\d+$/u.test(normalized) || !Number.isSafeInteger(parsed)) {
            ...
            throwIfRuntimeAssetAborted(signal);
            throw new Error(`wasm-rust runtime asset has an invalid Content-Length: ${contentLength}`);
        }
        total = parsed;
    }
    if (total !== undefined && total > maxAssetBytes) {
        cancelResponseBody(response);
        throwIfRuntimeAssetAborted(signal);
        throw new Error(`wasm-rust runtime asset ${assetLabel} download size exceeds the ${maxAssetBytes} byte limit`);
    }
    if (!response.body) {
        let cancelOnAbort;
        const aborted = signal
    ...
    return readBoundedStream(response.body, assetLabel, maxAssetBytes, 'download', total, onProgress, signal);
}
...
async function verifyRuntimeAssetReceipt(assetLabel, bytes, receipt, stage, signal) {
    const expectedBytes = stage === 'logical' ? (receipt.uncompressedBytes ?? receipt.bytes) : receipt.bytes;
    const expectedSha256 = stage === 'logical' ? (receipt.uncompressedSha256 ?? receipt.sha256) : receipt.sha256;
    if (bytes.byteLength !== expectedBytes) {
        throw new Error(`wasm-rust runtime asset ${assetLabel} ${stage} byte length differs from its receipt`);
    }
    if ((await sha256Hex(bytes, signal)) !== expectedSha256) {
        throw new Error(`wasm-rust runtime asset ${assetLabel} ${stage} SHA-256 differs from its receipt`);
    }
    return bytes;
}
export async function fetchRuntimeAssetBytes(assetUrl, assetLabel, fetchImpl = fetch, allowCompressedFallback = true, onProgress, options = {}) {
    ...
    if (resolvedAssetUrlObject.protocol !== 'http:' &&
        resolvedAssetUrlObject.protocol !== 'https:') {
        throw new Error(`unsupported wasm-rust runtime asset URL scheme: ${resolvedAssetUrlObject.protocol}`);
    }
    ...
    if (hasReceipt && (receipt.bytes > maxAssetBytes || expectedLogicalBytes > maxAssetBytes)) {
        throw new Error(`wasm-rust runtime asset ${assetLabel} receipt exceeds the ${maxAssetBytes} byte limit`);
    }
    ...
    if (response.url) {
        ...
        try {
            ...
            cancelResponseBody(response);
            throwIfRuntimeAssetAborted(options.signal);
            throw new Error(`wasm-rust runtime asset ${assetLabel} returned an invalid final URL: ${response.url}`);
        }
        if (finalUrl.href !== resolvedAssetUrl) {
            cancelResponseBody(response);
            throwIfRuntimeAssetAborted(options.signal);
            throw new Error(`wasm-rust runtime asset ${assetLabel} returned an unexpected final URL: ${response.url}`);
        }
    }
    if (!response.ok) {
        cancelResponseBody(response);
        ...
        if (!hasReceipt &&
            ...
            try {
                return await fetchRuntimeAssetBytes(compressedAssetUrl, assetLabel, fetchImpl, false, onProgress, options);
            }
            catch {
                throwIfRuntimeAssetAborted(options.signal);
            }
    ...
    const assetBytes = await readResponseBytes(response, assetLabel, downloadLimit, onProgress, options.signal);
    ...
    let shouldDecompress = resolvedAssetUrlObject.pathname.endsWith('.gz') &&
        assetBytes.byteLength >= 2 &&
        assetBytes[0] === 0x1f &&
        assetBytes[1] === 0x8b;
    if (hasReceipt) {
        const deliveredSha256 = await sha256Hex(assetBytes, options.signal);
        const matchesStorage = assetBytes.byteLength === receipt.bytes && deliveredSha256 === receipt.sha256;
        const hasDistinctLogicalReceipt = receipt.uncompressedBytes !== undefined;
        const matchesLogical = hasDistinctLogicalReceipt &&
        ...
        if (!matchesStorage) {
            ...
            const matchesLogicalLength = hasDistinctLogicalReceipt && assetBytes.byteLength === receipt.uncompressedBytes;
            throw new Error(matchesStorageLength
        ...
        if (assetBytes.byteLength < 2 || assetBytes[0] !== 0x1f || assetBytes[1] !== 0x8b) {
            throw new Error(`wasm-rust runtime asset ${assetLabel} storage bytes match a compressed receipt but are not gzip data`);
        }
    ...
    try {
        ...
        const decompressedBytes = await readBoundedStream(decompressedStream, assetLabel, hasReceipt ? expectedLogicalBytes : maxAssetBytes, 'decompressed', undefined, undefined, options.signal);
        return hasReceipt
            ? await verifyRuntimeAssetReceipt(assetLabel, decompressedBytes, receipt, 'logical', options.signal)
            : decompressedBytes;
    }
    catch (error) {
        throwIfRuntimeAssetAborted(options.signal);
...
export async function fetchRuntimeAssetJson(assetUrl, assetLabel, fetchImpl = fetch, onProgress, options = {}) {
    return JSON.parse(new TextDecoder().decode(await fetchRuntimeAssetBytes(assetUrl, assetLabel, fetchImpl, true, onProgress, options)));
}
